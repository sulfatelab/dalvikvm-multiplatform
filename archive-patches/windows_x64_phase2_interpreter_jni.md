# Windows x64 Phase 2 — Interpreter JNI helpers (A3)
...
Windows x64 PE builds cannot use ART quick JNI / `art_jni_dlsym_lookup_stub` (SysV + `%gs` Thread TLS).
...
// (that stub needs %gs Thread TLS and a quick frame, neither available on Windows x64 -Xint).
...
// calls the native function via a uniform C prototype. Sufficient for Windows x64 Phase-2
...
static bool InterpreterJniGeneric(Thread* self,
  ...
  // Handle float/double and common NativeConverter shorties (Windows x64 -Xint).
