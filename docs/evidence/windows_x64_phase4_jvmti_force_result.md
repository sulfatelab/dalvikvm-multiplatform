# Windows x64 JVMTI forced-interpreter result
...
The Windows x64 `openjdkjvmti.dll` plugin loads through ART's normal
...
## Windows x64 plugin boundary

`openjdkjvmti` remains a separate DLL, matching the Linux shared-library
topology. All 29 upstream translation units build for Windows x64. The PE-specific
...
Linux and Windows x64 do not have separate boot jars. The Wine probe and Linux L-005
gate consume the same Windows x64-staged shared multipath artifact:

```text
build/windows_x64_phase1/run/boot.jar
