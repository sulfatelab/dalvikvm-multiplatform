...
cp -f /tmp/windows_x64_jni_stubs/libcombined.dll ./libjavacore.dll
...
```bash
bash tools/windows_x64/jni_stubs/build_combined.sh
```
...
- MemMap mprotect/msync/madvise Windows x64
- LinearAlloc / arenas low 4GB on Windows x64
- Windows x64 `ArtMethod::Invoke` → interpreter (no SysV quick stubs)
...
- Historical Phase-2 VEH + skipped SignalCatcher. Stage D now requires the
  Windows x64 managed-fault chain for every started runtime; `-Xno-sig-chain`
...
- JIT / quick codegen Windows x64 ABI + GS TLS not required for A3
