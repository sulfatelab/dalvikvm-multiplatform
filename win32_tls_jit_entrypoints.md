...
**JIT memory / codegen:** see [win32_jit_memory.md](win32_jit_memory.md) §13 — managed JIT uses the corrected dual view; D-1 r15 TLS audit is complete; the compiled-JNI managed/native split plus XMM-to-XMM moves pass a 7/7 registered/unresolved normal/FastNative mixed/high-FP matrix across unregister/dlsym/re-register binding transitions without recompilation; and direct CriticalNative unified ordinals, shadow/stack layout, dlsym caller-PC preservation, and unresolved mixed-signature lookup through the ART-owned native-load bridge pass in both memory modes. Native methods remain gated pending W-024 instrumentation/deoptimization transitions, product/diagnostic cleanup, and real-Windows acceptance.
