...
Invoke-CheckedProcess 'mem_map_policy' 'windows_x64_w013_mem_map_probe.exe' '' @('W013_MEM_MAP_POLICY_PASS', 'boundary=tested', 'transitions=32', 'fragments=', 'exhaustion_reservations=', 'destruction_cycles=128')
Invoke-CheckedProcess 'dlmalloc_config' 'W013DlmallocConfigProbe.exe' '' @('W013_DLMALLOC_CONFIG_PASS')
Invoke-CheckedProcess 'mspace_owner' 'windows_x64_w013_mspace_owner_probe.exe' 'success' @('W013_MSPACE_OWNER_PASS')
...
Invoke-CheckedProcess 'jit_dual_compile' 'dalvikvm.exe' "$Common -Xms64m -Xmx512m -cp run\hello.jar Hello" @('JitCodeCache::Create OK', 'Windows x64 CompileMethod done success=1 method=java.lang.StringBuilder', 'Hello from dalvikvm!', 'main end exception=0')

$env:ART_WINDOWS_X64_JIT = '0'
Invoke-CheckedProcess 'jit_disabled' 'dalvikvm.exe' "$Common -Xms64m -Xmx512m -cp run\hello.jar Hello" @('Hello from dalvikvm!', 'main end exception=0') @('Windows x64 CompileMethod done success=1')
Remove-Item Env:ART_WINDOWS_X64_JIT -ErrorAction SilentlyContinue

Remove-Item Env:ART_WINDOWS_X64_JIT_LOG_COMPILES -ErrorAction SilentlyContinue
...
Invoke-CheckedProcess 'jit_filter' 'dalvikvm.exe' "$Common -Xms64m -Xmx512m -cp run\hello.jar Hello" @('Windows x64 CompileMethod done success=1 method=java.lang.StringBuilder', 'Hello from dalvikvm!', 'main end exception=0')
Remove-Item Env:ART_WINDOWS_X64_JIT_FILTER -ErrorAction SilentlyContinue

$env:ART_WINDOWS_X64_JIT_EXCLUDE = 'StringBuilder'
Invoke-CheckedProcess 'jit_exclude' 'dalvikvm.exe' "$Common -Xms64m -Xmx512m -cp run\hello.jar Hello" @('Hello from dalvikvm!', 'main end exception=0') @('Windows x64 CompileMethod done success=1 method=java.lang.StringBuilder')
Remove-Item Env:ART_WINDOWS_X64_JIT_EXCLUDE -ErrorAction SilentlyContinue

Remove-Item Env:ART_WINDOWS_X64_JIT_LOG_COMPILES -ErrorAction SilentlyContinue
Invoke-CheckedProcess 'jit_quiet' 'dalvikvm.exe' "$Common -Xms64m -Xmx512m -cp run\hello.jar Hello" @('Hello from dalvikvm!', 'main end exception=0') @('Windows x64 CompileMethod done success=1')

$env:ART_WINDOWS_X64_JIT_DUAL = '0'
$env:ART_WINDOWS_X64_JIT_LOG_COMPILES = '1'
Invoke-CheckedProcess 'jit_j1_compile' 'dalvikvm.exe' "$Common -Xms64m -Xmx512m -cp run\hello.jar Hello" @('JitCodeCache::Create OK', 'Windows x64 CompileMethod done success=1 method=java.lang.StringBuilder', 'Hello from dalvikvm!', 'main end exception=0')
