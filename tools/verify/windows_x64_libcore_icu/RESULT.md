...
cmake -S tools/verify/windows_x64_libcore_icu -B build/windows_x64_libcore_icu -G Ninja \
  -DCMAKE_TOOLCHAIN_FILE=$WINDOWS_X64_CMAKE_TOOLCHAIN -DCMAKE_BUILD_TYPE=RelWithDebInfo
cmake --build build/windows_x64_libcore_icu -j$(nproc)
```

## Staging into ART package

```bash
cp build/windows_x64_libcore_icu/{icuuc,icui18n,icu_jni}.dll build/windows_x64_phase1/
cp build/windows_x64_libcore_icu/icu_jni.dll build/windows_x64_phase1/libicu_jni.dll
...
- Wire install into `package_windows_x64_phase3.sh`
...
Artifact: `build/windows_x64_libcore_icu/javacore.dll` (~92K)
...
Artifact: `build/windows_x64_libcore_icu/openjdk.dll` (~113K), `openjdkjvm.dll` (~13K)
...
- **W-018 CLOSED:** `getsockoptLinger` / `setsockoptLinger` in `tools/windows_x64/jni_stubs/win_net_natives.c` + register table; wine NetProbe PASS.
- **ICU_DATA defaults:** phase3/phase4 runners already export `ICU_DATA=run/icu`; `package_windows_x64_phase3.sh` now **requires** `run/icu/icudt72l.dat` (copies from build or vendor stubdata path fallback) and stages `icuuc`/`icui18n`/`openjdkjvm` when present.
...
```bash
tools/windows_x64/stage_run_assets.sh <dest_root> [build_dir]
# -> <dest>/run/boot.jar
...
```bash
tools/windows_x64/stage_native_modules.sh <dest> [build/windows_x64_libcore_icu] [build/windows_x64_phase1]
```
...
`tools/windows_x64/jni_stubs/libcombined.dll` is **legacy / non-product**.
...
`tools/windows_x64/jni_stubs/native_converter.c` is obsolete and is **not** linked into product PE or the legacy combined stub build.
...
Configure flag: `-DMDVM_WINDOWS_X64_BUILD_CRYPTO=ON` (default ON in harness).
...
Configure: `-DMDVM_WINDOWS_X64_BUILD_CRYPTO=ON` + `-DMDVM_WINDOWS_X64_BUILD_SSL=ON` (defaults ON).

Windows x64 conscrypt header fixes (vendor nested):
...
Wine smoke (cwd `build/windows_x64_phase1`):
...
### Runtime PE support
- `System.mapLibraryName("javacrypto")` → `libjavacrypto.dll` via `tools/windows_x64/jni_stubs/libcore_hello3.c`
...
- **W-019:** `java.lang.Math` `@CriticalNative` (and FastNative double ABI) crashed on Windows x64 PE
...
Root causes closed:
1. Windows x64 -Xint FastNative/native bridge routing (interpreter)
2. `FileChannelImpl.map0` Windows x64 LLP64 pointer truncation (`unsigned long` cast)
...
- generator: `tools/windows_x64/generate_cacerts.sh`
- hermetic assets: `tools/windows_x64/assets/cacerts` (121 roots, subject_hash_old)
- stage: `tools/windows_x64/stage_run_assets.sh` → `run/etc/security/cacerts`
...
- Build: `tools/bootjar/build_okhttp_windows_x64.sh` → boot.jar includes `com.android.okhttp.*` + okio (241 classes; multi-dex)
- Wine HttpsProbe:
...
**Intentional residual:** AOSP `libcore_io_Linux.cpp` not compiled on Windows x64; Os map uses Win bridges (needed=0). Crypto under L-002.
...
The old Windows x64 `Runtime.nativeLoad` path directly called `LoadLibraryA` and
...
could not resolve exported `Java_*` methods for unresolved app JNI; Windows x64 soft
...
rerun `build_conscrypt_windows_x64.sh` for the TLS product jar. It is not a reason to
