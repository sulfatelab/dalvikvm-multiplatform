...
cmd=['timeout','45','wine64','./dalvikvm.exe','-Xbootclasspath:run/boot.jar','-Xbootclasspath-locations:run/boot.jar','-Ximage:/nonexistent-no-boot-image','-Xno-sig-chain','-XjdwpProvider:none','-Xint','-Xms64m','-Xmx512m','-cp','run/netprobe.jar','NetProbe']
...
checks=[
 ...
 ('match', 'match=true' in text),
 ('echoMatch', 'echoMatch=true' in text),
 ('done', 'NetProbe.done=ok' in text),
]
fail=0
print('=== ASSERT ===')
for n,ok in checks:
