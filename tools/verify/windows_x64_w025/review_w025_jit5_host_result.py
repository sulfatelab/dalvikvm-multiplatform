#!/usr/bin/env python3
"""Independently review a returned W-025 JIT-5 native result archive."""
...
def review_removal_contract(root: Path) -> None:
    report = read_text(root / "W025_JIT5_SOURCE_REPORT.txt")
    require(
        report,
        "JIT-5 source report",
    ...
    runner = read_text(root / "scripts/RUN_W025_JIT5_HOST.ps1")
    require(
        runner,
        "JIT-5 host runner",
        "smoke_retired_optout",
...
def main() -> int:
    ...
    with tempfile.TemporaryDirectory(prefix="w025-jit5-review-") as directory:
        extracted = Path(directory)
        safe_extract(archive, extracted)
        returned = payload_root(extracted)
        compare_identity(returned, issued)
        review_removal_contract(returned)
        pass_count, fail_count = review(returned)
    print(
        "W-025 JIT-5 native host result review: PASS "
...
if __name__ == "__main__":
    try:
        ...
        print(f"W-025 JIT-5 native host result review: FAIL: {error}", file=sys.stderr)
        sys.exit(1)
