...
function Test-Jit5RemovalContract {
    $reportPath = Join-Path $Root 'W025_JIT5_SOURCE_REPORT.txt'
    ...
    @(
        ...
        'W025_JIT5_SOURCE_CHECK_PASS'
    ) | ForEach-Object {
        if (-not $report.Contains($_)) { throw "JIT-5 source report lacks: $_" }
    }

    $artBytes = [System.IO.File]::ReadAllBytes((Join-Path $Root 'art.dll'))
    $artAscii = [System.Text.Encoding]::ASCII.GetString($artBytes)
    $artUtf16 = [System.Text.Encoding]::Unicode.GetString($artBytes)
    foreach ($retired in @('ART_WINDOWS_X64_JIT_DUAL', 'falling back to single-view (J-1)')) {
        if ($artAscii.Contains($retired) -or $artUtf16.Contains($retired)) {
            throw "art.dll contains retired Windows JIT marker: $retired"
        }
    }
}
...
Add-Result "W-025 JIT-5 final native regression $(Get-Date -Format 'yyyy-MM-dd HH:mm:ss')"
try {
...
try {
    ...
    Copy-Item -LiteralPath (Join-Path $Root 'W025_JIT5_SOURCE_REPORT.txt') `
        -Destination (Join-Path $Logs 'W025_JIT5_SOURCE_REPORT.txt')
...
try {
    ...
    Add-Result 'PASS jit5_removal_contract source_absent=true binary_absent=true failure_policy=fail-closed'
} catch {
    $_.Exception.ToString() | Set-Content (Join-Path $Logs 'JIT5_REMOVAL_CONTRACT_ERROR.txt')
    Add-Result 'FAIL jit5_removal_contract'
    $script:Failed = $true
}
...
# The retired environment key must be inert: setting it to zero still takes J-2.
Clear-ArtEnvironment
$env:ART_WINDOWS_X64_JIT_DUAL = '0'
$env:ART_WINDOWS_X64_JIT_LOG_COMPILES = '1'
Invoke-CheckedProcess 'smoke_retired_optout' "$script:Common -cp run\hello.jar Hello" @(
    $HelloMarkers
    'JitCodeCache::Create OK'
    'Windows x64 JIT dual-view (J-2) created'
    'Windows x64 CompileMethod done success=1 method=java.lang.StringBuilder'
...
$resultPath = Join-Path $Logs 'RESULT_W025_JIT5.txt'
