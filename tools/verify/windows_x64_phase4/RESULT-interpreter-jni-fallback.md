...
The early Windows x64 port expanded `InterpreterJni` with many PE-specific shorties
...
This audit asks whether any current runtime-started Windows x64 path still reaches
...
Linux and Windows x64 consume the same dex and annotation bytes:

```text
path   build/windows_x64_phase1/run/boot.jar
...
2. `ArtInterpreterToInterpreterBridge`, where the early Windows x64 port allowed a
...
`MDVM_WINDOWS_X64_INTERPRETER_JNI_TRIPWIRE=ON`. The definition was source-scoped to
...
The Windows x64 `art` and `dalvikvm` targets built successfully. With both calls
...
| Math CriticalNative, Windows x64 `-Xint` | 1/1 |
...
No tripwire fired. In particular, Windows x64 `-Xint` and real JVMTI single-step
...
and Windows x64 `art.dll`, `dalvikvm.exe`, and `openjdkjvmti.dll` were rebuilt
successfully. Final product-mode Math controls passed in dual-view, J-1,
Windows x64 `-Xint`, Linux `-Xint`, and Linux threshold-zero JIT modes.
...
The same commit removed the native-method JIT exclusion so Windows x64 follows the
common ART native compilation policy by default.

The final Windows x64 build passed default native ABI 7/7, CriticalNative,
