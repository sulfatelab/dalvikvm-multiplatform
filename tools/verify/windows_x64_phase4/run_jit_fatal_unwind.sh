...
PY
  ...
  if [[ $rc -eq 0 || $rc -eq 124 ]] ||
     ...
     ! grep -qE 'Windows x64 CompileMethod done success=1 method=.*CrashNativeProbe\.jitCrashCaller\(' "$log" ||
     ! grep -qE 'Windows x64 CompileMethod done success=1 method=.*CrashNativeProbe\.nativeSegfault\(' "$log" ||
    ...
    printf 'Windows x64 threshold-zero JIT fatal unwind %s FAIL exit=%s log=%s\n' \
      "$mode" "$rc" "$log" >&2
    tail -200 "$log" >&2
    return 1
  fi
...
PY
  ...
  if [[ $valid_dump -ne 1 ]]; then
    printf 'Windows x64 threshold-zero JIT fatal unwind %s created no valid new MDMP dump\n' \
      "$mode" >&2
    tail -200 "$log" >&2
    return 1
  fi
