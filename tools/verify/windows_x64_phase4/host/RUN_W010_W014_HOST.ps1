...
function Test-StructuralReport {
    ...
    $required = @(
        ...
        '^explicit_stack_checks=Windows x64 explicit stack-check contract: PASS \(Windows x64 object, Linux object\)$'
...
$HandledForbidden = @('ART Win32 VEH', 'ART Win32 UEF', 'minidump written', 'unexpected_continue')
...
$env:ART_WINDOWS_X64_JIT_LOG_COMPILES = '1'
Invoke-CheckedProcess -Name 'jit_npe' -Executable 'dalvikvm.exe' -Arguments "$Common -verbose:jit -Xjitwarmupthreshold:0 -Xjitthreshold:0 -cp run\w010managedfaultprobe.jar W010ManagedFaultProbe npe" -Markers @('W010ManagedFaultProbe NPE OK read=64 write=64 recovery=128 gc=16', 'Windows x64 CompileMethod done success=1 method=void W010ManagedFaultProbe.runNullChecks()', 'W010ManagedFaultProbe OK mode=npe', 'main end exception=0') -ForbiddenMarkers $HandledForbidden
Invoke-CheckedProcess -Name 'jit_so' -Executable 'dalvikvm.exe' -Arguments "$Common -verbose:jit -Xjitwarmupthreshold:0 -Xjitthreshold:0 -cp run\w010managedfaultprobe.jar W010ManagedFaultProbe so" -Markers @('W010ManagedFaultProbe SO OK main=2 child=2 recovery=4 gc=4', 'Windows x64 CompileMethod done success=1 method=int W010ManagedFaultProbe.recurse(int)', 'Windows x64 CompileMethod done success=1 method=int W010ManagedFaultProbe.runStackOverflowRounds()', 'W010ManagedFaultProbe OK mode=so', 'main end exception=0') -ForbiddenMarkers $HandledForbidden
Clear-ArtEnvironment
...
foreach ($fatalMode in @('j2', 'j1')) {
    ...
    $env:ART_WINDOWS_X64_JIT_DUAL = if ($fatalMode -eq 'j2') { '1' } else { '0' }
