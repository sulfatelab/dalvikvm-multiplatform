...
$env:ART_WINDOWS_X64_FATAL_UNWIND_TRACE = '1'
foreach ($mode in $lateModes) {
    ...
    $artVeh = Has-Marker $late 'ART Win32 VEH: exception 0xc0000005'
    $artUef = Has-Marker $late 'ART Win32 UEF: exception 0xc0000005'
    ...
    $unwindBegin = Has-Marker $late 'ART_WINDOWS_X64_UNWIND_TRACE begin'
    $unwindEnd = Has-Marker $late 'ART_WINDOWS_X64_UNWIND_TRACE end'
    $unwindFrames = ([regex]::Matches($late.Text, 'ART_WINDOWS_X64_UNWIND_TRACE frame=')).Count
    $newDumps = @(
...
Remove-Item -Path Env:ART_WINDOWS_X64_FATAL_UNWIND_TRACE -ErrorAction SilentlyContinue
