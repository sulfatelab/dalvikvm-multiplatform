# Windows x64 JIT smoke test — default dual-view and native compilation

Date: 2026-07-24. VM: agent01, Wine. Build: windows_x64_phase1 RelWithDebInfo.
...
| T6 | `ART_WINDOWS_X64_JIT=0` disables all compile and completes Hello cleanly | **PASS** (0 compiles) |
| T7 | `-Xusejit:false` completes Hello cleanly | **PASS** |
| T8a | `ART_WINDOWS_X64_JIT_FILTER=StringBuilder` completes Hello cleanly | **PASS** (5 compiles) |
| T8b | `ART_WINDOWS_X64_JIT_EXCLUDE=StringBuilder` completes Hello cleanly | **PASS** |
...
- Per-method `Windows x64 CompileMethod done` records are now opt-in through
  `ART_WINDOWS_X64_JIT_LOG_COMPILES=1`. Acceptance harnesses that count compilation
...
- `vendor/art/runtime/jit/jit.cc` — `CompileMethodInternal`: common native
  compilation policy plus Windows x64 opt-in compile diagnostics
