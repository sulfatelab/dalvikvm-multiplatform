...
#   T6 – ART_WINDOWS_X64_JIT=0 disables all compile
...
run_dalvik() {
  ...
  ART_WINDOWS_X64_JIT_LOG_COMPILES="${ART_WINDOWS_X64_JIT_LOG_COMPILES:-}" \
...
count_compiles() {
  local cnt
  cnt=$(grep -c "Windows x64 CompileMethod done success=1" <<< "$1" 2>/dev/null) || cnt=0
  echo "$cnt"
}
...
has_clean_native_hello() {
  local output="$1"
  grep -q "Windows x64 CompileMethod done success=1 method=.*StringFactory.newStringFromBytes" \
    <<< "$output" &&
...
OUT1=$(ART_WINDOWS_X64_JIT_LOG_COMPILES=1 run_dalvik "$RUN/hello.jar" "Hello")
...
assert "At least one managed method JIT-compiled" \
  grep -q "Windows x64 CompileMethod done success=1 method=.*StringBuilder" <<< "$OUT1"
...
NATIVE_COMPILE=$(grep "Windows x64 CompileMethod done success=1" <<< "$OUT1" 2>/dev/null | grep -i "StringFactory" || true)
...
# --------------------------------------------------------------------
cyan "=== T6: ART_WINDOWS_X64_JIT=0 disables all compile ==="

OUT6=$(ART_WINDOWS_X64_JIT=0 ART_WINDOWS_X64_JIT_LOG_COMPILES=1 \
  run_dalvik "$RUN/hello.jar" "Hello")
NCOMP6=$(count_compiles "$OUT6")
echo "  JIT_OFF mode ncomp: $NCOMP6"
assert "ART_WINDOWS_X64_JIT=0 completes Hello cleanly" has_clean_hello "$OUT6"
assert "ART_WINDOWS_X64_JIT=0 produces zero JIT compiles" [ "$NCOMP6" -eq 0 ]
...
# Note: -Xusejit:false may still create the JIT cache on Windows x64 b/c
...
OUT8_FILT=$(ART_WINDOWS_X64_JIT_FILTER=StringBuilder ART_WINDOWS_X64_JIT_LOG_COMPILES=1 \
  run_dalvik "$RUN/hello.jar" "Hello")
FCOMP=$(count_compiles "$OUT8_FILT")
echo "  Filter mode ncomp: $FCOMP"
assert "ART_WINDOWS_X64_JIT_FILTER completes Hello cleanly" has_clean_hello "$OUT8_FILT"

OUT8_EXCL=$(ART_WINDOWS_X64_JIT_EXCLUDE=StringBuilder ART_WINDOWS_X64_JIT_LOG_COMPILES=1 \
  run_dalvik "$RUN/hello.jar" "Hello")
assert "ART_WINDOWS_X64_JIT_EXCLUDE completes Hello cleanly" has_clean_hello "$OUT8_EXCL"
