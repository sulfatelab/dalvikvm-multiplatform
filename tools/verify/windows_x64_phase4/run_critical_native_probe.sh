...
# Verify the Windows x64 optimizing-compiler direct @CriticalNative ABI, including
# threshold-zero ()J and unresolved exported mixed-signature dlsym paths.

REPO="$(cd "$(dirname "$0")/../../.." && pwd)"
BUILD="${BUILD:-$REPO/build/windows_x64_phase1}"
RUN="$BUILD/run"
NATIVE_BUILD="${NATIVE_BUILD:-$REPO/build/windows_x64_critical_native_probe}"
...
if [[ ! -x "$BUILD/dalvikvm.exe" || ! -f "$RUN/FloatProbe.jar" ]]; then
  echo "missing Windows x64 dalvikvm or FloatProbe.jar under $BUILD" >&2
  exit 1
fi

cmake -S "$REPO/tools/verify/windows_x64_phase4/critical_native" \
  -B "$NATIVE_BUILD" \
  -G Ninja \
...
run_vm() {
  ...
  if (
    ...
    timeout "$TIMEOUT" "$WINE" ./dalvikvm.exe \
      ...
      -Dcritical.instrumentation="$instrumentation" \
...
CRITICAL_TRACING_VALUES="CriticalNativeProbe tracing values longs=190 doubles=91.0 mixed=159.5 mixed32=87 floatReturn=15.25 calls=63 branchSeen=true"
CRITICAL_POST_TRACING_VALUES="CriticalNativeProbe postTracing values longs=190 doubles=91.0 mixed=159.5 mixed32=87 floatReturn=15.25 calls=63 branchSeen=true"
