...
r15 has two related but distinct roles. Windows x64 optimizing code reserves it as
...
The current matched Windows x64 and Linux quick-entrypoint objects both contain:
...
is `sysv_abi` on Windows x64. The existing shared frames are aligned for those calls
...
The Windows x64 quick-entrypoint object contains only `.text`, `.data`, and `.bss`.
...
- clean Windows x64 and Linux builds with `-j32`;
- the structural gate;
...
- `SAVE_WINDOWS_X64_NATIVE_XMMS` and `RESTORE_WINDOWS_X64_NATIVE_XMMS` reserve 96 bytes
...
- quick OSR saves before entering compiled OSR code and restores after its
  return;
- adding 96 bytes keeps stack alignment unchanged and updates only the Windows x64
...
Verification after rebuilding Windows x64 and Linux with `-j32`:
...
[RESULT-w003-frame-probe.md](RESULT-w003-frame-probe.md).

The Stage D package is generated by
`tools/windows_x64/host_package/package_windows_x64_w003.sh`. Before issuing the ZIP, the
pipeline reruns the structural gate and both focused Wine matrices, validates
the staged PE exports and hashes, smokes all seven modes from the package,
restores product `art.dll`, removes runtime output, and validates the final
...
instructions are in [W003_HOST_CHECKLIST.md](W003_HOST_CHECKLIST.md).
...
See [evidence/w003_host/ACCEPTANCE.md](evidence/w003_host/ACCEPTANCE.md).
