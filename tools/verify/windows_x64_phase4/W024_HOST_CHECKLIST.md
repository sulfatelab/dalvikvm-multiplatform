...
The values must match `BUILD_INFO.txt`. The expected shared Linux/Windows x64
...
| `math_xint` | Math CriticalNative with Windows x64 `-Xint` |
...
```bat
findstr /S /C:"Windows x64 InterpreterJni tripwire" logs\*.log && echo FAIL || echo PASS
```
...
Post-change verification passed the Windows x64 build, JIT smoke 12/12, JIT matrix
