#!/usr/bin/env python3
"""Review historical native-Windows W-010/W-014 Stage E evidence."""
...
def review(returned: Path, issued: Path) -> None:
    ...
    for name, markers in managed_cases.items():
        ...
        if name in {"switch_so", "nterp_npe", "nterp_so"}:
            forbidden = (*forbidden, "Windows x64 CompileMethod done success=1 method=")
    ...
    for mode in ("j2", "j1"):
        ...
        if mode == "j2":
            jit_markers = (*jit_markers, "Windows x64 JIT dual-view (J-2) created")
        else:
            jit_forbidden = (*jit_forbidden, "Windows x64 JIT dual-view (J-2) created")
        ...
        if mode == "j2":
            osr_markers = (*osr_markers, "Windows x64 JIT dual-view (J-2) created")
        else:
            osr_forbidden = (*osr_forbidden, "Windows x64 JIT dual-view (J-2) created")
        text = require_markers(
