#!/usr/bin/env bash
set -euo pipefail
# Build a Phase-4 probe jar into build/windows_x64_phase1/run using the same pipeline as phase3.
CLS="${1:?class name}"
...
repo=r'''$REPO'''
cls=r'''$CLS'''
dex=os.path.join(repo,f'tools/verify/windows_x64_phase4/bin/{cls}_dex/classes.dex')
out=os.path.join(repo,f'build/windows_x64_phase1/run/{cls.lower()}.jar')
