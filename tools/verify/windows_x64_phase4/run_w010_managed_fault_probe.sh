...
run_one() {
  ...
  if (
    ...
    export ART_WINDOWS_X64_JIT_FILTER="W010ManagedFaultProbe"
  ...
  if [[ $rc -ne 0 ]] ||
     ...
     grep -qE 'ART Win32 (VEH|UEF)|minidump written' "$log"; then
  ...
  if [[ "$execution_mode" == "jit" ]]; then
    ...
    for required_method in "${required_methods[@]}"; do
      if ! grep -qE "Windows x64 CompileMethod done success=1 method=.*W010ManagedFaultProbe\\.${required_method}\\(" \
          "$log"; then
  ...
  elif grep -qF "Windows x64 CompileMethod done success=1 method=" "$log"; then
