...
# Verify default mixed/high-FP Windows x64 compiled-JNI normal/FastNative
# conventions, rebinding, and tracing.

REPO="$(cd "$(dirname "$0")/../../.." && pwd)"
BUILD="${BUILD:-$REPO/build/windows_x64_phase1}"
RUN="$BUILD/run"
NATIVE_BUILD="${NATIVE_BUILD:-$REPO/build/windows_x64_native_abi_probe}"
...
TIMEOUT="${TIMEOUT:-60}"
DEFAULT_LOG="${TMPDIR:-/tmp}/windows_x64-fastnative-default.log"
TRACE_LOG="${TMPDIR:-/tmp}/windows_x64-fastnative-instrumentation.log"

cmake -S "$REPO/tools/verify/windows_x64_phase4/native_abi" \
  -B "$NATIVE_BUILD" \
  -G Ninja \
...
JAVA_TMP="$(mktemp -d "${TMPDIR:-/tmp}/windows_x64-native-abi-java.XXXXXX")"
...
"$JAVAC" -d "$JAVA_TMP/classes" \
  ...
  "$REPO/tools/verify/windows_x64_phase4/src/FastNativeAbiProbe.java"
