...
cmake --build "$BUILD" --target windows_x64_pthread_once_probe -j"$(nproc)"

for iteration in $(seq 1 "$REPEATS"); do
  log="${TMPDIR:-/tmp}/windows_x64-pthread-once-${iteration}.log"
  if (
    cd "$BUILD"
    WINEDEBUG="${WINEDEBUG:--all}" timeout "$TIMEOUT" \
      "$WINE" ./windows_x64_pthread_once_probe.exe
  ) >"$log" 2>&1 &&
