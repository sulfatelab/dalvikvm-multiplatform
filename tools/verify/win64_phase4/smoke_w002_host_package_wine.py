...
def main() -> int:
    ...
    for mode, dual in (("dual", "1"), ("j1", "0")):
        for interpreter in ("default", "switch"):
            base_env = {"ART_WIN64_JIT_DUAL": dual}
            if interpreter == "switch":
                base_env["ART_WIN64_NTERP"] = "0"

            osr_env = {
                **base_env,
                "ART_WIN64_JIT_FILTER": "W002OsrProbe.osrLoop",
            ...
            attach_env = {
                **base_env,
                "ART_WIN64_JIT_FILTER": "W002AttachProbe.attachedCallback",
                "ART_WIN64_JIT_LOG_COMPILES": "1",
            }
            run_case(
                root,
