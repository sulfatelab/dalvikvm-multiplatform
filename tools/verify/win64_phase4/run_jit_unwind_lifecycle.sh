...
"$JAVAC" -d "$temp_dir/classes" \
  "$REPO/tools/verify/win64_phase4/src/JitUnwindLifecycleProbe.java"
...
run_one() {
  ...
  if (
    ...
    export ART_WIN64_JIT_FILTER=JitUnwindLifecycleProbe
  ...
  if [[ $rc -ne 0 ]] ||
     ...
     grep -qE 'ART Win64 (VEH|UEF)|minidump written|Check failed|Fatal signal' "$log"; then
    printf 'Win64 JIT unwind lifecycle %s FAIL exit=%s collections=%s log=%s\n' \
  ...
  if [[ "$dual" == 1 ]]; then
    ...
    printf 'Win64 JIT unwind lifecycle %s unexpectedly used J-2\n' "$mode" >&2
    return 1
  fi

  printf 'Win64 JIT unwind lifecycle %s PASS collections=%s\n' \
    "$mode" "$collection_count"
}

run_one j2 1
run_one j1 0
printf 'Win64 JIT unwind lifecycle acceptance: J-2 and J-1 PASS\n'
