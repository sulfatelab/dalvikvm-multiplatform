...
python3 "$REPO/tools/verify/win64_phase1/check_win32_boundary_unwind.py" \
...
PY
  ...
  if (
    cd "$BUILD"
    unset ART_WIN64_CRASH_NATIVE_WARMUP
    export ART_WIN64_NTERP=0
    export ART_WIN64_JIT_DUAL="$dual"
    export ART_WIN64_JIT_FILTER=CrashNativeProbe.osrCrashLoop
...
PY
  ...
  if [[ $rc -eq 0 || $rc -eq 124 ]] ||
     ...
     ! grep -qE 'Win64 CompileMethod done success=1 method=long CrashNativeProbe\.osrCrashLoop\(int\)' "$log" ||
    ...
    printf 'Win64 OSR-origin fatal unwind %s FAIL exit=%s log=%s\n' \
      "$mode" "$rc" "$log" >&2
    tail -200 "$log" >&2
    return 1
  fi
  ...
  if [[ "$dual" == 1 ]]; then
    ...
    printf 'Win64 OSR-origin fatal unwind %s unexpectedly used J-2\n' "$mode" >&2
    return 1
  fi
...
PY
  ...
  if [[ $valid_dump -ne 1 ]]; then
    printf 'Win64 OSR-origin fatal unwind %s created no valid new MDMP dump\n' \
      "$mode" >&2
    tail -200 "$log" >&2
    return 1
  fi

  printf 'Win64 OSR-origin fatal unwind %s PASS\n' "$mode"
)

run_one j2 1
run_one j1 0
printf 'Win64 OSR-origin fatal unwind acceptance: J-2 and J-1 PASS\n'
