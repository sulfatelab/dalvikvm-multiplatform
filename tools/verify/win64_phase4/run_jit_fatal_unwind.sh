...
if [[ ! -f "$BUILD/art.dll" || ! -f "$RUN/art.dll" ]] ||
  ...
  printf 'built and staged art.dll must exist and match before JIT fatal-unwind testing\n' >&2
  exit 1
fi
...
PY
  ...
  if (
    cd "$BUILD"
    export ART_WIN64_CRASH_NATIVE_WARMUP=20000
    export ART_WIN64_JIT_DUAL="$dual"
    export ART_WIN64_JIT_FILTER=CrashNativeProbe
    ...
    timeout "$TIMEOUT" "$WINE" ./dalvikvm.exe \
      ...
      -Xms64m -Xmx512m \
      -cp "$RUN/crashnativeprobe.jar" CrashNativeProbe jit
  ) >"$log" 2>&1; then
    rc=0
  else
    rc=$?
...
PY
  ...
  if [[ $rc -eq 0 || $rc -eq 124 ]] ||
     ! grep -qF 'CrashNativeProbe.jit_ready calls=20000' "$log" ||
     ! grep -qE 'Win64 CompileMethod done success=1 method=.*CrashNativeProbe\.jitCrashCaller\(' "$log" ||
     ! grep -qE 'Win64 CompileMethod done success=1 method=.*CrashNativeProbe\.nativeSegfault\(' "$log" ||
     ! grep -qF 'ART Win64 VEH: exception 0xc0000005' "$log" ||
     ! grep -qF 'ART Win64 UEF: exception 0xc0000005' "$log" ||
     ! grep -qF 'ART Win64 crash: minidump written to ' "$log" ||
     grep -qF 'CrashNativeProbe.unexpected_continue' "$log"; then
    printf 'Win64 threshold-zero JIT fatal unwind %s FAIL exit=%s log=%s\n' \
      "$mode" "$rc" "$log" >&2
    tail -200 "$log" >&2
    return 1
  fi
  ...
  if [[ "$dual" == 1 ]]; then
    grep -qF 'Win64 JIT dual-view (J-2) created' "$log"
  elif grep -qF 'Win64 JIT dual-view (J-2) created' "$log"; then
    printf 'Win64 threshold-zero JIT fatal unwind %s unexpectedly used J-2\n' "$mode" >&2
    return 1
  fi
...
PY
  ...
  if [[ $valid_dump -ne 1 ]]; then
    printf 'Win64 threshold-zero JIT fatal unwind %s created no valid new MDMP dump\n' \
      "$mode" >&2
    tail -200 "$log" >&2
    return 1
  fi

  printf 'Win64 threshold-zero JIT fatal unwind %s PASS\n' "$mode"
)

run_one j2 1
run_one j1 0
printf 'Win64 threshold-zero JIT fatal unwind acceptance: J-2 and J-1 PASS\n'
