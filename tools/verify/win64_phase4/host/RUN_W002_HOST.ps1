...
$AttachMarkers = @(
    ...
    'Win64 CompileMethod done success=1 method=long W002AttachProbe.attachedCallback(boolean, int)'
    'main end exception=0'
)
...
foreach ($mode in @('dual', 'j1')) {
    foreach ($interpreter in @('default', 'switch')) {
        foreach ($repeat in 1..2) {
            ...
            $env:ART_WIN64_JIT_FILTER = 'W002OsrProbe.osrLoop'
            ...
            $env:ART_WIN64_JIT_FILTER = 'W002AttachProbe.attachedCallback'
            $env:ART_WIN64_JIT_LOG_COMPILES = '1'
