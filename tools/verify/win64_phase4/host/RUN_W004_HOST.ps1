...
Invoke-CheckedProcess 'jit_dual' 'dalvikvm.exe' "$Common -cp run\hello.jar Hello" @(
    ...
    'Win64 CompileMethod done success=1 method=java.lang.StringBuilder'
    'Hello from dalvikvm!'
    'main end exception=0'
)
...
foreach ($mode in @('dual', 'j1')) {
    Clear-JitEnvironment
    if ($mode -eq 'dual') { $env:ART_WIN64_JIT_DUAL = '1' } else { $env:ART_WIN64_JIT_DUAL = '0' }
...
foreach ($mode in @('dual', 'j1')) {
    Clear-JitEnvironment
    if ($mode -eq 'dual') { $env:ART_WIN64_JIT_DUAL = '1' } else { $env:ART_WIN64_JIT_DUAL = '0' }
    $env:ART_WIN64_JIT_FILTER = 'FastNativeAbiProbe'
    $env:ART_WIN64_JIT_LOG_COMPILES = '1'
...
foreach ($mode in @('dual', 'j1')) {
    Clear-JitEnvironment
    if ($mode -eq 'dual') { $env:ART_WIN64_JIT_DUAL = '1' } else { $env:ART_WIN64_JIT_DUAL = '0' }
    $env:ART_WIN64_JIT_FILTER = 'JvmtiForceProbe'
    $env:ART_WIN64_JIT_LOG_COMPILES = '1'
