...
RUN="$BUILD/run"
NATIVE_BUILD="${NATIVE_BUILD:-$REPO/build/win64_w002_attach_probe}"
WIN64_TOOLCHAIN="${WIN64_TOOLCHAIN:-/home/agent/Projects/win64-dev-env/cmake/Win64LLVM.cmake}"
WINE="${WINE:-wine64}"
REPEATS="${REPEATS:-2}"
TIMEOUT="${TIMEOUT:-120}"

bash "$REPO/tools/verify/win64_phase4/build_one.sh" W002AttachProbe
cmake -S "$REPO/tools/verify/win64_phase4/w002_attach" \
...
run_one() {
  ...
  if (
    ...
    export ART_WIN64_JIT_FILTER="W002AttachProbe.attachedCallback"
  ...
  if [[ $rc -ne 0 ]] ||
     ...
     ! grep -qF "Win64 CompileMethod done success=1 method=long W002AttachProbe.attachedCallback(boolean, int)" "$log" ||
