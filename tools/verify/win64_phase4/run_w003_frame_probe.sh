...
BUILD="${BUILD:-$REPO/build/win64_w003_frames}"
NATIVE_BUILD="${NATIVE_BUILD:-$REPO/build/win64_w003_frame_probe}"
...
cmake -S "$REPO/tools/verify/win64_phase1" \
  -B "$BUILD" \
  -G Ninja \
...
cmake -S "$REPO/tools/verify/win64_phase4/w003_frame_probe" \
  -B "$NATIVE_BUILD" \
  -G Ninja \
  -DCMAKE_TOOLCHAIN_FILE="$WIN64_TOOLCHAIN" \
...
find "$PRODUCT_BUILD" -maxdepth 1 -type f -name '*.dll' ! -name 'art.dll' \
  -exec cp -f {} "$BUILD/" \;
bash "$REPO/tools/win64/stage_run_assets.sh" "$BUILD" "$PRODUCT_BUILD"
bash "$REPO/tools/win64/stage_native_modules.sh" \
  "$BUILD" "$REPO/build/win64_libcore_icu" "$PRODUCT_BUILD"
...
"$JAVAC" -d "$JAVA_TMP/classes" \
  "$REPO/tools/verify/win64_phase4/src/W003FrameProbe.java"
...
run_one() {
  ...
  case "$mode" in
    ...
    jit)
      ...
      mode_env+=(ART_WIN64_JIT_FILTER=W003FrameProbe)
