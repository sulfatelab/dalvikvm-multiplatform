#!/usr/bin/env python3
"""Verify the narrow Win64 explicit-stack-check source contract."""
...
def check_win64_object(build: Path, objdump: str) -> None:
    ...
    if not defines_path.is_file():
        raise SystemExit(f"FAIL Win64 asm defines: missing {defines_path}")
    ...
    if len(compare_indices) != 7:
        raise SystemExit(
            "FAIL Win64 nterp object: expected seven explicit stack compares "
    ...
    for index in compare_indices:
        ...
        if len(sequence) != 3 or not sequence[1].startswith("jae\t") or sequence[2] != tail_jump:
            raise SystemExit(
                "FAIL Win64 nterp object: malformed compare/branch/tail-jump sequence: "
                + " | ".join(sequence)
            )
    reject(disassembly, "-0x2000(%rsp)", "Win64 nterp implicit probe")
...
def main() -> int:
    ...
    parser.add_argument(
        ...
        type=Path,
        default=None,
        help="configured Win64 build directory; enables object verification",
    )
    parser.add_argument(
        "--linux-build",
        type=Path,
