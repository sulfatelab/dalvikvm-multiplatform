#!/usr/bin/env bash
# Build shared multipath boot.jar for Win32 ART (and Linux).
#
...
mkdir -p "$REPO/build/windows_x64_phase1/run"
cp -f /tmp/bootbuild/boot.jar "$REPO/build/windows_x64_phase1/run/boot.jar"
# Also stage a Linux-shared copy for L-005 if desired
mkdir -p /tmp/vm/run
cp -f /tmp/bootbuild/boot.jar /tmp/vm/run/boot.jar
echo "staged build/windows_x64_phase1/run/boot.jar ($(stat -c%s "$REPO/build/windows_x64_phase1/run/boot.jar") bytes)"
echo "staged /tmp/vm/run/boot.jar (shared multipath)"
python3 - <<PY
import zipfile
d=zipfile.ZipFile("$REPO/build/windows_x64_phase1/run/boot.jar").read('classes.dex')
