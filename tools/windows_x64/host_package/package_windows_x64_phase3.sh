#!/usr/bin/env bash
# Stage a portable Win32 ART Phase-3 smoke tree for a real Windows host.
set -euo pipefail
REPO="$(cd "$(dirname "$0")/../../.." && pwd)"
BUILD="${BUILD:-$REPO/build/windows_x64_phase1}"
OUT="${1:-$REPO/dist/windows_x64_phase3_host}"
rm -rf "$OUT"
...
# Never ship tools/windows_x64/jni_stubs/libcombined.dll multi-name aliases (W-005).
bash "$REPO/tools/windows_x64/stage_native_modules.sh" "$OUT" \
  "${MDVM_HYBRID_BUILD:-$REPO/build/windows_x64_libcore_icu}" "$BUILD"
# Required product assets: boot.jar + run/icu/icudt72l.dat + run/etc/security/cacerts
bash "$REPO/tools/windows_x64/stage_run_assets.sh" "$OUT" "$BUILD"
...
cd /d %~dp0\..
if not exist logs mkdir logs
echo Windows x64 Phase3 host goldens %DATE% %TIME%> logs\RESULT_HOST.txt
set FAIL=0
for %%S in (
...
# Windows x64 Phase 3 host smoke package
...
```bash
bash tools/windows_x64/host_package/smoke_package_wine64.sh
```
MD
