...
OUT="${1:-$REPO/dist/windows_x64_w013_host}"
JOBS="${JOBS:-16}"

"$REPO/tools/verify/windows_x64_w013/run_dlmalloc_config_probe.sh"
cmake --build "$BUILD" --target windows_x64_w013_mem_map_probe windows_x64_w013_mspace_owner_probe -j"$JOBS"
"$REPO/tools/verify/windows_x64_w013/run_non_moving_stress.sh"

"$REPO/tools/windows_x64/host_package/package_windows_x64_phase3.sh" "$OUT"

cp -a "$BUILD/windows_x64_w013_mem_map_probe.exe" "$OUT/"
cp -a "$BUILD/windows_x64_w013_mspace_owner_probe.exe" "$OUT/"
...
cp -a "$REPO/tools/verify/windows_x64_w013/host/RUN_W013_HOST.ps1" "$OUT/scripts/"
cp -a "$REPO/tools/verify/windows_x64_w013/W013_HOST_CHECKLIST.md" "$OUT/"
rm -rf "$OUT/logs"
mkdir -p "$OUT/logs"
