#!/usr/bin/env bash
# Build, preflight, and package W-025 JIT-2 native Windows acceptance.
set -euo pipefail

REPO="$(cd "$(dirname "$0")/../../.." && pwd)"
BUILD="${BUILD:-$REPO/build/windows_x64_phase1}"
OUT="${1:-$REPO/dist/windows_x64_w025_jit2_host}"

"$REPO/tools/verify/windows_x64_w025/run_w025_jit2_preflight.sh"
python3 "$REPO/tools/verify/windows_x64_w025/check_w025_jit2_source.py" \
  --repo "$REPO" --build "$BUILD" >"$BUILD/W025_STRUCTURAL_REPORT.txt"

"$REPO/tools/windows_x64/host_package/package_windows_x64_phase3.sh" "$OUT"

cp -a "$BUILD/libw025jitmappingprobe.dll" "$OUT/"
cp -a "$BUILD/W025SectionPolicyProbe.exe" "$OUT/"
cp -a "$BUILD/W025PolicyLauncher.exe" "$OUT/"
cp -a "$BUILD/run/w025jitmappingprobe.jar" "$OUT/run/"
cp -a "$BUILD/W025_STRUCTURAL_REPORT.txt" "$OUT/"
cp -a "$REPO/tools/verify/windows_x64_w025/host/RUN_W025_JIT2_HOST.ps1" "$OUT/scripts/"
cp -a "$REPO/tools/verify/windows_x64_w025/W025_JIT2_HOST_CHECKLIST.md" "$OUT/"
...
# W-025 JIT-2 native Windows gate
...
`W025_STRUCTURAL_REPORT.txt`, `MANIFEST.json`, and `SHA256SUMS.txt`.
EOF
...
write_manifests
python3 "$REPO/tools/verify/windows_x64_w025/check_w025_jit2_host_package.py" "$OUT"
python3 "$REPO/tools/verify/windows_x64_w025/smoke_w025_jit2_host_package_wine.py" "$OUT"
find "$OUT/logs" -mindepth 1 -type f -delete
find "$OUT/jit-temp" -mindepth 1 -delete
write_manifests
python3 "$REPO/tools/verify/windows_x64_w025/check_w025_jit2_host_package.py" "$OUT"
...
printf 'W025_JIT2_HOST_PACKAGE_PASS path=%s bytes=%s\n' "$OUT.zip" "$(stat -c %s "$OUT.zip")"
