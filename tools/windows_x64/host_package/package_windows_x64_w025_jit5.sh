#!/usr/bin/env bash
# Build, verify, and package the W-025 JIT-5 final native regression.
set -euo pipefail

REPO="$(cd "$(dirname "$0")/../../.." && pwd)"
BUILD="${BUILD:-$REPO/build/windows_x64_phase1}"
HYBRID="${MDVM_HYBRID_BUILD:-$REPO/build/windows_x64_libcore_icu}"
OUT="${1:-$REPO/dist/windows_x64_w025_jit5_host}"
JOBS="${JOBS:-16}"
WINEDEBUG="${WINEDEBUG:--all}"
TEMP_ROOT="$(mktemp -d -p "${TMPDIR:-/tmp}" w025-jit5-package.XXXXXX)"
...
python3 -B "$REPO/tools/verify/windows_x64_w025/check_w025_jit5_source.py" \
  --repo "$REPO" --build "$BUILD"

# The existing W-004 package provides the native/JVMTI artifacts and repeats
# its focused Wine gate before JIT-5 adds the post-removal regression cases.
...
rm -rf "$OUT"
cp -a "$BASE" "$OUT"
rm -f "$OUT/scripts/RUN_W004_HOST.ps1" "$OUT/W004_HOST_CHECKLIST.md"
...
cp -a "$REPO/tools/verify/windows_x64_w025/host/RUN_W025_JIT5_HOST.ps1" "$OUT/scripts/"
cp -a "$REPO/tools/verify/windows_x64_w025/W025_JIT5_HOST_CHECKLIST.md" "$OUT/"
...
printf '%s\n' 'JIT-5 fatal cases preserve exactly three valid minidumps here.' \
  >"$OUT/run/crash/README.txt"

python3 -B "$REPO/tools/verify/windows_x64_w025/check_w025_jit5_source.py" \
  --repo "$REPO" --build "$BUILD" >"$OUT/W025_JIT5_SOURCE_REPORT.txt"
...
jit5_matrix_records=14
jit5_lifecycle_cycles=8
...
echo "W025_JIT5_HOST_PACKAGE_PASS path=$OUT.zip bytes=$(stat -c %s "$OUT.zip")"
