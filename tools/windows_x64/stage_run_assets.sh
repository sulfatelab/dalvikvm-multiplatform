#!/usr/bin/env bash
# Stage product runtime assets that must ship with every Win32 ART tree,
...
DEST="${1:?dest root required (e.g. build/windows_x64_phase1 or dist/windows_x64_phase3_host)}"
BUILD="${2:-$REPO/build/windows_x64_phase1}"
ASSET_CACERTS="$REPO/tools/windows_x64/assets/cacerts"
...
if [[ ! -f "$DEST/run/icu/icudt72l.dat" ]]; then
  for cand in \
      ...
      "$REPO/dist/windows_x64_phase3_host/run/icu/icudt72l.dat"; do
    if [[ -f "$cand" ]]; then
...
if [[ "$need_cacerts" -eq 1 ]]; then
  ...
  elif [[ -x "$REPO/tools/windows_x64/generate_cacerts.sh" ]]; then
    bash "$REPO/tools/windows_x64/generate_cacerts.sh" "$DEST/run/etc/security/cacerts"
...
if [[ "$(cacerts_count "$DEST/run/etc/security/cacerts")" -lt 1 ]]; then
  ...
  echo "  Generate with: bash tools/windows_x64/generate_cacerts.sh tools/windows_x64/assets/cacerts" >&2
  exit 1
fi
