#!/usr/bin/env bash
# Stage real Windows x64 PE native modules for product trees (L-004: one name each).
#
...
#
# Usage:
#   tools/windows_x64/stage_native_modules.sh <dest_root> [hybrid_build_dir] [phase1_dir]
#
# Rejects product use of tools/windows_x64/jni_stubs/libcombined.dll (W-005).
set -euo pipefail
REPO="$(cd "$(dirname "$0")/../.." && pwd)"
DEST="${1:?dest root required}"
HYBRID="${2:-$REPO/build/windows_x64_libcore_icu}"
PHASE1="${3:-$REPO/build/windows_x64_phase1}"
...
if [[ -f "$REPO/tools/windows_x64/jni_stubs/libcombined.dll" ]]; then
  comb=$(md5sum "$REPO/tools/windows_x64/jni_stubs/libcombined.dll" | awk '{print $1}')
