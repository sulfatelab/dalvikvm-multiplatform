/* WinNTFileSystem + Linux open/read/write/close/stat for Win32 ART Phase 3 */
#include <jni.h>
#include <stdint.h>
#include <stdlib.h>
#include <string.h>
...
__declspec(dllexport) jint Java_java_io_WinNTFileSystem_getBooleanAttributes0(JNIEnv* env, jclass cls, jstring jpath) {
  (void)cls;
  if (!jpath) {
    fprintf(stderr, "Windows x64 getBooleanAttributes0: null path\n");
    fflush(stderr);
    return 0;
  }
  ...
  if (!p) {
    fprintf(stderr, "Windows x64 getBooleanAttributes0: GetStringUTFChars failed jlen=%d\n", (int)jlen);
    fflush(stderr);
    return 0;
  }
  fprintf(stderr, "Windows x64 getBooleanAttributes0 jlen=%d raw_first=%02x\n", (int)jlen, (unsigned char)p[0]);
  fflush(stderr);
  ...
  fprintf(stderr, "Windows x64 getBooleanAttributes0 path='%s' attr=0x%lx rv=0x%x\n",
