...
class Emitter:
    ...
    def _emit_one_gensrcs(self, gname: str, gen_paths: list[str],
        ...
        for src in gs.srcs:
            ...
            blocks.append(
                ...
                f"            {_BP2CMAKE}/bp2cmake/capture_output.py\n"
    ...
    def emit_module(self, m: Module) -> str:
        ...
        lines.append("# Port-policy decisions live in //overlay/art_port_policy.py.")
        lines.append("")
    ...
    def module_manifest(self, module: Module) -> dict[str, object]:
        """Return a deterministic, path-independent description of one target."""
        m = deepcopy(module)
        pol = self.ov.policy_for(m.name)
        apply_module_policy(m, pol, self.ov.global_policy)
        sources = self._sources(m)
        generated, _blocks = self._generated_sources(m)
        sources.extend(generated)
        sources.extend(f"{_GENSRC}/{path}" for path in pol.add_gensrc_sources)
        return {
            "aosp_name": m.name,
            "cmake_target": self.ov.target_name(m.name),
            "module_type": m.type,
            "kind": self._kind(m),
            "sources": sorted(_dedupe(sources)),
            "include_dirs": sorted(self._include_dirs(m)),
            "link_dependencies": sorted(self._link_libs(m)),
            "enabled": m.enabled,
        }
