...
"""
    ...
    assert len(catalog["probes"]) == 112
    assert sum(probe["applicable"] for probe in catalog["probes"]) == 109
    assert sum(bool(probe["target_ids"]) for probe in catalog["probes"]) == 58
    ...
    assert {
        ...
        "managed_w036_aot_dispatch": 1800,
        "managed_w037_aot_execution": 1800,
        "managed_w038_aot_exception_fatal_unwind": 1800,
        "managed_w039_aot_unwind_corruption": 1800,
    }
    assert [
    ...
    w031 = [probe for probe in catalog["probes"] if probe["stage"] == "w031"]
    assert [probe["name"] for probe in w031] == [
        "w031aotunwindprobe",
        "managed_w031_aot_unwind",
    ]
    assert [probe["execution"] for probe in w031] == [
        "compile-only",
        "target-runnable",
    ]
    assert all(probe["ctest_registered"] is False for probe in w031)
    assert all(probe["target_ids"] == ["windows-x86_64-msvc"] for probe in w031)
    ...
    w036 = [probe for probe in catalog["probes"] if probe["stage"] == "w036"]
    assert [probe["name"] for probe in w036] == [
        "w036aotdispatchprobe",
        "managed_w036_aot_dispatch",
    ]
    assert [probe["execution"] for probe in w036] == [
        "compile-only",
        "target-runnable",
    ]
    assert all(probe["ctest_registered"] is False for probe in w036)
    assert all(probe["target_ids"] == ["windows-x86_64-msvc"] for probe in w036)
    w037 = [probe for probe in catalog["probes"] if probe["stage"] == "w037"]
    assert [probe["name"] for probe in w037] == [
        "w037aotexecutionprobe",
        "managed_w037_aot_execution",
    ]
    assert [probe["execution"] for probe in w037] == [
        "compile-only",
        "target-runnable",
    ]
    assert all(probe["ctest_registered"] is False for probe in w037)
    assert all(probe["target_ids"] == ["windows-x86_64-msvc"] for probe in w037)
    w038 = [probe for probe in catalog["probes"] if probe["stage"] == "w038"]
    assert [probe["name"] for probe in w038] == [
        "w038aotunwindprobe",
        "managed_w038_aot_exception_fatal_unwind",
    ]
    assert [probe["execution"] for probe in w038] == [
        "compile-only",
        "target-runnable",
    ]
    assert all(probe["ctest_registered"] is False for probe in w038)
    assert all(probe["target_ids"] == ["windows-x86_64-msvc"] for probe in w038)
    w039 = [probe for probe in catalog["probes"] if probe["stage"] == "w039"]
    assert [probe["name"] for probe in w039] == [
        "w039aotunwindcorruptionprobe",
        "managed_w039_aot_unwind_corruption",
    ]
    assert [probe["execution"] for probe in w039] == [
        "compile-only",
        "target-runnable",
    ]
    assert all(probe["ctest_registered"] is False for probe in w039)
    assert all(probe["target_ids"] == ["windows-x86_64-msvc"] for probe in w039)
