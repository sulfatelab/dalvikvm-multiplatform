...
- **Required native acceptance:** repair `ExecuteSwitchImplAsm` with a Win64 PE unwind record and mandatory 32-byte outgoing home area, add structural/body/epilogue virtual-unwind tests, and repeat the automated native exception-shape matrix. Design a replacement SOE delivery adapter that does not depend on retaining a fixed no-access page inside the Windows stack; then repeat repaired SOE recovery and fatal static/JIT/OSR paths. Also add debugger continue and stack-budget measurements, dynamic-JIT rollback/collection/sampling stress in J-2 and J-1, exception-unwind XMM sentinels, current-Windows repetition, and forced rejection of every named incompatible CET policy family. Native NPE, foreign VEH/frame-SEH, standalone UEF, OSR live unwind, HSP-disabled startup, and normal-return XMM sentinels already pass on build 19044; current Windows repeats stack/UEF diagnostics.
...
- **Blocked on / design doc:** confirmed `ExecuteSwitchImplAsm` Win64 frame repair, a replacement Windows SOE delivery design, then the repaired Stage E fatal/SOE matrix and broader dynamic-table sampling/churn acceptance; [win32_faults_and_stacks.md](win32_faults_and_stacks.md)
- **Opened:** 2026-07-16
- **Updated:** 2026-07-28 — native E4 on Windows build 26100 confirms `ExecuteSwitchImplAsm + 0x9` as the first missing live unwind record; wrapper repair and repeat are next
...
4. **W-010/W-014 Stage E** — repair the native-confirmed `ExecuteSwitchImplAsm` Win64 frame and repeat fatal dispatch; design a replacement Windows SOE delivery mechanism after native evidence invalidated the fixed page; then repeat repaired SOE paths and complete dynamic-table churn/sampling, debugger, policy, and stack-budget acceptance.
