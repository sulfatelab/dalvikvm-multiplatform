...
- **State:** OPEN (Stages 0/C/D and static fatal boundaries implemented locally; dynamic-JIT unwind and native acceptance remain)
...
- **Static fatal-boundary result:** PE unwind records now cover `art_quick_invoke_stub`, `art_quick_invoke_static_stub`, and `art_quick_generic_jni_trampoline`. An emitted-record audit verifies their frame anchors, allocations, GPR saves, and XMM saves. The hardened static `-Xint` JNI abort gate reaches initial VEH and UEF and creates a new valid `MDMP` dump. Recursive `RtlVirtualUnwind2` tracing also proves the remaining limitation: a threshold-zero JIT fatal path has no runtime-function entry, leaf-walks managed frame contents, and can fail exception dispatch before UEF. Dynamic JIT unwind registration and code-cache ownership/removal ordering are therefore required before universal fatal-UEF support can be claimed.
- **Required native acceptance:** repeat the nterp/JIT NPE/SOE and recovery/GC evidence on Windows 10/current Windows; exact wrong-address cases; foreign VEH before/after/promotion; frame-based SEH for an unrecognized AV; debugger continue; stack-budget measurements; static fatal predecessor-UEF/dump behavior; dynamic-JIT runtime-function dispatch; and HSP-disabled startup plus forced compatibility/audit/strict rejection.
- **Code anchors:** `vendor/art/runtime/runtime.cc`; `runtime/multiplatform/windows/sigchain_windows.cc`; `runtime_windows.cc`; `runtime/fault_handler.{h,cc}`; `runtime/arch/x86/fault_handler_x86.cc`; `runtime/arch/x86_64/quick_entrypoints_x86_64.S`; `tools/verify/win64_phase1/check_win32_boundary_unwind.py`; `tools/verify/win64_phase4/{src/W010ManagedFaultProbe.java,run_w010_managed_fault_probe.sh,run_crashnative.sh,RESULT-w010-managed-faults.md}`
- **Blocked on / design doc:** dynamic-JIT PE unwind design plus native Stage E evidence with W-014; [win32_faults_and_stacks.md](win32_faults_and_stacks.md)
- **Opened:** 2026-07-16
- **Updated:** 2026-07-27 — recovery/GC and chain probes pass; static fatal boundaries reach UEF/minidump; dynamic-JIT unwind and native policy/debugger/stack-budget acceptance remain
