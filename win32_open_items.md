...
| Threads / managed faults | **W-014 OPEN:** stack bounds are still clamped `VirtualQuery` estimates, pthread stack attributes are ignored, and ART stack protection is disabled. **W-010 OPEN:** the VEH remains diagnostic-only, so nterp/JIT implicit stack probes cannot become managed `StackOverflowError`. CET user shadow stacks/HSP are explicitly unsupported and must be disabled for the ART process. |
...
- **Current CET state:** lld 21 currently defaults to `/CETCOMPAT:NO`, and the inspected `dalvikvm.exe`, `art.dll`, and `c++.dll` have no CET-compatible extended characteristic or Guard EH-continuation table. This is accidental linker-default behavior: no explicit project link flag or startup mitigation-policy rejection has landed yet.
...
- **Opened:** 2026-07-16
- **Updated:** 2026-07-26 — from-scratch VEH/sigchain design selected; CET user shadow stacks are explicitly unsupported and require process-level disable plus early rejection
...
- **Opened:** 2026-07-16
- **Updated:** 2026-07-26 — from-scratch design adds complete allocation validation, `_beginthreadex`, pthread handle lifetime, page-state/detach restoration, and atomic W-010 activation
