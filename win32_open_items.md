...
- **State:** OPEN (P5 implementation plus JIT-1 encoding and JIT-2 mapping/policy native gates complete; JIT-3 lifecycle/unwind stress, final regression, and the temporary J-1 opt-out remain)
- **Kind:** host-validation gap / temporary diagnostic workaround
- **Area:** art / jit / compiler
- **Symptom / why:** The corrected default now reproduces ART's Linux-visible `[data R][code RX]` contiguous primary layout with a coherent RW updater alias. JIT-1 completed the direct encoding-site checks and JIT-2 completed real-Windows mapping, backing, pressure, CFG, and dynamic-code-policy acceptance. Remaining W-025 work is collection/reuse with concurrent unwind sampling, final default-path regression, and removal of the J-1 diagnostic fallback. Threshold zero is no longer a JIT-memory unknown; its implementation work is tracked under W-024.
- **Current behavior:**
...
- **Separate residual:** W-024 is closed and JIT-1/JIT-2 are complete. W-025 still requires collection/reuse with concurrent unwind lookup, the final default-path regression, and J-1 diagnostic-opt-out removal. CET user shadow-stack support is not W-025 work: it is an explicit non-goal, and the process must run with HSP disabled under W-010's activation contract.
...
- **Evidence:** `tools/verify/windows_x64_w025/RESULT-jit2-native.md`; `tools/verify/windows_x64_w025/evidence/jit2_native/ACCEPTANCE.md`; returned archive SHA-256 `3d32e3a15f4e4ad8b5e98a769ea02e638abeb975755064177b1a4d3a6bf9364e`
- **Opened:** 2026-07-19
- **Updated:** 2026-07-29 - JIT-2 native mapping/policy acceptance passes 14/14; JIT-3 lifecycle/unwind stress, final regression, and temporary J-1 diagnostic-opt-out removal remain
...
4. **W-025 JIT-3/FS-3** - run allocation/collection/reuse stress with
   concurrent dynamic unwind lookup and sampling; JIT-2 mapping/policy is
   native-accepted 14/14.
