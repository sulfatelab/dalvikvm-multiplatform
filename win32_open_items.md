...
| Heap memory | W-013 Stages A–C complete: explicit MoreCore-only dlmalloc, direct mspace owners, and `VirtualAlloc2`/shared-owner `MemMap`; Stages D–E remain OPEN |
...
- **Current behavior:** Stages A through C are complete. `_WIN32`/`WIN32` remain visible; dlmalloc's Win32 defaults respect ART's explicit `HAVE_MMAP=0`, `HAVE_MORECORE=1`, `MORECORE_CONTIGUOUS=1`, `USE_LOCKS=0`, mspace-only policy; growth granularity is `dwPageSize`; failures set `ENOMEM`. Every heap and JIT mspace stores its owner provider in `malloc_state::extp/exts`, and MoreCore dispatches directly without `Runtime::Current()`, JIT-cache lookup, or continuous-space scans. Heap mspaces use `DlMallocSpace::lock_`; JIT mspaces use `Locks::jit_lock_`, with provider detach/rebind across clear, reset, destruction, and JIT-region moves. Windows anonymous mappings now distinguish anywhere, below-4-GiB, and exact requests explicitly; low/aligned allocation uses `VirtualAlloc2` plus `MEM_ADDRESS_REQUIREMENTS`; reuse inside an existing reservation changes protection in place; and logical views share one allocation-base owner so private and section mappings use the correct whole-release API exactly once. `SetSize()`/`AlignBy()` no longer issue invalid partial `MEM_RELEASE`, but Stage D still needs explicit deactivate/discard operations. Fixed file-view replacement over an ordinary `VirtualAlloc` reservation remains unsupported and is not used by the imageless/JIT product path.
...
- **Evidence:** `tools/verify/win64_w013/RESULT.md`; external dlmalloc `f3356ce`; ART Stage A `8c900a9e4b`; ART Stage B `d011d72d56`; ART Stage C `2fa301a13b`
- **Opened:** 2026-07-16
- **Updated:** 2026-07-25 — Stages A–C complete; Stages D–E remain open
