...
| Threads / managed faults | **W-014 OPEN:** stack bounds are still clamped `VirtualQuery` estimates, pthread stack attributes are ignored, and ART stack protection is disabled. **W-010 OPEN:** the VEH remains diagnostic-only, so nterp/JIT implicit stack probes cannot become managed `StackOverflowError`. |
...
- **State:** OPEN (design re-evaluated 2026-07-26; no implementation or native acceptance yet)
...
- **Windows 10 design contract:** use `GetCurrentThreadStackLimits()` as the authoritative current-thread system-stack interval. The project baseline is Windows 10 build 17134+ and already compiles with `_WIN32_WINNT=0x0A00`, so no Windows 7 fallback or dynamic resolution is required. Validate that the current SP lies inside the interval, bounds are aligned and large enough, and `VirtualQuery(SP).AllocationBase` agrees with the low limit. Use `NtCurrentTeb()->NtTib.StackBase` and the changing committed `StackLimit` only as diagnostics; do not use undocumented `DeallocationStack` or treat `StackLimit` as the reservation low address. Reject impossible/manual-stack attachment instead of inventing bounds.
- **Thread-creation contract:** honor `pthread_attr_t::stacksize` with `CreateThread(..., STACK_SIZE_PARAM_IS_A_RESERVATION, ...)`; use zero only for the executable default. Disable ART's custom `pthread_attr_setstack()` thread-pool path on Windows and pass its requested size instead. Reject non-null custom stack addresses rather than silently accepting an operation Win32 threads cannot implement directly.
- **ART protection contract:** retain ART's Linux-like fixed one-page protected region and 8 KiB reserved gap. Keep one system-owned bottom page, commit the exact ART page inside the validated Windows stack reservation with `VirtualAlloc(MEM_COMMIT)`, then use `VirtualProtect(PAGE_NOACCESS)`; unprotect/reprotect only that page. Do not use one-shot `PAGE_GUARD` as ART's fixed page. Use a Windows-specific installation hook so Linux `VM_GROWSDOWN` recursive touching and stack `madvise()` are not reused.
- **W-010 ownership boundary:** W-014 owns bounds, requested stack capacity, stack accounting, and the fixed protected page. W-010 owns the `EXCEPTION_POINTERS`/`CONTEXT` adapter, generated-code recognition, expected `RSP - reserved_bytes` fault validation, and RIP redirection to `art_quick_throw_stack_overflow`. Windows implicit SO checks must not be enabled until both sides are complete. The VEH overflow path must remain non-allocating and very small because Windows has no `sigaltstack` and executes the handler on the faulting stack.
...
- **Required stages:** (A) bounds helper plus pthread/thread-pool stack-size correctness; (B) dormant Windows fixed-page protection with implicit SO still disabled; (C) W-010 stack-overflow VEH translation and atomic activation; (D) Wine/Linux regression plus native Windows acceptance.
- **Required acceptance:** main/default stack; Java and native-created 64 KiB/256 KiB/1 MiB/2 MiB/over-8-MiB reservations; ART runtime/GC/JIT pools; attached `_beginthreadex` and `CreateThread` threads; explicit fiber/manual-stack policy; repeated caught overflow under switch/nterp/JIT; post-overflow GC/JNI/second-overflow; debugger/VEH chaining; Windows 10 build 17134+ and current Windows; unchanged Linux `018-stack-overflow` behavior.
...
- **Microsoft contracts:** [`GetCurrentThreadStackLimits`](https://learn.microsoft.com/windows/win32/api/processthreadsapi/nf-processthreadsapi-getcurrentthreadstacklimits), [`CreateThread`](https://learn.microsoft.com/windows/win32/api/processthreadsapi/nf-processthreadsapi-createthread), [thread stack size](https://learn.microsoft.com/windows/win32/procthread/thread-stack-size), [`VirtualQuery`](https://learn.microsoft.com/windows/win32/api/memoryapi/nf-memoryapi-virtualquery), [`VirtualAlloc`](https://learn.microsoft.com/windows/win32/api/memoryapi/nf-memoryapi-virtualalloc), [`VirtualProtect`](https://learn.microsoft.com/windows/win32/api/memoryapi/nf-memoryapi-virtualprotect), and [guard-page behavior](https://learn.microsoft.com/windows/win32/memory/creating-guard-pages).
...
- **Blocked on / design doc:** shared activation with W-010; [win32_tls_jit_entrypoints.md](win32_tls_jit_entrypoints.md) §6.9–§6.10
- **Opened:** 2026-07-16
- **Updated:** 2026-07-26 — authoritative Windows 10 API design, pthread conflict, W-010 split, and Wine evidence recorded
...
- **Unwind and W-010 scope:** Win64 quick assembly still emits no `.pdata`/`.xdata`; ART managed stack walking is separate. W-003 closes with W-010 explicitly owning broader VEH/SEH/native-unwind policy and the independent nterp implicit-null fault at `nterp_op_invoke_virtual+0x3a`. The W-003 probe excludes only that implicit-null case and retains class-cast, array-store, and bounds paths; no product fallback was added.
