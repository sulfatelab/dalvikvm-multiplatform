...
| AOT/OAT | Boot-only implementation is active. Windows keeps Linux ART ELF identity and `ART_PAGE_SIZE_AGNOSTIC=1`; Linux remains 16-KiB aligned while Windows uses 64-KiB `kElfSegmentAlignment` with `kMaxPageSize = 16384`. W-028 completes trivial native generation, and W-029 pins one package-relative boot identity. W-030 now implements checked private-copy ELF segments for validation-only and executable opens plus reused VDEX `oatdex` bytes; its direct native probe and end-to-end startup complete sequence steps 4/5. Windows generates an LZ4 image because an uncompressed file view cannot replace the committed boot reservation; Linux stays uncompressed. Step 8 is complete: one manifest binds and validates each matching path-sensitive ART/OAT/VDEX cache set, without requiring cross-generation byte identity. The canonical experimental package starts without imageless fallback, but step 2 still lacks ART-level negative mismatch diagnostics and step 9 lacks normal product selection plus successful fallback. The gate uses `-Xint`, so unwind, CFG, real boot-OAT RX execution, and OAT-1 measurements remain open. Application OAT, OAT-2, unloading, explicit CFG enforcement, and security hardening are deferred. H-004 tracks the glibc positive-dlopen skip; H-005 tracks pre-dispatch characterization execution. See the tracker and accepted [W-028](docs/history/windows_x64_w028_result.md), [W-029](docs/history/windows_x64_w029_result.md), and [W-030](docs/history/windows_x64_w030_result.md) records. |
