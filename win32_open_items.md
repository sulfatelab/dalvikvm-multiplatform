...
- **Required native acceptance:** run the complete 30-record E6 host matrix to repeat static, JIT J-2/J-1, and OSR J-2/J-1 fatal origins after the repaired lookup chain, and quantify the remaining failures independently. Design a replacement SOE delivery adapter that does not depend on retaining a fixed no-access page inside the Windows stack; then repeat repaired SOE recovery. Also add debugger continue and stack-budget measurements, dynamic-JIT rollback/collection/sampling stress in J-2 and J-1, exception-unwind XMM sentinels, forced rejection of every named incompatible CET policy family, and a native exception inside the pending bridge range if practical. Native NPE, foreign VEH/frame-SEH, standalone UEF, OSR live unwind, HSP-disabled startup, normal-return XMM sentinels, and the diagnosed primary fatal chain already pass.
...
- **Blocked on / design doc:** complete native E6 host matrix, a replacement Windows SOE delivery design, then the repaired SOE matrix and broader dynamic-table sampling/churn acceptance; [win32_faults_and_stacks.md](win32_faults_and_stacks.md)
- **Opened:** 2026-07-16
- **Updated:** 2026-07-28 — native E6 restores JNI fatal UEF/dump dispatch through the primary interpreter bridge; full host matrix is next
...
4. **W-010/W-014 Stage E** — run the complete E6 native host matrix now that JNI fatal dispatch reaches UEF/dump; design a replacement Windows SOE delivery mechanism after native evidence invalidated the fixed page; then repeat repaired SOE paths and complete dynamic-table churn/sampling, debugger, policy, pending-tail, and stack-budget acceptance.
