...
| Threads / managed faults | **W-014 OPEN:** Stages A-B now have exact current-stack bounds, `_beginthreadex` reservation semantics across 64 KiB through 9 MiB probes, retained pthread lifetime, Windows thread-pool sizing, measured excluded-low accounting, a verified fixed `PAGE_NOACCESS` page, detach restoration, and post-SOE recovery/GC stress; native A-B/E acceptance remains. **W-010 OPEN:** Stages 0/C/D, static invoke/JNI plus split OSR unwind, full-width XMM6-XMM15 boundary adapters, and dynamic-JIT metadata/registration/lifecycle are implemented and Wine-verified. Product Win64 translates recognized nterp/JIT implicit null and stack-overflow faults through common ART handlers; static `-Xint` and dynamic threshold-zero JNI fatal dispatch reach UEF plus valid minidumps. Native HSP/debugger/stack-budget/NPE/SOE/OSR/XMM and large-table acceptance remain. |
...
- **State:** OPEN (Wine implementation and focused fatal matrix complete; native Windows Stage E acceptance remains)
- **Kind:** workaround → candidate permanent design
- **Area:** art / exceptions
- **Current behavior:** `sigchain_windows.cc` owns one immutable special-`SIGSEGV` action and a first VEH. It filters exact continuable access violations, forwards a minimal `siginfo_t` plus a non-owning view of the live Win64 `CONTEXT`, supports promotion/removal, and returns `EXCEPTION_CONTINUE_SEARCH` for all unsupported or unrecognized exceptions. `runtime_windows.cc` retains a separate diagnostic VEH and a fatal UEF that writes a best-effort minidump, then calls its predecessor or returns search. Runtime teardown removes the diagnostic VEH while `art.dll` is executable and preserves any later host UEF. Stage D enables Win64 x86_64 implicit null and stack-overflow checks, keeps implicit suspend checks off, registers stack before null, and registers the immutable nterp code range before startup can publish nterp entrypoints.
- **Current test isolation:** W-003's attributed frame probe still excludes its historical implicit-null subtest, but the dedicated `W010ManagedFaultProbe` now covers that product path. No explicit nterp null check, forced-interpreter fallback, or Windows-only compiler check was added.
- **Selected design:** Windows does not emulate general POSIX sigchain. The narrow facade is now implemented for the one special `SIGSEGV` action registered by common `FaultManager`; its first ART VEH filters exact continuable `EXCEPTION_ACCESS_VIOLATION` records, passes a minimal `siginfo_t` plus a non-owning view of the real Win64 `CONTEXT` and AV access kind into the existing generated-code handlers, and returns search for every unrecognized exception. The x86_64 adapter modifies `CONTEXT.Rip`/`Rsp`/`Rax` in place, validates `R15 == Thread*`, prevents recursive dispatch, keeps stack-before-null handler order, and requires read access plus containment in W-014's recorded fixed page. Win64 x86_64 implicit suspend checks are not part of this scope. Native `EXCEPTION_STACK_OVERFLOW`, moving `PAGE_GUARD`, execute AV, breakpoints, illegal instructions, and native/unregistered AVs are not converted to Java exceptions.
...
- **Activation:** implicit null/SO flags and Win64 nterp/JIT are one capability. The main thread's fixed page is installed before the runtime publishes the VEH/handlers; later non-AOT attachments install the page directly under the enabled flag. A normal started runtime now rejects `-Xno-sig-chain` on Windows exactly as on Linux. The option remains only for genuine non-started compiler/tool runtimes such as dex2oat.
- **CET/HSP contract:** Win64 ART does not support CET user shadow stacks (Hardware-enforced Stack Protection). `art_quick_do_long_jump` restores an older regular `RSP` and executes `ret` without synchronizing CET's protected return stack, affecting ordinary managed throws, deoptimization, JNI exception delivery, and W-010 implicit throws. W-010's `CONTEXT.Rip`/`Rsp` edits also conflict with context-IP validation. Every project PE link must explicitly use `/CETCOMPAT:NO`; packaged DLLs must omit `IMAGE_DLL_CHARACTERISTICS_EX_CET_COMPAT`; startup must reject every defined incompatible `ProcessUserShadowStackPolicy` field before ART threads/JIT. Compatibility, audit, strict, context-IP-validation, and non-CET-binary fields are unsupported. `CetDynamicApisOutOfProcOnly` is allowed because it does not enable HSP or context-IP validation; `ReservedFlags` is ignored because reserved bits have no defined meaning. CFG remains separate W-025 work; `/guard:ehcont`, dynamic continuation targets, CET-compatible JIT ranges, IBT, or `-fcf-protection` are not fixes.
- **Current CET state:** Stage 0 is implemented locally. Generator policy applies `LINKER:/CETCOMPAT:NO` to every generated non-static target; nine handwritten Win64 CMake harnesses and three direct Clang/lld links use the same explicit option. `Runtime::Init()` queries `ProcessUserShadowStackPolicy` after logger selection and before `MemMap::Init()`, ART threads, nterp, or JIT. It constructs an incompatibility mask from named SDK fields, accepts `CetDynamicApisOutOfProcOnly` and reserved fields, accepts `ERROR_INVALID_PARAMETER` only below Windows build 19041, and fails closed on unexpected query/version failures. The probe tests every named incompatible field, safe/reserved fields, and mixtures. The structural/package gate reports six enforced host packagers and 25 PE link targets; it inspects 25 build-tree PE files and 54 files with the focused W-010/W-014 staged package, including external LLVM `c++.dll`, all without the CET-compatible marker. A synthetic CET-marked PE is rejected. Wine build 19043 reports `known_incompatible=0x00000000` and PASS. The first Windows build-19044 native candidate exposed the former classifier bug: raw `flags=0x00000100` is `CetDynamicApisOutOfProcOnly`, but the obsolete all-zero check rejected it. The corrected package must be rerun; forced named-incompatible policy rejection remains a Stage 0 blocker.
...
- **Dynamic-JIT implementation:** the x86_64 assembler emits explicit version-1 unwind bytes with shortest legal allocation forms, descending instruction-end offsets, even-slot padding, and validation. Optimizing Win64 JIT methods reserve and force-spill RBP, then establish it after the fixed allocation. Normal/FastNative JNI stubs use the same anchor and a four-register RBX/R12-R14 scratch set; CriticalNative retains a fixed-RSP record. Generation is independent of DWARF CFI, and invalid/missing enabled metadata rejects compilation before allocation. `Reserve()` adds the DWORD-aligned xdata tail after roots/stack maps; `Commit()` writes through the RW alias and registers a stable one-entry table before publication. `FreeLocked()` deletes the exact table before debug-info removal and mspace reuse, while destruction clears all entries before mapping teardown. The J-2/J-1 lifecycle gate proves invalidation retention, real collection, lookup disappearance, exact address reuse, re-registration, and replacement execution; the registry gate proves generated-frame virtual unwind.
- **Required native acceptance:** repeat the nterp/JIT NPE/SOE and recovery/GC evidence on Windows 10/current Windows; exact wrong-address cases; foreign VEH before/after/promotion; frame-based SEH for an unrecognized AV; debugger continue; stack-budget measurements; static fatal predecessor-UEF/dump behavior; both OSR records' lookup/live unwind plus the JIT-origin and OSR-origin fatal paths; dynamic-JIT lookup/unwind/rollback/collection/sampling stress in J-2 and J-1; full XMM6-XMM15 normal-return and unwind sentinels; HSP-disabled startup with `known_incompatible=0`; and forced rejection of every named incompatible policy family. A raw word containing only `CetDynamicApisOutOfProcOnly` or reserved fields must not be rejected.
- **Code anchors:** `vendor/art/runtime/runtime.cc`; `runtime/multiplatform/windows/sigchain_windows.cc`; `runtime_windows.cc`; `runtime/fault_handler.{h,cc}`; `runtime/arch/x86/fault_handler_x86.cc`; `runtime/arch/x86_64/quick_entrypoints_x86_64.S`; `runtime/multiplatform/windows/jit_unwind_windows.*`; `compiler/optimizing/code_generator_x86_64.*`; `compiler/utils/x86_64/{assembler,jni_macro_assembler}_x86_64.*`; `runtime/jit/{jit_code_cache,jit_memory_region}.*`; `tools/verify/win64_phase1/{check_win32_boundary_unwind.py,win32_osr_unwind_probe.cc,win32_jit_unwind_registry_probe.cc}`; `tools/verify/win64_phase4/{src/W010ManagedFaultProbe.java,src/JitUnwindLifecycleProbe.java,src/CrashNativeProbe.java,run_osr_unwind_probe.sh,run_w010_managed_fault_probe.sh,run_jit_unwind_lifecycle.sh,run_jit_fatal_unwind.sh,run_osr_fatal_unwind.sh,RESULT-w010-managed-faults.md}`
- **Blocked on / design doc:** native Stage E evidence with W-014, native OSR lookup/fatal acceptance, native repetition of the full XMM6-XMM15 sentinels, and broader dynamic-table sampling/churn acceptance; [win32_faults_and_stacks.md](win32_faults_and_stacks.md)
- **Opened:** 2026-07-16
- **Updated:** 2026-07-27 — recovery/GC and chain probes pass; static, JIT-origin, and OSR-origin J-2/J-1 fatal paths reach UEF/minidump; R12-anchored OSR entry plus RBP JIT handoff, serialization, xdata, registration, collection/reuse lifecycle, JNI metadata, split OSR entry/return unwind, and full XMM6-XMM15 boundary preservation pass focused Wine; native repetition, broader churn/sampling, and native policy/debugger/stack-budget acceptance remain
...
- **Current protection behavior:** every attached non-AOT Windows thread preserves the lowest page and adjacent bottom no-access/guard regions, selects the first exact reserved or committed-private read/write page, records its original state, installs and verifies `PAGE_NOACCESS`, and publishes stack bounds above the measured prefix and protected page. `ProtectStack()`/`UnprotectStack()` perform verified transitions; install rollback and `Thread` teardown restore and verify the exact original reserved/committed state. The main thread installs this page before architecture flags are selected; Stage D then enables implicit SO/null checks and all later attachments take the enabled path directly. The state record remains below ART's native-code-visible TLS layout.
- **Generated-code integration:** x86_64 optimizing code and nterp keep the normal unconditional `rsp - ART_STACK_OVERFLOW_GAP_x86_64` probe. Stage D registers common stack-before-null handlers and translates exact read faults contained in the recorded page. Repeated main/child-thread nterp and threshold-zero JIT SOE passes under Wine. The switch interpreter continues to use explicit `Thread::stack_end_` comparisons.
...
- **ART protection contract:** retain ART's Linux-like fixed one-page protected region and 8 KiB x86_64 reserve. Preserve the lowest allocation page plus every adjacent bottom `PAGE_NOACCESS`/`PAGE_GUARD` region; select the first suitable `MEM_RESERVE` or ordinary committed-private read/write page above that measured excluded-low prefix. Commit a reserved candidate with `VirtualAlloc(MEM_COMMIT, PAGE_READWRITE)`; leave a committed candidate and its contents intact. Then protect exactly that page with `VirtualProtect(PAGE_NOACCESS)`. Never assume `low + one page` is safe and never adopt the moving guard. Record the excluded-low size and the page address/state/original state in `Thread`; the Stage C W-010 handler validates read-operation, protected state, and page containment without `VirtualQuery()` in VEH. Unprotect/reprotect only that page. Do not use one-shot `PAGE_GUARD`, Linux `VM_GROWSDOWN` recursive touching, or stack `madvise()`.
...
- **Wine/Linux/native evidence (2026-07-27):** the permanent `win32_thread_stack_probe` passes main/default, requested 64 KiB, 256 KiB, 1 MiB, 2 MiB, and 9 MiB cases under Wine; stable self/ID extraction; callback results; current-only inspection; detach before and after exit; attribute-detached operation; 512 join and 128 detach iterations; raw `CreateThread`; invalid/custom attribute rejection; and fiber rejection. Wine reports sub-default requests at the executable default, but native Windows build 19044 reports exact 64 KiB and 256 KiB reservations. With `STACK_SIZE_PARAM_IS_A_RESERVATION`, the native result is required. The corrected probe now requires request-based allocation-granularity rounding on native Windows, requires `runtime=native ... wine_default_clamps=0` in returned evidence, and permits Wine's detected/default-clamp behavior only with an explicit `runtime=wine` diagnostic. The Stage B probe passes eight deterministic selectors, 64 committed-page restore cycles on Wine's main stack, 64 real reserved-page commit/decommit cycles, a 2 MiB pthread stack, and 258 exact assembly-load faults. Wine reports `excluded=8192`, original `MEM_COMMIT/PAGE_READWRITE` on both real stacks. The managed SO gate catches two overflows on the main thread and two on a child in both nterp and threshold-zero JIT; every catch constructs a stack trace, resumes allocation/time operations, and requests GC. The W-002 gate now makes every raw thread attach/callback/detach, consume about 16 KiB of native stack, reattach/callback/detach, and passes all eight mode/repeat processes. Win64 `dalvikvm -showversion`, default JIT Hello, ThreadHeavy, and the complete Phase-4 suite pass with fixed pages installed. The Linux full rebuild, `dalvikvm -showversion`, and shared-boot imageless Hello pass. The CET audit covers 25 PE link targets, 25 build-tree PE files, and 54 files with the focused W-010/W-014 staged package. Native guard-growth, protected-stack layout, and corrected-package small-reservation acceptance remain open; native join/detach handle counts passed in the first combined candidate.
- **Required stages:** Stage 0 build/startup CET exclusion, W-014 Stages A-B, W-010 Stage C VEH/non-owning real-`CONTEXT` adaptation, and Stage D atomic implicit-null/SO activation are implemented locally with focused Wine and Linux evidence. Stage E native Windows acceptance remains.
...
- **Opened:** 2026-07-16
- **Updated:** 2026-07-27 — expanded reservation, post-SOE stack-trace/allocation/GC, chain probes, and the coupled JIT/OSR fatal-unwind Wine matrix pass; native A-B/E acceptance remains
...
| CET user shadow stacks / Hardware-enforced Stack Protection | Unsupported for current Win64 ART; all defined incompatible HSP/context-validation fields must be disabled, while `CetDynamicApisOutOfProcOnly` and reserved fields are not treated as HSP enablement ([win32_faults_and_stacks.md](win32_faults_and_stacks.md)) |
...
- **Native-boundary repair:** W-003 originally made `art_quick_invoke_stub`, `art_quick_invoke_static_stub`, and `art_quick_osr_stub` reserve a Windows-only 96-byte area for XMM6-XMM11, which is the accepted native R1 contract. The later W-010 exception-unwind work expands only that adapter to 160 bytes and preserves the lower 128 bits of XMM6-XMM15 before managed argument setup or OSR. The area remains outside canonical ART frames, preserves alignment, changes the Win64 OSR conceptual CFA to 256, and leaves Linux at 80. Focused Wine proves `fullSelfTestMask=1023`; native repetition belongs to W-010 Stage E without reopening W-003.
...
4. **W-010/W-014 Stage E** — run and review the staged 30-record native package, which now includes split OSR/invoke unwind, full XMM6-XMM15 sentinels, and static/JIT/OSR fatal paths with at least five preserved minidumps; then complete dynamic-table churn/sampling, debugger, policy, and stack-budget acceptance.
5. **H-001** — host Phase-4 with multiplatform package.
6. ~~**L-005** — Linux Hello gate~~ **CLOSED**.
...
*Last snapshot: 2026-07-27 — W-001/W-002/W-003/W-004/W-011/W-012/W-013/W-024 closed; Nterp ON; corrected pagefile-section dual view is the managed/native-JIT default (12/12 smoke, 14/14 matrix); D-1 complete; W-002 native R2 passes 21/21 records with 8/8 OSR, 8/8 attach, exact deterministic thresholds/checksum, and no fatal marker or dump; W-003 native R1 passes 19/19 records with matched PE/ELF trap parity, repaired XMM6–XMM11 invoke/OSR boundaries, 8/8 attributed frame runs, 6/6 XMM sentinel runs with exact `0x3f` self-test, explicit J-2 creation, and no fatal marker or dump; W-010 Stages 0/C/D now provide explicit `/CETCOMPAT:NO`, early fail-closed HSP policy enforcement, exact record/live-`CONTEXT` adaptation, and active common null/SO translation. Focused Wine passes 64+64 caught NPEs and repeated main/child SOEs in nterp and threshold-zero JIT with post-fault stack-trace/allocation/GC recovery, started-runtime `-Xno-sig-chain` rejection, foreign VEH ordering/promotion and frame-SEH checks, clean handled-fault diagnostics/dump state, and the complete Phase-4 aggregate. Static invoke/JNI PE unwind records let the `-Xint` fatal gate reach UEF and create a valid minidump. The static OSR boundary now uses an R12-anchored copy interval, an explicit RBP-to-copied-RSP JIT handoff, and a zero-prologue RSP-based return record; exact emitted inspection, variable-depth live virtual unwind with clobbered entry/return RBP, full XMM6-XMM15 restore across both OSR ranges and both invoke records, canonical epilogue unwind, the 6/6 full-width normal-return sentinel, and the 8/8 actual OSR matrix pass. Section 7.9 dynamic-JIT support forces/anchors RBP in optimizing and normal/FastNative JNI code, emits validated SDK-independent PE unwind bytes, retains fixed-RSP CriticalNative records, stores aligned xdata in the primary JIT data allocation, registers before publication, unregisters before debug-info removal/mspace reuse, and clears before mapping teardown. The 6-group serializer, standalone lookup/virtual-unwind/delete/re-register probe, J-2/J-1 product collection plus exact-address-reuse lifecycle gate, threshold-zero J-2/J-1 and OSR-origin J-2/J-1 UEF/minidump gates, 12/12 JIT smoke, 7/7 normal/FastNative ABI modes, focused CriticalNative, and Linux builds pass. The focused W-010/W-014 native candidate now stages a 30-PASS automated matrix with per-case minidump preservation and a five-dump minimum. Native OSR lookup/fatal acceptance, native repetition of the full XMM6-XMM15 sentinel, broader dynamic-table churn/sampling, debugger-quality dump-stack evidence, and native policy/NPE/SOE/debugger/stack-budget acceptance remain open. W-014 Stages A-B have exact stack bounds, 64 KiB through 9 MiB `_beginthreadex` reservation semantics, opaque pthread lifetime/tagged external identity, measured excluded-low accounting, a verified fixed page, and detach restoration with focused Wine page/reattach/SOE recovery gates while native A-B/E acceptance remains; W-013 native R2 passes 56/56 with complete metrics, 20/20 repeated starts, large-heap pressure, J-1/default JIT, and no dumps; Math.ceil/floor and one common ELF/PE table are restored; interpreter.cc matches upstream; compile records are opt-in; `ART_WIN64_JIT_DUAL=0` temporarily retains J-1 for diagnosis; W-025 broader real-host acceptance remains; 5 OPEN workarounds remaining.*
