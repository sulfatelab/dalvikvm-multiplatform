...
| AOT/OAT | Boot-only implementation is active. Windows keeps Linux ART ELF identity and `ART_PAGE_SIZE_AGNOSTIC=1`; Linux remains 16-KiB aligned while Windows uses 64-KiB `kElfSegmentAlignment` with `kMaxPageSize = 16384`. W-028 completes trivial native generation, W-029 pins one package-relative boot identity, and W-030 completes boot-only private-copy ELF/VDEX loading plus matching LZ4 cache-set staging. W-031 implements checked/deduplicated managed, JNI, and seven-trampoline `.oat_unwind.windows` metadata, both loader modes, registration, lookup, synthetic virtual unwind, and JIT-disabled calls. W-039 now rejects 23 semantic corruptions through 46 corrupt opens, proves canonical registration/deletion lifetime, and passes 23 unwind-diagnosed whole-transaction imageless fallbacks with zero boot image spaces; step 6 remains partial only for actual XMM-bearing boot-AOT frame execution. W-032 completes `.oat_cfg.windows` transport and forced-policy guarded quick/JNI observation. W-036 proves ordinary boot-OAT dispatch, W-037 proves representative paired relocation/translated NPE/BSS-root survival, and W-038 proves managed-exception plus fatal live walking through the repaired 187-record nterp boundary. Step 10's remaining finite matrix is resolution-trampoline, interface/IMT, quick-to-interpreter, deoptimization, translated-SOE, reflection, and class initialization. OAT-1's RW/NX-to-RX transition remains default-valid observation only. W-033 must compare the conditional OAT-2 replacement candidate with the implemented private-copy baseline; W-034/W-035 require an explicit proceed decision, and final integration must remove the losing executable path. Step 2 still lacks ART-level negative mismatch diagnostics. Step 9 still lacks normal product selection and missing/stale/wrong-target/cross-artifact fallback, although W-039 closes its unwind-corruption member. Full baseline measurements, H-005, and step-3 two-target trampoline disassembly remain open. Application OAT, successful unloading, outgoing quick-code CFG instrumentation, and security hardening are deferred. See the [AOT/OAT design](win32_aot_oat.md), tracker, and accepted [W-028](docs/evidence/windows_x64_w028_result.md), [W-029](docs/evidence/windows_x64_w029_result.md), [W-030](docs/evidence/windows_x64_w030_result.md), [W-031](docs/evidence/windows_x64_w031_result.md), [W-032](docs/evidence/windows_x64_w032_result.md), [W-036](docs/evidence/windows_x64_w036_result.md), [W-037](docs/evidence/windows_x64_w037_result.md), [W-038](docs/evidence/windows_x64_w038_result.md), and [W-039](docs/evidence/windows_x64_w039_result.md) records. |
...
*Last snapshot: 2026-08-08 - W-001/W-002/W-003/W-004/W-011/W-012/W-013/W-024/W-025, W-010/W-014 FS-2, H-001, H-002, and authoritative-host FS-4 are closed; FS-5 is conditionally closed. Nterp and the corrected pagefile-section JIT dual view are product defaults. W-010/W-014 E9, FS-1, FS-2, FS-4, FS-5, JIT-5, and H-001 retain their accepted Server 2025 results. W-038 accepts managed-exception and fatal boot-OAT walking through the repaired 187-record nterp adapter boundary; W-039 accepts 23 unwind corruptions through both OAT open modes and 23 diagnosed imageless fallbacks, while actual XMM-bearing boot-AOT restoration remains open. Fixed-page recursion remains rejected and its machinery is diagnostic-only. Remaining W-010/W-014 work is reservation correlation, negative exceptions, and debugger-quality dump-stack coverage; the AOT/OAT row above owns its separate finite queue.*
