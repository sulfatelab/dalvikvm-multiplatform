...
| Quick/JIT/TLS | **Managed and native JIT ON with the corrected dual view by default:** rSELF=r15; nterp N-1 default ON; D-1 complete (37/37 Thread sites); W-002 CLOSED after native R2 passes 21/21 records; W-003 CLOSED after native R1 passes 19/19 records with 8/8 frame attribution and 6/6 XMM sentinel; JIT smoke 12/12; JIT matrix 14/14; W-004 direct Runtime singleton load native-accepted; JIT-1 checked JIT-root/CodeInfo encodings and its post-change native W-004 regression passes 28/28; JIT-2 native mapping/policy passes 14/14; JIT-3/FS-3 lifecycle/unwind passes 9/9 with 52 collections and zero missing/stale/failed records; JIT-4 final default-J-2 regression passes 28 cases and 34/34 aggregate records with three valid dumps; compile records opt-in |
| Memory | One unnamed pagefile section is mapped as a contiguous low R/RX primary view plus a full RW alias; native 64 MiB/1 GiB, low-VA, pressure, CFG, and dynamic-policy acceptance passes; J-1 remains only as the temporary `ART_WINDOWS_X64_JIT_DUAL=0` diagnostic opt-out whose JIT-5 removal is now authorized |
...
| Threads / managed faults | **W-010/W-014 core path native-accepted:** E9 passes 30/30 on Windows Server 2025 build 26100. Windows x64 uses explicit pre-prologue stack checks; each attached thread has a verified minimum four-page stack guarantee; bounds debit the inaccessible prefix, configured guarantee, and moving guard before ART's unchanged 8192-byte reserve. Linux retains implicit probes. Zero handled dumps and five fatal static/JIT/OSR dumps pass; shared FS-3 dynamic-table sampling/churn and JIT-4's three-origin default-J-2 fatal/unwind cross-regression are also accepted. The items remain OPEN only for broader debugger, forced-policy, stack-budget, exception-unwind XMM, pending-range, and embedding coverage. |
| Linux multiplatform | Native build and L-005 imageless Hello PASS using the exact Windows x64-staged shared multipath `boot.jar` bytes |
...
- **Remaining native acceptance:** keep the accepted E9 30-record matrix as a regression gate. Add debugger continue and stack-budget/high-water measurements, dynamic-JIT rollback fault injection and method-redefinition/OSR retirement extensions, exception-unwind XMM sentinels, forced rejection of every named incompatible CET policy family, broader predecessor-UEF embedding cases, and a native exception inside the pending bridge range if practical.
...
- **Blocked on / design doc:** no blocker for core managed-fault delivery or FS-3 dynamic-table churn; debugger, forced-policy, stack-budget, exception-unwind XMM, pending-range, embedding, and optional retirement-path extensions remain; [win32_faults_and_stacks.md](win32_faults_and_stacks.md)
- **Opened:** 2026-07-16
- **Updated:** 2026-07-29 — E9 remains accepted 30/30; shared JIT-3/FS-3 churn and the JIT-4 default-J-2 three-origin fatal/unwind cross-regression pass on build 26100
...
- **Wine/Linux/native evidence (2026-07-28):** Wine passes the bounds/lifetime, direct page-state, and managed-SOE matrix. Native build 19044 validates exact small reservations and rejects fixed-page recursion. Controlled build-26100 guarantee requests prove terminal fault positions of `low+0x3000`, `+0x3000`, `+0x4000`, `+0x5000`, `+0x9000`, and `+0x11000` for requests 0, 8192, 12288, 16384, 32768, and 65536. E9's sum accounting then passes switch/nterp/JIT SOE, zero handled dumps, and the complete 30/30 runner on build 26100.
- **Required acceptance:** repeat E9 on another supported Windows 10 host; measure handler/throw high-water and debug/release stack budget; correlate Java post-`FixStackSize()` and representative ART pool reservations; repeat fiber/manual-stack rejection and external detach/continue/reattach under deep guard movement; preserve unchanged Linux `018-stack-overflow` and object-level probe behavior.
...
- **Blocked on / design doc:** no core delivery blocker; broader host repetitions and stack-budget coverage remain; [win32_faults_and_stacks.md](win32_faults_and_stacks.md) is authoritative; [win32_tls_jit_entrypoints.md](win32_tls_jit_entrypoints.md) records the managed-ABI interaction
- **Opened:** 2026-07-16
- **Updated:** 2026-07-28 — E9 guarantee-aware bounds plus explicit Windows x64 checks pass the complete native 30-record runner
...
- **State:** OPEN (P5 implementation and JIT-1 through JIT-4 native gates complete; only JIT-5 removal of the temporary J-1 opt-out and post-removal regressions remain)
- **Kind:** host-validation gap / temporary diagnostic workaround
- **Area:** art / jit / compiler
- **Symptom / why:** The corrected default now reproduces ART's Linux-visible `[data R][code RX]` contiguous primary layout with a coherent RW updater alias. JIT-1 completed the direct encoding-site checks, JIT-2 completed real-Windows mapping/backing/pressure/policy acceptance, JIT-3 completed collection/reuse with concurrent unwind sampling, and JIT-4 completed the final default-J-2 regression. Remaining W-025 work is removal of the J-1 diagnostic fallback and post-removal Windows/Wine/Linux regression. Threshold zero is no longer a JIT-memory unknown; its implementation work is tracked under W-024.
- **Current behavior:**
  ...
  - **Native JIT-2 acceptance:** Windows Server 2025 build 26100 passes nine child cases and 14 aggregate checks at 64 MiB and 1 GiB. Low-VA failure/recovery, 1 GiB `SEC_COMMIT`, standalone and ART CFG execution, and `ProhibitDynamicCode` rejection of J-2/J-1 executable operations all pass with no dump or JIT temporary file.
...
- **Implemented proper fix:** Keep ART's observable layout and post-mapping JIT logic Linux-like while containing the Windows difference in the section-allocation helper:
  ...
  6. JIT-4 now authorizes removal of the temporary `ART_WINDOWS_X64_JIT_DUAL=0` opt-out; perform the removal and post-removal Windows/Wine/Linux regressions as JIT-5.
...
- **Separate residual:** W-024 is closed and JIT-1/JIT-2/JIT-3/JIT-4 are complete. W-025 remains open only for JIT-5 J-1 diagnostic-opt-out removal plus post-removal Windows/Wine/Linux regressions. CET user shadow-stack support is not W-025 work: it is an explicit non-goal, and the process must run with HSP disabled under W-010's activation contract.
- **Code anchors:** `mem_map_windows.cc` constrained section mapping; `mem_map.cc` Windows in-place split ownership; `jit_memory_region.cc` corrected dual-view branch and common post-mapping logic; `utils.cc` cache flush; `code_generator_x86_64.cc` `PatchJitRootUse`; `oat_quick_method_header.h` `code_info_offset_`; `runtime/interpreter/mterp/x86_64ng/main.S` hard-float return adapter; `jit.cc` opt-in compile records; `tools/verify/windows_x64_w025/W025JitLifecycleStressProbe.cc`; `art-dlmalloc.cc` `USE_LOCKS=0`
- **Verified:** default corrected dual-view Hello passes with about 28–30 total successful compile records after native-JIT gate removal; JIT smoke 12/12, including default-silent compile diagnostics; JIT matrix 14/14; J-1 diagnostic Hello passes; D-1 audit complete (37/37 GS sites); threshold-zero, registered, unresolved mixed-dlsym, method-traced, and JVMTI-forced native probes pass in both memory modes; the normal/FastNative mixed/high-FP matrix compiles 7/7 targets by default and survives rebinding plus method tracing without extra target compilation; standalone section-layout probe passes coherence, execution, protection, forced low-space fragmentation, and non-64-KiB capacity cases under Wine; W-013 native R2 validates J-2 protections, metrics, pressure, and repeated starts; W-003 native R1 validates four additional threshold-zero J-2 processes with successful frame/XMM compilation and clean fatal/dump scans; JIT-1 encoder boundary/overflow tests, Windows/Linux builds, Wine JIT/unwind gates, and the Windows Server 2025 post-change W-004 regression pass 28/28 with no dump; JIT-2 native mapping/policy passes 14/14 with 1 GiB pressure, CFG, graceful error-1655 policy rejection, empty JIT temp, and no dump; JIT-3/FS-3 native lifecycle/unwind passes 9/9 with 52 collections, 1,344 compilations, 1,248 exact reuses, zero missing/stale/failed records, empty JIT temp, and no dump; JIT-4 final default-J-2 regression passes 28 cases and 34/34 aggregate records with eight lifecycle cycles, three valid fatal dumps, empty JIT temp, no trace, and no J-1 arm
...
- **Evidence:** JIT-2: `tools/verify/windows_x64_w025/RESULT-jit2-native.md`, `evidence/jit2_native/ACCEPTANCE.md`, returned SHA-256 `3d32e3a15f4e4ad8b5e98a769ea02e638abeb975755064177b1a4d3a6bf9364e`; JIT-3: `tools/verify/windows_x64_w025/RESULT-jit3-native.md`, `evidence/jit3_native/ACCEPTANCE.md`, returned SHA-256 `dcd3062a95a00296ca939062cc52fb7907405cc7c4e08ae72723a318063284fd`; JIT-4: `tools/verify/windows_x64_w025/RESULT-jit4-native.md`, `evidence/jit4_native/ACCEPTANCE.md`, issued SHA-256 `411671ab378dab9fa4c4732934deb575d7dfb5873b5ab75ffe605514afcc8cf1`, returned SHA-256 `843391f11e22225516162b25de0412d790c9ea669d0383a996e739aae8480096`
- **Opened:** 2026-07-19
- **Updated:** 2026-07-29 - JIT-4 final default-J-2 regression passes 28 cases and 34/34 aggregate records; only JIT-5 temporary J-1 diagnostic-opt-out removal and post-removal regressions remain
...
4. **W-025 JIT-5** - remove `ART_WINDOWS_X64_JIT_DUAL=0` and the Windows J-1
   single-view diagnostic branch, then run the post-removal Windows/Wine/Linux
   regressions; JIT-4 authorizes this cleanup after 28 cases and 34/34
   aggregate records pass without a J-1 arm.
5. **W-010/W-014 residual Stage E** — keep the native-accepted E9 30/30
   matrix green while completing debugger, forced-policy, pending-tail,
   exception-unwind XMM, embedding, and stack-budget acceptance; FS-3
   dynamic-table churn/sampling is complete.
...
*Last snapshot: 2026-07-29 - W-001/W-002/W-003/W-004/W-011/W-012/W-013/W-024 are closed; nterp and the corrected pagefile-section JIT dual view are product defaults; D-1, W-002 native R2, W-003 native R1, W-004, W-013 native R2, W-024, and W-025 JIT-1/JIT-2/JIT-3/JIT-4 are accepted. W-010/W-014 E9 is native-accepted 30/30 on Windows Server 2025 build 26100: explicit Windows x64 stack checks, guarantee-aware bounds, switch/nterp/JIT SOE, zero handled dumps, and five fatal static/JIT/OSR dumps all pass. The post-JIT-1 W-004 cross-regression passes 28/28 with no dump; JIT-2 mapping/policy passes 14/14; JIT-3/FS-3 dynamic-table churn passes 9/9 with 52 collections and zero missing/stale/failed records; JIT-4's default-J-2 fatal/unwind cross-regression passes 28 cases and 34/34 aggregate records with eight lifecycle cycles, three valid dumps, empty JIT temp, no trace, and no J-1 arm. Linux keeps implicit probes. Fixed-page recursion remains rejected and its machinery is diagnostic-only. W-010/W-014 remain open only for broader debugger, forced-policy, stack-budget, exception-unwind XMM, pending-range, and embedding coverage. W-025 remains open only for JIT-5 temporary J-1 opt-out removal and post-removal Windows/Wine/Linux regressions.*
