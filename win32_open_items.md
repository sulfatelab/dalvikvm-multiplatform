...
- **Proper fix / closure:** Design stages A–E, configuration checks, and the standalone mock-owner mspace create/grow/free/trim/regrow/failure lifecycle gate are complete. Close only after debug owner/lifetime death tests, dedicated product non-moving pressure, `VirtualQuery` extent/protection checks, large-heap commit tests, GCStress/ThreadHeavy/HandleLeak, JIT and Linux regression, and repeated native Windows 10 starts pass with recorded evidence. `GcProbe` alone is insufficient.
...
- **Evidence:** `tools/verify/win64_w013/RESULT.md`; external dlmalloc `f3356ce`; ART Stage A `8c900a9e4b`; ART Stage B `d011d72d56`; ART Stage C `2fa301a13b`; ART Stage D `9ea15456a2`; ART Stage E `47567cebcc`
- **Opened:** 2026-07-16
