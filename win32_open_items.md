...
- **State:** OPEN (Stages 0/C/D, static fatal boundaries, and the dynamic-JIT compiler metadata slice are implemented locally; runtime registration and native acceptance remain)
...
- **Static fatal-boundary result:** PE unwind records now cover `art_quick_invoke_stub`, `art_quick_invoke_static_stub`, and `art_quick_generic_jni_trampoline`. An emitted-record audit verifies their frame anchors, allocations, GPR saves, and current full-width XMM6-XMM11 saves. The hardened static `-Xint` JNI abort gate reaches initial VEH and UEF and creates a new valid `MDMP` dump. Recursive `RtlVirtualUnwind2` tracing also proves the remaining limitation: a threshold-zero JIT fatal path has no runtime-function entry, leaf-walks managed frame contents, and can fail exception dispatch before UEF. Every invoke/OSR native-to-managed boundary must also extend full-width preservation through XMM15 because ART managed frames preserve only 64-bit scalar XMM12-XMM15 state while Win64 requires 128 bits. `art_quick_osr_stub` still needs its own static runtime-function record for the fixed saves, RBP anchor, and variable copied-stack interval.
...
- **Dynamic-JIT implementation slice:** the x86_64 assembler now emits explicit version-1 unwind bytes with shortest legal allocation forms, descending instruction-end offsets, even-slot padding, and validation. Optimizing Win64 JIT methods reserve and force-spill RBP, then establish it after the fixed allocation. Normal/FastNative JNI stubs use the same anchor and a four-register RBX/R12-R14 scratch set; CriticalNative retains a fixed-RSP record. Generation is independent of DWARF CFI, invalid/missing enabled metadata rejects compilation before `Reserve()`, and JNI compilation carries the opaque bytes. The current runtime does not yet reserve an aligned xdata tail or call `RtlAddFunctionTable()`, so the fatal threshold-zero limitation remains.
- **Required native acceptance:** repeat the nterp/JIT NPE/SOE and recovery/GC evidence on Windows 10/current Windows; exact wrong-address cases; foreign VEH before/after/promotion; frame-based SEH for an unrecognized AV; debugger continue; stack-budget measurements; static fatal predecessor-UEF/dump behavior; OSR structural/live unwind; dynamic-JIT lookup/unwind/rollback/collection/sampling stress in J-2 and J-1; full XMM6-XMM15 normal-return and unwind sentinels; and HSP-disabled startup plus forced compatibility/audit/strict rejection.
- **Code anchors:** `vendor/art/runtime/runtime.cc`; `runtime/multiplatform/windows/sigchain_windows.cc`; `runtime_windows.cc`; `runtime/fault_handler.{h,cc}`; `runtime/arch/x86/fault_handler_x86.cc`; `runtime/arch/x86_64/quick_entrypoints_x86_64.S`; planned `runtime/multiplatform/windows/jit_unwind_windows.*`; `compiler/optimizing/code_generator_x86_64.*`; `compiler/utils/x86_64/{assembler,jni_macro_assembler}_x86_64.*`; `runtime/jit/{jit_code_cache,jit_memory_region}.*`; `tools/verify/win64_phase1/check_win32_boundary_unwind.py`; `tools/verify/win64_phase4/{src/W010ManagedFaultProbe.java,run_w010_managed_fault_probe.sh,run_crashnative.sh,RESULT-w010-managed-faults.md}`
- **Blocked on / design doc:** section 7.9 runtime registration/lifecycle implementation plus native Stage E evidence with W-014; [win32_faults_and_stacks.md](win32_faults_and_stacks.md)
- **Opened:** 2026-07-16
- **Updated:** 2026-07-27 — recovery/GC and chain probes pass; static fatal boundaries reach UEF/minidump; dynamic JIT anchors/serialization and JNI metadata carry pass focused Win64/Linux builds and ABI probes; registration/lifecycle, XMM12-XMM15 boundaries, OSR unwind, and native policy/debugger/stack-budget acceptance remain
...
4. **W-010/W-014 Stage E** — finish §7.9 xdata allocation, runtime-function registration, and lifecycle ownership; complete native XMM6-XMM15 boundary preservation; then run native policy/fault/stack acceptance.
