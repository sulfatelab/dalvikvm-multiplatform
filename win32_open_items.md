...
| Threads / managed faults | **W-014 OPEN:** Stages A-B now have exact current-stack bounds, `_beginthreadex` reservation semantics, retained pthread lifetime, Windows thread-pool sizing, measured excluded-low accounting, a verified fixed `PAGE_NOACCESS` page, and detach restoration; native A-B acceptance remains. **W-010 OPEN:** Stage 0 CET exclusion and the dormant Stage C VEH/context adapter are implemented and Wine-verified; Windows implicit faults remain disabled, so product nterp/JIT faults still do not become managed Java exceptions. Atomic activation, native HSP policy evidence, and native NPE/SOE/chain acceptance remain. |
...
- **Opened:** 2026-07-16
- **Updated:** 2026-07-27 — Stage C exact-record probe and live VEH/context probe pass under Wine (`failures=0 cases=8`; two redirected faults with promotion/removal); stack containment is enforced in the x86_64 handler; product activation and native acceptance remain
...
- **State:** OPEN (Stages A-B implemented and locally verified; native A-B acceptance and Stages C-E remain)
...
- **Current protection behavior:** every attached non-AOT Windows thread now preserves the lowest page and adjacent bottom no-access/guard regions, selects the first exact reserved or committed-private read/write page, records its original state, installs and verifies `PAGE_NOACCESS`, and publishes stack bounds above the measured prefix and protected page. `ProtectStack()`/`UnprotectStack()` perform verified transitions; install rollback and `Thread` teardown restore and verify the exact original reserved/committed state. The state record is below ART's native-code-visible TLS layout. Runtime initialization still forces implicit SO/null/suspend checks off until W-010 is complete.
- **Generated-code conflict:** x86_64 optimizing code and nterp still emit the normal unconditional `rsp - ART_STACK_OVERFLOW_GAP_x86_64` probe. Stage B makes its eventual low-stack access deterministic at ART's fixed page, and Stage C can classify that record, but runtime policy still installs no stack/null generated-code handlers because implicit checks remain disabled. A sufficiently deep generated stack therefore remains an unhandled native AV until Stage D activation. The switch interpreter continues to use explicit `Thread::stack_end_` comparisons.
...
- **Required stages:** Stage 0 build/startup CET exclusion, W-014 Stages A-B, and the dormant W-010 Stage C VEH/non-owning real-`CONTEXT` adapter are implemented locally, with focused Wine evidence. Next is (D) atomic implicit-null/SO and nterp/JIT activation; then (E) final Wine/Linux regression plus native Windows acceptance.
...
- **Opened:** 2026-07-16
- **Updated:** 2026-07-27 — Stage B behavior retained; Stage C now enforces read-only exact-page containment and has deterministic/live Wine probes; native A-B acceptance, product activation, and native W-010 acceptance remain
...
*Last snapshot: 2026-07-27 — W-001/W-002/W-003/W-004/W-011/W-012/W-013/W-024 closed; Nterp ON; corrected pagefile-section dual view is the managed/native-JIT default (12/12 smoke, 14/14 matrix); D-1 complete; W-002 native R2 passes 21/21 records with 8/8 OSR, 8/8 attach, exact deterministic thresholds/checksum, and no fatal marker or dump; W-003 native R1 passes 19/19 records with matched PE/ELF trap parity, repaired XMM6–XMM11 invoke/OSR boundaries, 8/8 attributed frame runs, 6/6 XMM sentinel runs with exact `0x3f` self-test, explicit J-2 creation, and no fatal marker or dump; W-010 Stage 0 has explicit `/CETCOMPAT:NO`, early fail-closed HSP policy enforcement, and a 19-target/20-inspected-PE audit; dormant Stage C now has exact record filtering, real `CONTEXT` forwarding, R15/recursion/page checks, promotion/removal, predecessor UEF chaining, and a passing complete Phase-4 Wine aggregate while product activation and native policy/NPE/SOE/chain evidence remain open; W-014 Stages A-B have exact stack bounds, `_beginthreadex` reservation semantics, opaque pthread lifetime/tagged external identity, measured excluded-low accounting, a verified fixed page, and detach restoration with focused Wine page/reattach gates while native A-B acceptance remains; direct CriticalNative and 7/7 mixed/high-FP normal/FastNative matrices pass unresolved dlsym, rebinding, method tracing, and JVMTI forced interpretation; W-013 native R2 passes 56/56 with complete metrics, 20/20 repeated starts, large-heap pressure, J-1/default JIT, and no dumps; Math.ceil/floor and one common ELF/PE table are restored; interpreter.cc matches upstream; compile records are opt-in; `ART_WIN64_JIT_DUAL=0` temporarily retains J-1 for diagnosis; W-025 broader real-host acceptance remains; 5 OPEN workarounds remaining.*
