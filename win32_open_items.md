...
| Quick/JIT/TLS | **Managed and native JIT ON with the corrected dual view by default:** rSELF=r15; nterp N-1 default ON; D-1 complete (37/37 Thread sites); W-002 CLOSED after native R2 passes 21/21 records; W-003 four-family attribution passes 8/8 under Wine, the XMM sentinel passes 6/6, and its checked native-host package is ready; JIT smoke 12/12; JIT matrix 14/14; W-004 direct Runtime singleton load native-accepted; compile records opt-in |
...
- **State:** OPEN (historical SETUP trap and native-boundary ABI defect repaired; Wine focused gates and native-host package complete; native execution remains)
...
- **Native-boundary repair:** `art_quick_invoke_stub`, `art_quick_invoke_static_stub`, and `art_quick_osr_stub` now reserve a Windows-only 96-byte area and preserve the lower 128 bits of XMM6–XMM11 before managed argument setup/jump. They restore those registers before returning to the ordinary Microsoft-x64 C++ caller. XMM12–XMM15 remain covered by the ART managed convention. The save area is outside the canonical ART frame, is a multiple of 16, and changes the Win64 OSR conceptual CFA from 96 to 192 while Linux remains 80.
...
- **Current evidence:** `check_w003_quick_boundaries.py` verifies all four SETUP source contracts, the three PE save/restore sequences, Linux absence of the Windows save area, and matched PE/ELF `int3` symbol/count multisets. The rebuilt objects pass with three boundaries, 212 trap-bearing functions, and 401 traps. The opt-in attributed frame probe is absent from product PE/ELF artifacts and passes 8/8 Wine processes across `-Xint`, switch, nterp, and threshold-zero JIT; nterp and JIT each report positive refs-only, refs-and-args, all-callee-saves, and save-everything counts. The dedicated XMM sentinel saves/seeds/checks every byte of XMM6–XMM11 around a twelve-double JNI callback, restores the caller state, emits its own valid PE unwind record, and passes 6/6 nterp/switch/JIT Wine processes; its intentional-clobber self-test returns exactly `0x3f`. The native-host pipeline now rebuilds with `-j32`, reruns those 14 focused processes, validates the staged PE/hash contract, passes a seven-mode package Wine smoke, restores product `art.dll`, and issues a clean ZIP with no logs, dumps, or traces. W-002/W-004 structural gates, Linux/Win64 builds, Linux imageless Hello, CoreProbe, and the four-pair Wine OSR matrix also pass. Numeric trap totals remain a build snapshot rather than hard-coded acceptance values.
- **Required work:** (1) **done:** preserve/restore XMM6–XMM11 at all three ordinary Microsoft C++→managed boundaries; (2) **done:** update Win64 OSR CFA/stack accounting and add source/PE/Linux structural checks; (3) **done on Wine:** add a focused quick-frame probe covering refs-only, refs-and-args, all-callee-saves, and everything paths under `-Xint`, switch, nterp, and JIT, with positive attributed thresholds; (4) **done on Wine:** add a native register-sentinel around JNI/quick invoke and retain the focused structural plus W-002 OSR boundary checks; native-host repetition remains under item 6; (5) **done structurally:** compare matched PE/ELF trap distributions and reject Windows-only SETUP traps; (6) **package done, native run pending:** run the issued Windows 10/11 acceptance matrix and return exactly 19 PASS records with fatal/dump scans clean; (7) keep PE quick-assembly unwind metadata explicitly owned by W-010 unless implemented.
...
- **Probe scope / W-010 dependency:** The first frame workload exposed a current nterp implicit-null failure at `nterp_op_invoke_virtual+0x3a`. A null-only workload reproduced it in the ordinary non-instrumented product, proving it is not probe corruption. The W-003 test therefore excludes implicit-null translation and retains explicit class-cast, array-store, and bounds paths. This is documented test isolation, not a product workaround; W-010 owns the fault-to-managed-exception repair.
- **Evidence / analysis:** [RESULT-w003-quick-frames-analysis.md](tools/verify/win64_phase4/RESULT-w003-quick-frames-analysis.md); [RESULT-w003-frame-probe.md](tools/verify/win64_phase4/RESULT-w003-frame-probe.md); [RESULT-w003-xmm-sentinel.md](tools/verify/win64_phase4/RESULT-w003-xmm-sentinel.md); [W003_HOST_CHECKLIST.md](tools/verify/win64_phase4/W003_HOST_CHECKLIST.md)
...
- **Opened:** 2026-07-16
- **Updated:** 2026-07-26 — XMM6–XMM11 repair, matched PE/ELF gate, 8/8 frame-family attribution, 6/6 Wine native sentinel, and checked native-host package complete; native execution remains
...
- **State:** OPEN (diagnostic VEH exists; ART managed-fault translation is incomplete)
...
- **Proper fix:** Design a Win32 VEH bridge that recognizes ART generated-code ranges and converts Windows exception records/`CONTEXT` into the same null, stack-overflow, suspend, and related managed transitions used by Linux, while keeping the platform delta inside the fault-adaptation layer. Validate nterp and JIT implicit-null paths, stack overflow, handler chaining, debugger coexistence, and native Windows minidumps before deciding whether PE quick-assembly unwind metadata is also required.
...
- **Opened:** 2026-07-16
- **Updated:** 2026-07-26 — W-003 isolation proved the current VEH does not translate nterp implicit-null faults
...
2. ~~**W-001**, **W-002**, **W-004**, **W-011**, **W-012**, and **W-024**~~ closed; **W-003** next needs the prepared 19-record native-host acceptance after completing the Microsoft XMM6–XMM11 repair, 8/8 attributed frame-family Wine gate, 6/6 Wine sentinel, and checked package.
...
- **Separate residual:** W-024 is closed. W-025's broader mapping/mitigation real-host acceptance and J-1 diagnostic-opt-out removal remain separate.
...
*Last snapshot: 2026-07-26 — W-001/W-002/W-011/W-012/W-013/W-024 closed; Nterp ON; corrected pagefile-section dual view is the managed/native-JIT default (12/12 smoke, 14/14 matrix); D-1 complete; W-002 native R2 passes 21/21 records with 8/8 OSR, 8/8 attach, exact deterministic thresholds/checksum, and no fatal marker or dump; W-003 has all four SETUP families enabled, matched PE/ELF trap parity, repaired XMM6–XMM11 invoke/OSR boundaries, 8/8 attributed four-family Wine runs, a 6/6 nterp/switch/JIT Wine sentinel with an exact `0x3f` self-test, and a checked native-host package; only its 19-record native execution remains; W-010 now explicitly records that the diagnostic VEH does not translate nterp implicit-null faults; direct CriticalNative and 7/7 mixed/high-FP normal/FastNative matrices pass unresolved dlsym, rebinding, method tracing, and JVMTI forced interpretation; W-013 native R2 passes 56/56 with complete metrics, 20/20 repeated starts, large-heap pressure, J-1/default JIT, and no dumps; Math.ceil/floor and one common ELF/PE table are restored; interpreter.cc matches upstream; compile records are opt-in; `ART_WIN64_JIT_DUAL=0` temporarily retains J-1 for diagnosis; W-025 broader real-host acceptance remains; 6 OPEN workarounds remaining.*
