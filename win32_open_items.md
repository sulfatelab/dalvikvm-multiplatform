...
| [win32_aot_oat.md](win32_aot_oat.md) | Early boot-only AOT design: Linux-style ART ELF identity with Windows 64-KiB alignment, the low-divergence private-copy baseline, conditional OAT-2 replacement candidate, specified unwind and `.oat_cfg.windows` transports, fallback work, and acceptance gates |
...
| `docs/evidence/windows_x64_*_result.md` | Historical gate evidence |
...
| AOT/OAT | Boot-only implementation is active. Windows keeps Linux ART ELF identity and `ART_PAGE_SIZE_AGNOSTIC=1`; Linux remains 16-KiB aligned while Windows uses 64-KiB `kElfSegmentAlignment` with `kMaxPageSize = 16384`. W-028 completes trivial native generation, W-029 pins one package-relative boot identity, and W-030 completes boot-only private-copy ELF/VDEX loading plus matching LZ4 cache-set staging. W-031 makes unwind step 6 partial: `.oat_unwind.windows` carries checked/deduplicated managed, JNI, and seven-trampoline metadata; validation-only parsing, executable registration, lookup, synthetic virtual unwind, and JIT-disabled managed/JNI body calls pass. W-032 completes `.oat_cfg.windows` transport: 18 semantic corruptions reject through 38 real opens, all eight metadata/relro layouts pass, and a verified guarded PE caller enters quick/JNI bodies under forced CFG without target-state API calls. W-036 narrows the Windows post-start nterp repair to switch-bridge methods, preserves published AOT entrypoints, and observes ordinary `Integer.parseInt(String)` dispatch exactly once at its current registered boot-OAT RX PC with JIT disabled. W-037 adds a coherent JIT-disabled case: nonzero 64-KiB-aligned image relocation with the same OAT delta, one low-address access violation inside registered OAT RX code from an ordinary null `Arrays.sort(int[])` call recovered as `NullPointerException`, and one non-null BSS GC root preserved through eight completed explicit GC rounds. W-038 closes the managed-exception/fatal-walk gap: an explicit managed throw retains and names its registered boot-OAT target, while a fatal native callback unwinds through the exact armed boot-OAT frame to ART's UEF and exactly one valid minidump. Its nterp boundary repair restores RBX, RBP, and R12-R15 in all 187 dynamic-gap adapter records; the enhanced structural audit and native affected-stage regressions pass. Step 10 remains partial only for concrete relocation/fault/root and other explicitly untested execution variants. OAT-1's RW/NX-to-RX transition remains default-valid observation only. The merged OAT-2 design is a replacement candidate, not a parallel product mode: W-033 must compare its placeholder/pagefile-section topology with the implemented private-copy baseline for API semantics, runtime/resource efficiency, code size, ownership complexity, and Linux semantic divergence. W-034/W-035 require an explicit proceed decision, and any final integration must remove the losing executable path. Step 2 still lacks ART-level negative mismatch diagnostics; step 9 lacks normal product selection plus successful fallback; unwind corruption/fallback injection, actual XMM-bearing boot-AOT execution, remaining step-10 breadth, and full baseline measurements remain open. Application OAT, successful unloading, outgoing quick-code CFG instrumentation, and security hardening are deferred. H-004 tracks the glibc positive-dlopen skip; H-005 tracks pre-dispatch characterization execution. See the [AOT/OAT design](win32_aot_oat.md), tracker, and accepted [W-028](docs/evidence/windows_x64_w028_result.md), [W-029](docs/evidence/windows_x64_w029_result.md), [W-030](docs/evidence/windows_x64_w030_result.md), [W-031](docs/evidence/windows_x64_w031_result.md), [W-032](docs/evidence/windows_x64_w032_result.md), [W-036](docs/evidence/windows_x64_w036_result.md), [W-037](docs/evidence/windows_x64_w037_result.md), and [W-038](docs/evidence/windows_x64_w038_result.md) records. |
...
- **W-038 boot-OAT boundary repair:** live fatal tracing through an ordinary registered boot-OAT call showed two legitimate nterp adapter frames with increasing RSP. The defect was that each adapter's synthetic allocation skipped the nterp frame without restoring its saved nonvolatile registers, so the following OAT frame received the wrong RBP. All 187 dynamic-gap adapters now describe RBX, RBP, and R12-R15 saves while keeping the existing continuation slot. Public range markers let the Windows reviewer verify exact count, allocation/save offsets, zero prologues, and contiguous ranges. The rebuilt native W-010 stage passes 8/8, and W-038 reaches the exact armed boot-OAT frame, ART's UEF, and one valid minidump on build 26100.
...
- **Opened:** 2026-07-16
- **Updated:** 2026-08-07 — W-038 repairs and audits all 187 nterp compiled-invoke adapter records; native W-010 structural 8/8 and the fatal boot-OAT UEF/minidump path pass on build 26100
...
- **Exit criteria:** Met for the available native host; the sanitized [H-001 result](docs/evidence/windows_x64_h001_phase4_result.md) records `OVERALL PASS`. The authoritative-host FS-4 repeat is also accepted; the separate Win10/second-host repetition is skipped by policy.
- **Opened:** 2026-07-16
...
- **Current evidence:** `docs/evidence/windows_x64_fs4_same_host_result.md`
...
| PE32+ OAT | Rejected; Windows boot OAT keeps Linux ART's current ELF64 identity/layout and uses an ART-owned private-copy mapping path ([win32_aot_oat.md](win32_aot_oat.md)) |
...
- **Code anchors:** `native/CMakeLists.txt`; `overlay/art_port_policy.py`; historical result `docs/evidence/windows_x64_libcore_icu_result.md`
...
- **Code anchors:** `overlay/art_port_policy.py`; historical result `docs/evidence/windows_x64_libcore_icu_result.md`
- **Opened:** 2026-07-16
...
- **Evidence:** `docs/evidence/windows_x64_w024_interpreter_jni_result.md`
...
- **Evidence:** `docs/evidence/windows_x64_w024_interpreter_jni_result.md`, `tests/cases/jni-critical-native/RESULT.md`, `tests/cases/jni-native-abi/RESULT.md`
...
- **Evidence:** `docs/evidence/windows_x64_w013_result.md`; returned archive SHA-256 `456e297d70c2f166308c869812ddec262fa38bc6dcd2852ea56edd5b2205078e`; external dlmalloc `f3356ce`; ART `8c900a9e4b`, `d011d72d56`, `2fa301a13b`, `9ea15456a2`, `6253d01afc`, `47567cebcc`, `1509b1f95e`, `27a1ac74a4`; root `c943f1f`, `caad337`; libcore `67ec4ab8dd70`
- **Opened:** 2026-07-16
...
- **Code anchors:**
  ...
  - `tests/cases/jvmti-force/` plus the unified managed runner and `docs/evidence/windows_x64_phase4_jvmti_force_result.md`
  ...
  - `docs/evidence/windows_x64_w024_interpreter_jni_result.md` (accepted Wine and native-Windows tripwire reachability audit and package identity)
...
- **Closed by:** ART `42a03f2ea0`; native Windows acceptance in `docs/evidence/windows_x64_w024_interpreter_jni_result.md`; final historical regressions on 2026-07-24; unified Linux/Windows W-004 acceptance on 2026-08-01
...
- **Evidence:** JIT-2 through JIT-5: `docs/evidence/windows_x64_w025_jit2_result.md`, `docs/evidence/windows_x64_w025_jit3_result.md`, `docs/evidence/windows_x64_w025_jit4_result.md`, and `docs/evidence/windows_x64_w025_jit5_result.md`; JIT-5 issued SHA-256 `7b35eab8001ee2ba4881985b63d8df6921a954e023f8e70289f964499f57cd32`, returned SHA-256 `2bddf51924a7ca6b9719ffde433e859007465babf6bf2ca7a12f417eecd6289f`
- **Opened:** 2026-07-19
...
- **Progress / residual (not exit blockers):**
  ...
  - Historical details: `docs/evidence/windows_x64_libcore_icu_result.md`
...
- **Code anchors:** historical `docs/evidence/windows_x64_libcore_icu_result.md`; `tests/CMakeLists.txt`; `tests/support/generate_conscrypt_constants.py`; `compat/java-resources/java/security/security.properties`; `native/runtime-assets/etc/security/cacerts`; `tools/build_art.py`
- **Opened:** 2026-07-17
- **Closed:** 2026-07-17
...
- **Fix / evidence:**
  ...
  - Historical absolute archive paths remain only inside documents under `docs/evidence/`; they are not product inputs.
- **Opened:** 2026-07-17
- **Closed:** 2026-07-17
