...
| Threads / managed faults | **W-010/W-014 core path and FS-1 native-accepted:** E9 passes 30/30 and FS-1 passes Release/Debug switch, nterp, and JIT on Windows Server 2025 build 26100. Windows x64 uses explicit pre-prologue stack checks; each attached thread has a verified minimum four-page stack guarantee; bounds debit the inaccessible prefix, configured guarantee, and moving guard before ART's 8192-byte product reserve. Debug Windows x86_64 uses a measured 40-KiB recovery reserve; product and non-Windows remain at 8192 bytes. FS-1 records at least 6784 Release and 37168 Debug bytes of native margin with no handled dump. Linux retains implicit probes. Shared FS-3 dynamic-table sampling/churn and JIT-5's three-origin fatal/unwind cross-regression are also accepted. The items remain OPEN only for broader debugger, forced-policy, exception-unwind XMM, pending-range, embedding, reservation correlation, and second-host coverage. |
...
- **Remaining native acceptance:** keep the accepted E9 and FS-1 matrices as regression gates. Add debugger continue, dynamic-JIT rollback fault injection and method-redefinition/OSR retirement extensions, exception-unwind XMM sentinels, forced rejection of every named incompatible CET policy family, broader predecessor-UEF embedding cases, Java/ART-pool reservation correlation, a second supported host, and a native exception inside the pending bridge range if practical.
...
- **Blocked on / design doc:** no blocker for core managed-fault delivery, FS-1 stack budget, or FS-3 dynamic-table churn; debugger, forced-policy, exception-unwind XMM, pending-range, embedding, reservation correlation, second-host, and optional retirement-path extensions remain; [win32_faults_and_stacks.md](win32_faults_and_stacks.md)
- **Opened:** 2026-07-16
...
- **Required acceptance:** repeat E9 and FS-1 on another supported Windows 10 host; correlate Java post-`FixStackSize()` and representative ART pool reservations; repeat fiber/manual-stack rejection and external detach/continue/reattach under deep guard movement; preserve unchanged Linux `018-stack-overflow` and object-level probe behavior.
