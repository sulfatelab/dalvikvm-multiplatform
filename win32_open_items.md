...
| Threads / managed faults | **W-014 OPEN:** Stage A now has exact current-stack bounds, `_beginthreadex` reservation semantics, retained pthread lifetime, and Windows thread-pool sizing; native Stage A acceptance plus the fixed ART page remain. **W-010 OPEN:** Stage 0 explicitly marks every project PE `/CETCOMPAT:NO` and rejects active/nonzero HSP policy before memory/thread/JIT startup, while the VEH remains diagnostic-only, so nterp/JIT implicit faults still do not become managed Java exceptions. Native HSP compatibility/audit/strict rejection evidence remains pending. |
...
- **Wine/Linux evidence (2026-07-26):** the permanent `win32_thread_stack_probe` passes exact main/default, 1 MiB, and 2 MiB bounds; stable self/ID extraction; callback results; current-only inspection; detach before and after exit; attribute-detached operation; 512 join and 128 detach iterations; raw `CreateThread`; invalid/custom attribute rejection; and fiber rejection. Wine reports exact 1 MiB default/explicit and 2 MiB reservations, but does not provide usable `GetProcessHandleCount()` evidence and previously rounded a requested 256 KiB reservation to 1 MiB. Win64 ART/`dalvikvm`, Hello, ThreadHeavy, all W-002 attach modes, and the full Phase-4 suite pass; the Linux full rebuild and `dalvikvm -showversion` also pass.
- **Required stages:** Stage 0 build/startup CET exclusion and Stage A bounds/thread-creator/pthread-lifetime work are implemented locally, with their native acceptance pending. Next is (B) measured excluded-low selection, dormant fixed-page state, and detach restoration; then (C) the dormant W-010 VEH/non-owning real-`CONTEXT` adapter; (D) atomic implicit-null/SO and nterp/JIT activation; and (E) final Wine/Linux regression plus native Windows acceptance.
...
- **Opened:** 2026-07-16
- **Updated:** 2026-07-26 — Stage A exact bounds, `_beginthreadex`, pthread lifetime/identity, thread-pool sizing, focused Wine gate, Linux regression, and diagnostic VEH/UEF teardown implemented; native acceptance remains and Stage B is next
