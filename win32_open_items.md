...
| Threads / managed faults | **W-014 OPEN:** native build 19044 accepts exact 64 KiB through 9 MiB reservations, pthread lifetime, direct fixed-page selection/restoration, and related bounds checks, but switch/nterp/JIT recursion reaches native `STATUS_STACK_OVERFLOW` before ART's fixed page; the managed-SOE mechanism is reopened. **W-010 OPEN:** native CET classification, NPE translation, sigchain/frame-SEH, split OSR live unwind, and all six XMM6-XMM15 sentinel runs pass. All five fatal AV origins reach VEH but miss UEF/minidump. Isolated stack-growth, standalone UEF, and late ART UEF-ownership probes are packaged for the next native run. |
...
- **Diagnostic stage:** `win32_stack_growth_probe.exe` records recursive guard/page state and re-protection before/after `_resetstkoflw`; `win32_uef_probe.exe` isolates frame-SEH, unhandled, chained, and worker-thread UEF dispatch; `CrashNativeProbe uef` installs a late filter and identifies/chains its predecessor. `RUN_W010_W014_DIAGNOSTICS.ps1` keeps these observations separate from the unchanged 30-record acceptance runner. Wine verifies all safe modes and the late ART predecessor/dump chain; protected recursive growth is native-only because Wine itself segfaults in that case.
...
- **Required native acceptance:** first resolve managed SOE delivery and fatal UEF ownership/dispatch using the diagnostic package. Then repeat the repaired SOE recovery matrix and fatal static/JIT/OSR paths, add debugger continue and stack-budget measurements, dynamic-JIT rollback/collection/sampling stress in J-2 and J-1, exception-unwind XMM sentinels, current-Windows repetition, and forced rejection of every named incompatible CET policy family. Native NPE, foreign VEH/frame-SEH, OSR live unwind, HSP-disabled startup, and normal-return XMM sentinels already pass on build 19044.
...
- **Blocked on / design doc:** native stack-growth and UEF diagnostic return, then the repaired Stage E fatal/SOE matrix and broader dynamic-table sampling/churn acceptance; [win32_faults_and_stacks.md](win32_faults_and_stacks.md)
- **Opened:** 2026-07-16
- **Updated:** 2026-07-27 — second native run passes CET, NPE, sigchain/frame-SEH, OSR unwind, reservations, direct page restore, and XMM6-XMM15, but exposes native moving-guard SOE failure and a separate all-origin fatal UEF failure; isolated native diagnostics are packaged
...
- **State:** OPEN (native bounds/reservation/page-state subsets pass; recursive managed SOE fails on build 19044 and the fixed-page delivery design is reopened)
...
- **Current protection behavior:** every attached non-AOT Windows thread preserves the lowest page and adjacent bottom no-access/guard regions, selects the first exact reserved or committed-private read/write page, records its original state, installs and verifies `PAGE_NOACCESS`, and publishes stack bounds above the measured prefix and protected page. Direct protect/unprotect/restore passes on native Windows, including a reserved original page. Recursive validity does not: native guard growth reaches `STATUS_STACK_OVERFLOW` before the fixed-page AV, and switch recovery observes unexpected protection when re-protecting the temporarily writable page.
...
- **Former fixed-page contract, now reopened:** the implemented candidate preserves the bottom page/guard prefix, selects a reserved or committed-private page, commits if needed, and protects exactly one page while retaining the 8 KiB x86_64 probe gap. This remains sound page-state infrastructure and keeps Linux divergence low, but the second native run proves it is not a complete recursive SOE mechanism. Do not adopt `PAGE_GUARD`, `SetThreadStackGuarantee`, explicit compiler checks, or native `EXCEPTION_STACK_OVERFLOW` translation as a product fix until the isolated protected/writable growth probe identifies the real Windows transition.
...
- **Wine/Linux/native evidence (2026-07-27):** Wine passes the full bounds/lifetime matrix, direct page-state stress, and managed SOE recovery, while clamping sub-default requests. Native build 19044 returns exact 64 KiB and 256 KiB reservations, stable handle counts, and a reserved bottom layout with the ART page at `low + 4096`; direct page protection/restoration passes 64 cycles. Native recursive switch/nterp/JIT instead terminate with `STATUS_STACK_OVERFLOW`. The new growth probe records baseline/protected/writable/direct cases, page and guard state, first terminal code/RSP/fault, re-protection before and after `_resetstkoflw`, and exact restoration. Wine validates every safe mode but crashes in protected recursive growth, so that result must come from real Windows.
- **Required stages:** bounds, creation, lifetime, page selection, and direct restoration are implemented and partly native-accepted. Managed SOE activation is not accepted and must be redesigned or repaired after the diagnostic return.
- **Required acceptance:** protected/writable recursive guard interaction; final switch/nterp/JIT SOE delivery and post-overflow recovery; VEH/pre-unprotect stack high-water marks; Java post-`FixStackSize()` and representative ART pool reservations; fiber/manual-stack rejection; external detach/continue/reattach under deep guard movement; current Windows repetition; unchanged Linux `018-stack-overflow` behavior.
...
- **Blocked on / design doc:** native protected/writable growth diagnostics and the coupled W-010 delivery redesign; [win32_faults_and_stacks.md](win32_faults_and_stacks.md) is authoritative; [win32_tls_jit_entrypoints.md](win32_tls_jit_entrypoints.md) records the managed-ABI interaction
- **Opened:** 2026-07-16
- **Updated:** 2026-07-27 — native reservations and direct page state pass, but recursive switch/nterp/JIT SOE loses to Windows moving-guard `STATUS_STACK_OVERFLOW`; isolated native growth diagnostics are packaged and the fixed-page delivery contract is reopened
...
*Last snapshot: 2026-07-27 — W-001/W-002/W-003/W-004/W-011/W-012/W-013/W-024 are closed; nterp and the corrected pagefile-section JIT dual view are product defaults; D-1, W-002 native R2, W-003 native R1, W-004, W-013 native R2, and W-024 are accepted. W-010/W-014 local Wine coverage remains green, including dynamic PE unwind registration/lifecycle, static and OSR boundaries, full XMM6-XMM15 preservation, NPE/SOE recovery, and fatal dumps. The second Windows 10 build-19044 W-010/W-014 run passes 20 checks: CET `0x100` classification, exact requested reservations, stable pthread handles, direct reserved-page restoration, fault records, sigchain/frame-SEH, nterp/JIT NPE, OSR live unwind, six XMM sentinel runs, and no-chain rejection. It fails 12 checks in two groups: switch/nterp/JIT recursion reaches native `STATUS_STACK_OVERFLOW` before ART's fixed page, and static/JIT/OSR fatal AVs reach VEH but not UEF/minidump. The fixed-page SOE contract is reopened; isolated native stack-growth, standalone UEF, and late ART UEF-ownership diagnostics are packaged before any product fix. W-025 broader real-host acceptance and five open workarounds remain.*
