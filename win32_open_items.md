...
- **Required native acceptance:** replace the managed-SOE delivery adapter so it does not depend on retaining a fixed no-access page inside the Windows stack, then pass repaired switch/nterp/JIT SOE and the complete 30-record runner. Keep the now-accepted five-origin fatal subset as a regression gate. Also add debugger continue and stack-budget measurements, dynamic-JIT rollback/collection/sampling stress in J-2 and J-1, exception-unwind XMM sentinels, forced rejection of every named incompatible CET policy family, and a native exception inside the pending bridge range if practical. Native NPE, foreign VEH/frame-SEH, standalone UEF, OSR live unwind, HSP-disabled startup, normal-return XMM sentinels, and all static/JIT/OSR fatal origins already pass.
...
- **Blocked on / design doc:** a replacement Windows SOE delivery design, then the repaired SOE matrix and broader dynamic-table sampling/churn acceptance; [win32_faults_and_stacks.md](win32_faults_and_stacks.md)
- **Opened:** 2026-07-16
- **Updated:** 2026-07-28 — full native E6 accepts all five fatal origins and isolates the remaining runner failures to managed SOE
...
4. **W-010/W-014 Stage E** — design a replacement Windows SOE delivery mechanism after native evidence invalidated the fixed page; then pass repaired SOE and the complete host runner, followed by dynamic-table churn/sampling, debugger, policy, pending-tail, and stack-budget acceptance. The five-origin fatal matrix is already native-accepted.
...
*Last snapshot: 2026-07-28 — W-001/W-002/W-003/W-004/W-011/W-012/W-013/W-024 are closed; nterp and the corrected pagefile-section JIT dual view are product defaults; D-1, W-002 native R2, W-003 native R1, W-004, W-013 native R2, and W-024 are accepted. W-010/W-014 local Wine coverage remains green, and native builds accept CET classification, reservations/lifetime, direct page restore, fault records, sigchain/frame-SEH, nterp/JIT NPE, OSR live unwind, XMM6-XMM15 normal return, and all five static/JIT/OSR fatal origins. Runs 3-4 and current Windows invalidate fixed-page recursive SOE delivery and rule out UEF replacement. The complete E6 host runner is 25/30, with only managed SOE and its aggregate checks red. A replacement SOE delivery design is next. W-025 broader real-host acceptance and five open workarounds remain.*
