...
| Threads / managed faults | **W-014 OPEN:** Stages A-B now have exact current-stack bounds, `_beginthreadex` reservation semantics, retained pthread lifetime, Windows thread-pool sizing, measured excluded-low accounting, a verified fixed `PAGE_NOACCESS` page, and detach restoration; native A-B/E acceptance remains. **W-010 OPEN:** Stages 0, C, and D are implemented and Wine-verified. Product Win64 now translates recognized nterp/JIT implicit null and stack-overflow faults through common ART handlers; native HSP policy, handler-chain/debugger, stack-budget, and NPE/SOE acceptance remain. |
...
### W-010 — Windows managed-fault adapter and CET exclusion
- **State:** OPEN (Stages 0, C, and D implemented locally; native acceptance remains)
...
- **Local Stage D evidence:** the focused Wine gate passes started-runtime `-Xno-sig-chain` rejection; 64 caught read NPEs plus 64 caught write NPEs in nterp and threshold-zero JIT; two caught SOEs on the main thread plus two on a newly created child thread in both modes; required JIT compilation records; no managed-fault diagnostic VEH/UEF output; and no dump-state change. Unmanaged native AV still reaches fatal diagnostics, JIT smoke remains 12/12, the complete Phase-4 aggregate passes, and Linux rebuild/showversion/shared-boot Hello pass.
- **Required native acceptance:** repeat the nterp/JIT NPE/SOE evidence on Windows 10/current Windows; exact wrong-address cases; foreign VEH before/after/promotion; frame-based SEH for an unrecognized AV; debugger continue; stack-budget measurements; fatal predecessor-UEF/dump behavior; and HSP-disabled startup plus forced compatibility/audit/strict rejection.
- **Code anchors:** `vendor/art/runtime/runtime.cc`; `runtime/multiplatform/windows/sigchain_windows.cc`; `runtime_windows.cc`; `runtime/fault_handler.{h,cc}`; `runtime/arch/x86/fault_handler_x86.cc`; `tools/verify/win64_phase4/{src/W010ManagedFaultProbe.java,run_w010_managed_fault_probe.sh,RESULT-w010-managed-faults.md}`
- **Blocked on / design doc:** native Stage E evidence with W-014; [win32_faults_and_stacks.md](win32_faults_and_stacks.md)
- **Opened:** 2026-07-16
- **Updated:** 2026-07-27 — Stage D product activation and focused nterp/JIT NPE/SOE Wine stress pass; native policy/chain/debugger/stack-budget acceptance remains
...
- **Wine/Linux evidence (2026-07-26):** the permanent `win32_thread_stack_probe` passes exact main/default, 1 MiB, and 2 MiB bounds; stable self/ID extraction; callback results; current-only inspection; detach before and after exit; attribute-detached operation; 512 join and 128 detach iterations; raw `CreateThread`; invalid/custom attribute rejection; and fiber rejection. The Stage B probe passes eight deterministic selectors, 64 committed-page restore cycles on Wine's main stack, 64 real reserved-page commit/decommit cycles, a 2 MiB pthread stack, and 258 exact assembly-load faults. Wine reports `excluded=8192`, original `MEM_COMMIT/PAGE_READWRITE` on both real stacks. The W-002 gate now makes every raw thread attach/callback/detach, consume about 16 KiB of native stack, reattach/callback/detach, and passes all eight mode/repeat processes. Win64 `dalvikvm -showversion`, default JIT Hello, ThreadHeavy, and the complete Phase-4 suite pass with fixed pages installed. The Linux full rebuild, `dalvikvm -showversion`, and shared-boot imageless Hello pass. The CET audit now covers 19 PE link targets and 20 inspected PE files. Wine does not close native handle-count, guard-growth, small-reservation, or reserved-stack-layout evidence.
...
- **Opened:** 2026-07-16
- **Updated:** 2026-07-27 — Stage D translates repeated main/child nterp and JIT protected-page faults; native A-B/E and W-010 chain/policy acceptance remain
...
- **Unwind and W-010 scope:** Win64 quick assembly still emits no `.pdata`/`.xdata`; ART managed stack walking is separate. W-010 owns the exact VEH/non-owning-`CONTEXT` managed-fault adapter, cooperative VEH/SEH chaining, and the independent nterp implicit-null fault formerly observed at `nterp_op_invoke_virtual+0x3a`; Stage D now translates that product path in the dedicated W-010 gate. PE unwind metadata remains separate diagnostics hardening unless native W-010 testing proves it is required for correctness. The W-003 probe still excludes only that implicit-null case and retains class-cast, array-store, and bounds paths; no W-003 product fallback was added.
