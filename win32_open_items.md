...
- **Symptom / why:** Official AOSP libcore marks many natives `@CriticalNative` or `@FastNative` (Math/StrictMath were **@FastNative → @CriticalNative** in AOSP; see libcore `d021f1d8475c`). The two concrete compiler/stub ABI defects found in this stage are fixed, but Win64 still has product workarounds and incomplete coverage:
  ...
  5. Broader registration/unregistration, instrumentation, and entrypoint state transitions remain before the native-JIT gate becomes the product default.
  6. Interpreter JNI historically lacked full CriticalNative shorty coverage (partially papered by **W-019** for Math `DD`/`DDD`/…). This remains a fallback/workaround rather than proof of quick/direct parity.
  7. Product workaround: **Math.ceil / Math.floor** are pure Java (`ART-WinNT` comments in `Math.java`; natives unregistered in `Math.c`) even though AOSP exposes them as `@CriticalNative` natives.
  8. Product workaround: Win `Math.c` uses a PE-specific registration table (`gMethodsWin` CriticalNative-shaped pointers) versus Linux `FAST_NATIVE_METHOD`; retain it only until PE trampolines and registrations match AOSP.
- **Current behavior:**
  ...
  - The compiled-JNI convention split and XMM-to-XMM argument moves are implemented. The focused normal/FastNative matrix passes with 7/7 distinct JNI thunk targets compiled and exact mixed/high-FP values.
  - JIT compilation of all native methods is disabled by default. `ART_WIN64_JIT_NATIVE=1` remains an opt-in diagnostic override pending state-transition coverage, product demotion restoration, diagnostic cleanup, and real-Windows acceptance; mixed/high-FP ABI coverage is no longer the blocker.
...
- **Compiled-JNI / FastNative research (2026-07-24):**
  ...
  8. Unresolved CriticalNative mixed signatures remain covered separately. Remaining W-024 native-JIT work is state transitions and product/host cleanup, not the mixed/high-FP calling convention.
- **Proper fix:**
  ...
  4. Complete registration/unregistration, instrumentation, and entrypoint state-transition variants, then remove `ART_WIN64_JIT_NATIVE` after real-Windows acceptance and product cleanup.
  5. **Landed this stage:** add a Win64 branch to `CriticalNativeCallingConventionVisitorX86_64` using unified four-slot Microsoft x64 registers, a 32-byte shadow area, and stack arguments after it.
  6. **Landed this stage:** initialize the visitor stack offset with the shadow area so spilled arguments cannot overlap the home area.
  7. **Landed this stage:** preserve the unresolved-stub caller PC across `LOAD_RUNTIME_INSTANCE` by reloading it from the existing saved return-PC slot on Windows.
  8. **Landed this stage:** add direct-call tests for unresolved `()J`, registered FP-only/mixed/spilled signatures, and unresolved exported mixed-signature dlsym calls. Remaining direct-call test work is real Windows.
  9. Restore **every** multipath demotion of methods that were originally `@CriticalNative` / `@FastNative` in AOSP (starting with **Math.ceil / Math.floor** → native + `@CriticalNative` again).
  10. Re-register matching natives in `Math.c` (and any other demoted tables) on **both** PE and ELF with AOSP-correct CriticalNative/FastNative ABIs.
  11. Prefer real ART CriticalNative/FastNative trampolines over InterpreterJni shorty expansion; then **trim** PE-only shorty explosion (**W-011**) and dual `gMethodsWin` paths where redundant.
  12. Audit libcore for other `ART-WinNT` pure-Java / ABI demotions of Critical/Fast natives (not just Math) and revert once entrypoints are product-default.
...
- **Remaining exit criteria:**
  - Registration/unregistration and instrumentation/entrypoint state-transition coverage passes, and the `ART_WIN64_JIT_NATIVE` gate is removed.
...
- **Opened:** 2026-07-17
- **Updated:** 2026-07-24 — direct CriticalNative and mixed/high-FP normal/FastNative ABI coverage pass, including unresolved dlsym and deep spills; W-024 remains open for state transitions, product demotions, gate removal, and real Windows
...
- **Current behavior:**
  ...
  - **Threshold-zero stress:** resolved outside memory topology. The direct `@CriticalNative` path now has Win64 shadow/unified-argument handling and preserves its caller PC across the PE `LOAD_RUNTIME_INSTANCE` `r11` scratch. Repeated J-1 and dual-view acceptance passes; remaining W-024 scope is state-transition/product/host work.
  - Native gate: all native methods are excluded from JIT by default. The diagnostic `ART_WIN64_JIT_NATIVE=1` override passes the 7/7 mixed/high-FP normal/FastNative matrix; it remains opt-in pending W-024 state transitions and product/host cleanup.
...
- **Separate residual:** Complete the remaining W-024 registration/unregistration and instrumentation/entrypoint state-transition matrix before removing the native-JIT gate.
...
*Last snapshot: 2026-07-24 — W-001 closed; nterp ON; corrected pagefile-section dual view is the managed-JIT default (10/10 smoke, 14/14 matrix); D-1 complete; direct CriticalNative and 7/7 mixed/high-FP normal/FastNative matrices pass, including unresolved dlsym; `ART_WIN64_JIT_DUAL=0` temporarily retains J-1 for diagnosis; real-Windows acceptance, state transitions, product demotions, and gate cleanup remain; 12 OPEN workarounds remaining.*
