...
| AOT/OAT | Boot-only implementation is active. Windows keeps Linux ART ELF identity and `ART_PAGE_SIZE_AGNOSTIC=1`; Linux remains 16-KiB aligned while Windows uses 64-KiB `kElfSegmentAlignment` with `kMaxPageSize = 16384`. W-028 completes trivial native generation, W-029 pins one package-relative boot identity, and W-030 completes boot-only private-copy ELF/VDEX loading plus matching LZ4 cache-set staging. W-031 makes unwind step 6 partial: `.oat_unwind.windows` now carries checked/deduplicated managed, JNI, and seven-trampoline metadata; validation-only parsing, executable registration, lookup, synthetic virtual unwind, and JIT-disabled managed/JNI body calls pass. It also makes execution step 10 partial, but not complete: the probe obtains underlying code with `GetOatMethodQuickCode()` because startup upgrades many current entrypoints to nterp, so representative ordinary dispatch inside boot-OAT RX ranges is not proven. Remaining unwind work is corruption/fallback injection, exception/fatal stack walking, and stronger XMM-bearing AOT-frame execution. W-032 is now designed as `.oat_cfg.windows` emission/validation plus forced-CFG observation through a verified guarded PE caller; it does not call `SetProcessValidCallTargets` or instrument outgoing quick-code branches. OAT-1's documented RW/NX-to-RX transition is default-valid, so explicit targets remain a separate allocation/OAT-2 feasibility question. Step 2 still lacks ART-level negative mismatch diagnostics; step 9 lacks normal product selection plus successful fallback; CFG implementation and full OAT-1 measurements remain open. Application OAT, OAT-2, unloading, explicit CFG enforcement, and security hardening are deferred. H-004 tracks the glibc positive-dlopen skip; H-005 tracks pre-dispatch characterization execution. See the tracker and accepted [W-028](docs/history/windows_x64_w028_result.md), [W-029](docs/history/windows_x64_w029_result.md), [W-030](docs/history/windows_x64_w030_result.md), and [W-031](docs/history/windows_x64_w031_result.md) records. |
