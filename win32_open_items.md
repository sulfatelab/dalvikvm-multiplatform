...
- **Opened:** 2026-07-16
- **Updated:** 2026-07-29 — E9 remains accepted 30/30 and the shared JIT-3/FS-3 native dynamic-table churn gate passes 9/9 on build 26100
...
- **State:** OPEN (P5 implementation plus JIT-1 encoding, JIT-2 mapping/policy, and JIT-3 lifecycle/unwind native gates complete; final regression and the temporary J-1 opt-out remain)
- **Kind:** host-validation gap / temporary diagnostic workaround
- **Area:** art / jit / compiler
- **Symptom / why:** The corrected default now reproduces ART's Linux-visible `[data R][code RX]` contiguous primary layout with a coherent RW updater alias. JIT-1 completed the direct encoding-site checks, JIT-2 completed real-Windows mapping/backing/pressure/policy acceptance, and JIT-3 completed collection/reuse with concurrent unwind sampling. Remaining W-025 work is the final default-path regression followed by removal of the J-1 diagnostic fallback. Threshold zero is no longer a JIT-memory unknown; its implementation work is tracked under W-024.
- **Current behavior:**
...
- **Implemented proper fix:** Keep ART's observable layout and post-mapping JIT logic Linux-like while containing the Windows difference in the section-allocation helper:
  ...
  6. Remove the temporary `ART_WINDOWS_X64_JIT_DUAL=0` opt-out only after the pending JIT-4 final default-build native archive passes; run the removal and post-removal regressions as JIT-5.
...
- **Separate residual:** W-024 is closed and JIT-1/JIT-2/JIT-3 are complete. W-025 still requires the JIT-4 final default-path regression and JIT-5 J-1 diagnostic-opt-out removal plus post-removal regressions. CET user shadow-stack support is not W-025 work: it is an explicit non-goal, and the process must run with HSP disabled under W-010's activation contract.
...
- **Opened:** 2026-07-19
- **Updated:** 2026-07-29 - JIT-3/FS-3 native lifecycle/unwind acceptance passes 9/9; JIT-4 final regression and JIT-5 temporary J-1 diagnostic-opt-out removal remain
...
4. **W-025 JIT-4** - run the final default-build native regression archive
   across smoke/matrix, JIT-disabled, managed/native/OSR, and fatal gates;
   JIT-3/FS-3 lifecycle/unwind is native-accepted 9/9.
...
*Last snapshot: 2026-07-29 - W-001/W-002/W-003/W-004/W-011/W-012/W-013/W-024 are closed; nterp and the corrected pagefile-section JIT dual view are product defaults; D-1, W-002 native R2, W-003 native R1, W-004, W-013 native R2, W-024, and W-025 JIT-1/JIT-2/JIT-3 are accepted. W-010/W-014 E9 is native-accepted 30/30 on Windows Server 2025 build 26100: explicit Windows x64 stack checks, guarantee-aware bounds, switch/nterp/JIT SOE, zero handled dumps, and five fatal static/JIT/OSR dumps all pass. The post-JIT-1 W-004 cross-regression passes 28/28 with no dump; JIT-2 mapping/policy passes 14/14; JIT-3/FS-3 dynamic-table churn passes 9/9 with 52 collections, zero missing/stale/failed records, and no dump or JIT temp. Linux keeps implicit probes. Fixed-page recursion remains rejected and its machinery is diagnostic-only. W-010/W-014 remain open only for broader debugger, forced-policy, stack-budget, exception-unwind XMM, pending-range, and embedding coverage. W-025 JIT-4 final regression and JIT-5 temporary J-1 opt-out removal remain.*
