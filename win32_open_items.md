...
| Threads / managed faults | **W-014 OPEN:** native build 19044 accepts exact 64 KiB through 9 MiB reservations, pthread lifetime, direct fixed-page selection/restoration, and related bounds checks. Recursive growth commits/reprotects the fixed page as `PAGE_READWRITE` before `STATUS_STACK_OVERFLOW`, invalidating it as the native SOE mechanism. **W-010 OPEN:** native CET classification, NPE translation, sigchain/frame-SEH, split OSR live unwind, and all six XMM6-XMM15 sentinel runs pass. Fatal JNI AV reaches VEH while ART still owns the UEF slot, but no top-level UEF runs. A realistic GenericJNI unwind found and repaired RDI's save offset; native JNI-raised/JNI-hardware/native-worker isolation is next. |
...
- **State:** OPEN (native NPE/OSR/XMM/sigchain subsets pass; managed SOE and fatal UEF dispatch fail on Windows build 19044)
...
- **Current CET state:** Stage 0 is implemented. Generator policy applies `LINKER:/CETCOMPAT:NO` to every generated non-static target; nine handwritten Win64 CMake harnesses and three direct Clang/lld links use the same explicit option. `Runtime::Init()` queries `ProcessUserShadowStackPolicy` after logger selection and before `MemMap::Init()`, ART threads, nterp, or JIT. It constructs an incompatibility mask from named SDK fields, accepts `CetDynamicApisOutOfProcOnly` and reserved fields, accepts `ERROR_INVALID_PARAMETER` only below Windows build 19041, and fails closed on unexpected query/version failures. The probe tests every named incompatible field, safe/reserved fields, and mixtures. The structural/package gate now reports 27 PE link targets and inspects 58 files with the focused package, including external LLVM `c++.dll`, all without the CET-compatible marker. The second Windows build-19044 run accepts raw `flags=0x00000100` as `CetDynamicApisOutOfProcOnly` with `known_incompatible=0x00000000`. Forced named-incompatible policy rejection remains the Stage 0 blocker.
...
- **E4 live unwind diagnostic:** `ART_WIN64_FATAL_UNWIND_TRACE=1` now enables a bounded 32-frame walk from a copy of the live VEH `CONTEXT` only in the three late-UEF diagnostic children. It records module base/path/RVA, runtime-function RVAs, virtual/leaf steps, bounds, progress, and a terminal reason without changing dispatch. A direct Wine smoke crosses the native method, repaired GenericJNI, invoke stub, and ordinary ART frames, then finds the first live lookup miss at `ExecuteSwitchImplAsm + 0x9` (`pop %rbx`). That wrapper pushed RBX but has no PE record, so leaf fallback consumes saved RBX as the return PC. The complete `-j32` package/Wine preflight passes twice, including all three traced modes and 14-15 valid fatal dumps before clean regeneration. Native E4 must confirm the boundary. A confirmed repair should add both Win64 unwind metadata and the mandatory 32-byte MSVC outgoing home area while leaving Linux/SysV unchanged.
...
- **Required native acceptance:** return E4 and confirm whether `ExecuteSwitchImplAsm + 0x9` is the first invalid live lookup after GenericJNI; do not add metadata to unproven stack words. If confirmed, repair the wrapper's Win64 unwind and outgoing-home-area contract together, add structural/live virtual-unwind tests, and repeat fatal dispatch. Design a replacement SOE delivery adapter that does not depend on retaining a fixed no-access page inside the Windows stack; then repeat repaired SOE recovery and fatal static/JIT/OSR paths. Also add debugger continue and stack-budget measurements, dynamic-JIT rollback/collection/sampling stress in J-2 and J-1, exception-unwind XMM sentinels, current-Windows repetition, and forced rejection of every named incompatible CET policy family. Native NPE, foreign VEH/frame-SEH, standalone UEF, OSR live unwind, HSP-disabled startup, and normal-return XMM sentinels already pass on build 19044.
- **Code anchors:** `vendor/art/runtime/runtime.cc`; `runtime/multiplatform/windows/sigchain_windows.cc`; `runtime_windows.cc`; `runtime/fault_handler.{h,cc}`; `runtime/arch/x86/fault_handler_x86.cc`; `runtime/arch/x86_64/quick_entrypoints_x86_64.S`; `runtime/multiplatform/windows/jit_unwind_windows.*`; `compiler/optimizing/code_generator_x86_64.*`; `compiler/utils/x86_64/{assembler,jni_macro_assembler}_x86_64.*`; `runtime/jit/{jit_code_cache,jit_memory_region}.*`; `tools/verify/win64_phase1/{check_win32_boundary_unwind.py,win32_osr_unwind_probe.cc,win32_jit_unwind_registry_probe.cc,win32_stack_growth_probe.cc,win32_uef_probe.cc}`; `tools/verify/win64_phase4/{src/W010ManagedFaultProbe.java,src/JitUnwindLifecycleProbe.java,src/CrashNativeProbe.java,host/RUN_W010_W014_DIAGNOSTICS.ps1,W010_W014_DIAGNOSTICS.md,run_osr_unwind_probe.sh,run_w010_managed_fault_probe.sh,run_jit_unwind_lifecycle.sh,run_jit_fatal_unwind.sh,run_osr_fatal_unwind.sh,RESULT-w010-managed-faults.md}`
- **Blocked on / design doc:** native E4 confirmation of the local `ExecuteSwitchImplAsm + 0x9` lookup gap, a replacement Windows SOE delivery design, then the repaired Stage E fatal/SOE matrix and broader dynamic-table sampling/churn acceptance; [win32_faults_and_stacks.md](win32_faults_and_stacks.md)
- **Opened:** 2026-07-16
- **Updated:** 2026-07-28 — native run 4 isolates traversal through ART managed/GenericJNI frames; E4 bounded live tracing is implemented, and Wine identifies `ExecuteSwitchImplAsm + 0x9` as the first lookup gap awaiting native confirmation
...
- **State:** OPEN (native bounds/reservation/page-state subsets pass; recursive managed SOE fails on build 19044 and fixed-page delivery is invalidated)
...
- **Current protection behavior:** every attached non-AOT Windows thread preserves the lowest page and adjacent bottom no-access/guard regions, selects the first exact reserved or committed-private read/write page, records its original state, installs and verifies `PAGE_NOACCESS`, and publishes stack bounds above the measured prefix and protected page. Direct protect/unprotect/restore passes on native Windows, including a reserved original page. Recursive validity is disproven: stack growth commits/reprotects the selected page as ordinary `PAGE_READWRITE`, then raises `STATUS_STACK_OVERFLOW` above it. Re-protection succeeds before `_resetstkoflw()`, so the prior switch error 13 was secondary state-check/recovery fallout.
- **Generated-code integration:** x86_64 optimizing code and nterp still use the Linux-like unconditional `rsp - ART_STACK_OVERFLOW_GAP_x86_64` probe. It translates to managed SOE under Wine but not on native build 19044, where switch/nterp/JIT all terminate with native stack overflow. The JIT fault address is exactly `RSP - 0x2000`; classification, rather than probe generation, differs. The final Win64 delivery adapter is reopened.
...
- **Former fixed-page contract, invalidated for recursive SOE:** the implemented candidate preserves the bottom page/guard prefix, selects a reserved or committed-private page, commits if needed, and protects exactly one page while retaining the 8 KiB x86_64 probe gap. This remains useful page-state/restore infrastructure and keeps the selection boundary narrow, but native protected growth proves Windows consumes the page as stack backing. Do not present fixed-page placement as a viable recursive mechanism. `PAGE_GUARD`, `SetThreadStackGuarantee`, explicit compiler checks, and direct `EXCEPTION_STACK_OVERFLOW` translation remain unselected until a replacement is evaluated against Linux-visible semantics, repeatable recovery, generated-code divergence, and stack budget.
...
- **Wine/Linux/native evidence (2026-07-27):** Wine passes the full bounds/lifetime matrix, direct page-state stress, and managed SOE recovery, while clamping sub-default requests. Native build 19044 returns exact 64 KiB and 256 KiB reservations, stable handle counts, and a reserved bottom layout with the ART page at `low + 4096`; direct page protection/restoration passes 64 cycles. Native recursive switch/nterp/JIT instead terminate with `STATUS_STACK_OVERFLOW`. The run-3 protected/writable probe proves the page ends as ordinary committed `PAGE_READWRITE`, not `PAGE_GUARD`, and proves pre-reset re-protection works. Current-Windows repetition remains required.
- **Required stages:** bounds, creation, lifetime, page selection, and direct restoration are implemented and partly native-accepted. Fixed-page managed SOE activation is rejected by native evidence. The next stage is a fresh low-divergence delivery design, preserving common probe/throw semantics where Windows permits.
- **Required acceptance:** current-Windows repetition of page consumption; final switch/nterp/JIT SOE delivery and repeatable post-overflow recovery; VEH/pre-unprotect stack high-water marks; Java post-`FixStackSize()` and representative ART pool reservations; fiber/manual-stack rejection; external detach/continue/reattach under deep guard movement; unchanged Linux `018-stack-overflow` behavior.
...
- **Code anchors:** `vendor/art/runtime/thread.cc` (`FixStackSize`, `GetThreadStack`, `InitStack`, `InstallImplicitProtection`, `ProtectStack`, `UnprotectStack`); `vendor/art/runtime/runtime.cc` implicit-check policy; `vendor/art/runtime/thread_pool.cc`; `compat/src/win64_posix_stubs.c` pthread functions; x86_64 optimizing and nterp stack probes; Windows VEH/runtime hooks; `tools/verify/win64_phase1/win32_stack_growth_probe.cc`
- **Blocked on / design doc:** replacement Windows SOE delivery design and current-Windows repetition; [win32_faults_and_stacks.md](win32_faults_and_stacks.md) is authoritative; [win32_tls_jit_entrypoints.md](win32_tls_jit_entrypoints.md) records the managed-ABI interaction
- **Opened:** 2026-07-16
- **Updated:** 2026-07-27 — native run-3 proves recursive growth converts the selected fixed page to committed `PAGE_READWRITE` before `STATUS_STACK_OVERFLOW`; direct page-state infrastructure remains valid, but fixed-page recursive SOE delivery is invalidated
...
4. **W-010/W-014 Stage E** — return E4 and confirm the local `ExecuteSwitchImplAsm + 0x9` live lookup gap above the repaired GenericJNI frame; design a replacement Windows SOE delivery mechanism after native evidence invalidated the fixed page; then repeat repaired fatal/SOE paths and complete dynamic-table churn/sampling, debugger, policy, and stack-budget acceptance.
...
*Last snapshot: 2026-07-28 — W-001/W-002/W-003/W-004/W-011/W-012/W-013/W-024 are closed; nterp and the corrected pagefile-section JIT dual view are product defaults; D-1, W-002 native R2, W-003 native R1, W-004, W-013 native R2, and W-024 are accepted. W-010/W-014 local Wine coverage remains green, and native build 19044 accepts CET classification, reservations/lifetime, direct page restore, fault records, sigchain/frame-SEH, nterp/JIT NPE, OSR live unwind, and XMM6-XMM15 normal return. Runs 3-4 invalidate fixed-page recursive SOE delivery, rule out UEF replacement, and show that JNI hardware/raised AVs miss UEF while a JNI-created native worker reaches UEF/dump. The repaired GenericJNI RDI record is correct but insufficient. E4 bounded live tracing is implemented; Wine identifies `ExecuteSwitchImplAsm + 0x9` as the first lookup gap, and native confirmation plus a replacement SOE delivery design are next. W-025 broader real-host acceptance and five open workarounds remain.*
