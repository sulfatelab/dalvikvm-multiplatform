...
| Threads / managed faults | **W-014 OPEN:** native build 19044 and current Windows accept stack reservations, pthread lifetime, and direct fixed-page selection/restoration, but recursive growth consumes the fixed protection before `STATUS_STACK_OVERFLOW`, invalidating it as managed SOE delivery. **W-010 OPEN:** native CET classification, NPE translation, sigchain/frame-SEH, OSR live unwind, and normal-return XMM6-XMM15 pass. Native E5 moves the first live miss to `art_quick_to_interpreter_bridge + 0x82`; E6 now gives its 200-byte primary and 88-byte pending frames separate records and passes structural/live Wine gates. Native E6 is pending. |
...
- **State:** OPEN (native NPE/OSR/XMM/sigchain subsets pass; managed SOE and fatal UEF dispatch remain incomplete on build 19044 and current Windows)
...
- **Required native acceptance:** run the exact-commit E6 package and require both JNI traces to resolve and cross the primary interpreter-bridge record at `+0x82`; use the bounded trace to identify any later first miss before making another metadata change. Design a replacement SOE delivery adapter that does not depend on retaining a fixed no-access page inside the Windows stack; then repeat repaired SOE recovery and fatal static/JIT/OSR paths. Also add debugger continue and stack-budget measurements, dynamic-JIT rollback/collection/sampling stress in J-2 and J-1, exception-unwind XMM sentinels, current-Windows repetition, and forced rejection of every named incompatible CET policy family. Native NPE, foreign VEH/frame-SEH, standalone UEF, OSR live unwind, HSP-disabled startup, normal-return XMM sentinels, and the switch-wrapper unwind repair already pass.
...
- **Blocked on / design doc:** native E6 interpreter-bridge/fatal-dispatch result, a replacement Windows SOE delivery design, then the repaired Stage E fatal/SOE matrix and broader dynamic-table sampling/churn acceptance; [win32_faults_and_stacks.md](win32_faults_and_stacks.md)
- **Opened:** 2026-07-16
- **Updated:** 2026-07-28 — local E6 adds and probes distinct primary/pending interpreter-bridge records; native E6 is next
...
4. **W-010/W-014 Stage E** — issue and run the exact-commit E6 interpreter-bridge package, then follow its live trace if fatal dispatch still stops; design a replacement Windows SOE delivery mechanism after native evidence invalidated the fixed page; then repeat repaired SOE paths and complete dynamic-table churn/sampling, debugger, policy, and stack-budget acceptance.
