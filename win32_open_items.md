...
| Quick/JIT/TLS | **Managed and native JIT ON with the corrected dual view by default:** rSELF=r15; nterp N-1 default ON; D-1 complete (37/37 Thread sites); W-002 CLOSED after native R2 passes 21/21 records, including all default-nterp/switch OSR and attached-thread cases; JIT smoke 12/12; JIT matrix 14/14; W-004 direct Runtime singleton load native-accepted; compile records opt-in |
...
- **State:** OPEN (historical SETUP trap and native-boundary ABI defect repaired; frame-family/native-host acceptance remains)
...
- **Current evidence:** `check_w003_quick_boundaries.py` verifies all four SETUP source contracts, the three PE save/restore sequences, Linux absence of the Windows save area, and matched PE/ELF `int3` symbol/count multisets. The rebuilt objects pass with three boundaries, 212 trap-bearing functions, and 401 traps. The dedicated XMM sentinel saves/seeds/checks every byte of XMM6–XMM11 around a twelve-double JNI callback, restores the caller state, emits its own valid PE unwind record, and passes 6/6 nterp/switch/JIT Wine processes; its intentional-clobber self-test returns exactly `0x3f`. W-002/W-004 structural gates, Linux/Win64 builds, CoreProbe, and the four-pair Wine OSR matrix also pass. Numeric trap totals remain a build snapshot rather than hard-coded acceptance values.
- **Required work:** (1) **done:** preserve/restore XMM6–XMM11 at all three ordinary Microsoft C++→managed boundaries; (2) **done:** update Win64 OSR CFA/stack accounting and add source/PE/Linux structural checks; (3) add a focused quick-frame probe covering refs-only, refs-and-args, all-callee-saves, and everything paths under `-Xint`, switch, nterp, and JIT; (4) **done on Wine:** add a native register-sentinel around JNI/quick invoke and retain the focused structural plus W-002 OSR boundary checks; native-host repetition remains under item 6; (5) **done structurally:** compare matched PE/ELF trap distributions and reject Windows-only SETUP traps; (6) run broader Wine/Linux controls and native Windows 10 acceptance with fatal/dump scans; (7) keep PE quick-assembly unwind metadata explicitly owned by W-010 unless implemented.
...
- **Evidence / analysis:** [RESULT-w003-quick-frames-analysis.md](tools/verify/win64_phase4/RESULT-w003-quick-frames-analysis.md); [RESULT-w003-xmm-sentinel.md](tools/verify/win64_phase4/RESULT-w003-xmm-sentinel.md)
...
- **Opened:** 2026-07-16
- **Updated:** 2026-07-26 — XMM6–XMM11 repair, matched PE/ELF gate, and 6/6 Wine native sentinel complete; frame-family/native-host closure remains
...
2. ~~**W-001**, **W-002**, **W-004**, **W-011**, **W-012**, and **W-024**~~ closed; **W-003** next completes focused four-family frame gates and native-host acceptance after the Microsoft XMM6–XMM11 repair and Wine sentinel.
...
*Last snapshot: 2026-07-26 — W-001/W-002/W-011/W-012/W-013/W-024 closed; Nterp ON; corrected pagefile-section dual view is the managed/native-JIT default (12/12 smoke, 14/14 matrix); D-1 complete; W-002 native R2 passes 21/21 records with 8/8 OSR, 8/8 attach, exact deterministic thresholds/checksum, and no fatal marker or dump; W-003 has all four SETUP families enabled, matched PE/ELF trap parity, repaired XMM6–XMM11 invoke/OSR boundaries, and a 6/6 nterp/switch/JIT Wine sentinel with an exact `0x3f` self-test; focused four-family and native-host acceptance remain; direct CriticalNative and 7/7 mixed/high-FP normal/FastNative matrices pass unresolved dlsym, rebinding, method tracing, and JVMTI forced interpretation; W-013 native R2 passes 56/56 with complete metrics, 20/20 repeated starts, large-heap pressure, J-1/default JIT, and no dumps; Math.ceil/floor and one common ELF/PE table are restored; interpreter.cc matches upstream; compile records are opt-in; `ART_WIN64_JIT_DUAL=0` temporarily retains J-1 for diagnosis; W-025 broader real-host acceptance remains; 6 OPEN workarounds remaining.*
