...
- **Proper fix / closure:** Design stages A–E, configuration checks, the standalone mock-owner mspace lifecycle gate, and dedicated Win64/Linux product non-moving pressure are complete. Close only after debug owner/lifetime death tests, native `VirtualQuery` extent/protection checks, large-heap commit tests, GCStress/ThreadHeavy/HandleLeak, JIT and Linux regression, and repeated native Windows 10 starts pass with recorded evidence. `GcProbe` alone is insufficient.
