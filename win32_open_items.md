...
- **Current fatal diagnosis:** captured addresses map the native return site to `art_quick_generic_jni_trampoline + 0xc5`; an `art_jni_dlsym_lookup_stub` address is also present but may be saved data/function-pointer state. The GenericJNI PE record has structural audit only, unlike invoke/OSR live virtual-unwind coverage. This is a concrete test gap, not proof of bad metadata. Next add a realistic GenericJNI `RtlVirtualUnwind()` case and compare JNI `RaiseException`, JNI hardware AV, and a JNI-created native worker AV before selecting a production repair.
...
- **Blocked on / design doc:** GenericJNI virtual-unwind and JNI/native-worker fatal-dispatch isolation, a replacement Windows SOE delivery design, then the repaired Stage E fatal/SOE matrix and broader dynamic-table sampling/churn acceptance; [win32_faults_and_stacks.md](win32_faults_and_stacks.md)
- **Opened:** 2026-07-16
- **Updated:** 2026-07-27 — third native diagnostics invalidate fixed-page recursive SOE delivery, rule out UEF replacement and standalone UEF/runner/dump failures, and narrow fatal dispatch work to GenericJNI/common native-managed exception traversal
...
4. **W-010/W-014 Stage E** — add realistic GenericJNI virtual-unwind and JNI/native-worker fatal-dispatch probes; design a replacement Windows SOE delivery mechanism after native evidence invalidated the fixed page; then repeat repaired fatal/SOE paths and complete dynamic-table churn/sampling, debugger, policy, and stack-budget acceptance.
...
*Last snapshot: 2026-07-27 — W-001/W-002/W-003/W-004/W-011/W-012/W-013/W-024 are closed; nterp and the corrected pagefile-section JIT dual view are product defaults; D-1, W-002 native R2, W-003 native R1, W-004, W-013 native R2, and W-024 are accepted. W-010/W-014 local Wine coverage remains green, and native build 19044 accepts CET classification, reservations/lifetime, direct page restore, fault records, sigchain/frame-SEH, nterp/JIT NPE, OSR live unwind, and XMM6-XMM15 normal return. Run-3 diagnostics prove recursive Windows stack growth converts ART's fixed `PAGE_NOACCESS` page to committed `PAGE_READWRITE` before `STATUS_STACK_OVERFLOW`, invalidating fixed-page SOE delivery. Standalone UEF dispatch works and ART still owns the UEF slot immediately before the JNI crash, but neither late nor ART UEF runs after VEH. Next work is GenericJNI virtual-unwind/exception-shape isolation plus a replacement SOE delivery design. W-025 broader real-host acceptance and five open workarounds remain.*
