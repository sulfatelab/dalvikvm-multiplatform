...
- **State:** OPEN (Stages 0/C/D, static fatal boundaries, and dynamic-JIT metadata/registration/lifecycle are implemented locally; native acceptance, OSR-stub unwind, and full XMM boundary work remain)
...
- **Static and dynamic fatal result:** PE unwind records cover `art_quick_invoke_stub`, `art_quick_invoke_static_stub`, and `art_quick_generic_jni_trampoline`; an emitted-record audit verifies their frame anchors, allocations, GPR saves, and current full-width XMM6-XMM11 saves. The static `-Xint` and dynamic threshold-zero J-2/J-1 JNI AV gates reach initial VEH and UEF and create new valid `MDMP` dumps. This proves fatal dispatch across the exercised optimizing/JIT-JNI chain, not debugger-quality minidump stack reconstruction. Every invoke/OSR native-to-managed boundary must still extend full-width preservation through XMM15 because ART managed frames preserve only 64-bit scalar XMM12-XMM15 state while Win64 requires 128 bits. `art_quick_osr_stub` still needs its own static runtime-function record for the fixed saves, RBP anchor, and variable copied-stack interval.
...
- **Code anchors:** `vendor/art/runtime/runtime.cc`; `runtime/multiplatform/windows/sigchain_windows.cc`; `runtime_windows.cc`; `runtime/fault_handler.{h,cc}`; `runtime/arch/x86/fault_handler_x86.cc`; `runtime/arch/x86_64/quick_entrypoints_x86_64.S`; `runtime/multiplatform/windows/jit_unwind_windows.*`; `compiler/optimizing/code_generator_x86_64.*`; `compiler/utils/x86_64/{assembler,jni_macro_assembler}_x86_64.*`; `runtime/jit/{jit_code_cache,jit_memory_region}.*`; `tools/verify/win64_phase1/{check_win32_boundary_unwind.py,win32_jit_unwind_registry_probe.cc}`; `tools/verify/win64_phase4/{src/W010ManagedFaultProbe.java,src/JitUnwindLifecycleProbe.java,run_w010_managed_fault_probe.sh,run_jit_unwind_lifecycle.sh,run_jit_fatal_unwind.sh,RESULT-w010-managed-faults.md}`
- **Blocked on / design doc:** native Stage E evidence with W-014, OSR-stub unwind, full XMM6-XMM15 boundary preservation, and broader dynamic-table sampling/churn acceptance; [win32_faults_and_stacks.md](win32_faults_and_stacks.md)
- **Opened:** 2026-07-16
- **Updated:** 2026-07-27 — recovery/GC and chain probes pass; static and dynamic JIT fatal paths reach UEF/minidump; dynamic anchors, serialization, xdata, registration, collection/reuse lifecycle, and JNI metadata pass focused J-2/J-1 plus Linux builds; XMM12-XMM15 boundaries, OSR unwind, broader churn/sampling, and native policy/debugger/stack-budget acceptance remain
...
4. **W-010/W-014 Stage E** — complete OSR-stub unwind and native XMM6-XMM15 boundary preservation, broaden dynamic-table churn/sampling coverage, then run native policy/fault/stack acceptance.
