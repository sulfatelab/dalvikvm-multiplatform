...
- **State:** OPEN (Stages 0/C/D, static invoke/JNI plus split OSR unwind, and dynamic-JIT metadata/registration/lifecycle are implemented locally; native acceptance and full XMM boundary work remain)
...
- **Static and dynamic fatal result:** PE unwind records cover `art_quick_invoke_stub`, `art_quick_invoke_static_stub`, `art_quick_generic_jni_trampoline`, and two contiguous ranges for `art_quick_osr_stub`. The entry range uses fixed-bottom RBP through the variable copied-stack body; the return range describes the inherited 184-byte frame directly from RSP because OSR code reconstructs managed RBP before returning. This zero-prologue inherited-frame record is a deliberate Windows platform adapter, not a general called-function pattern or a fallback workaround. The emitted audit verifies exact frame anchors, allocations, GPR saves, completed-frame XMM offsets, and range contiguity. A live Wine probe restores RBP/RDI/RSI/RBX/R12-R15 and XMM6-XMM11 from a variable-depth entry context, a managed-RBP-clobbered return context, and the canonical return epilogue; the actual 8/8 OSR execution matrix passes. The static `-Xint` and dynamic threshold-zero J-2/J-1 JNI AV gates reach initial VEH and UEF and create new valid `MDMP` dumps. This proves fatal dispatch across the exercised optimizing/JIT-JNI chain, not debugger-quality minidump stack reconstruction or native OSR fatal dispatch. Every invoke/OSR native-to-managed boundary must still extend full-width preservation through XMM15 because ART managed frames preserve only 64-bit scalar XMM12-XMM15 state while Win64 requires 128 bits.
...
- **Required native acceptance:** repeat the nterp/JIT NPE/SOE and recovery/GC evidence on Windows 10/current Windows; exact wrong-address cases; foreign VEH before/after/promotion; frame-based SEH for an unrecognized AV; debugger continue; stack-budget measurements; static fatal predecessor-UEF/dump behavior; both OSR records' lookup/live unwind plus an OSR fatal path; dynamic-JIT lookup/unwind/rollback/collection/sampling stress in J-2 and J-1; full XMM6-XMM15 normal-return and unwind sentinels; and HSP-disabled startup plus forced compatibility/audit/strict rejection.
- **Code anchors:** `vendor/art/runtime/runtime.cc`; `runtime/multiplatform/windows/sigchain_windows.cc`; `runtime_windows.cc`; `runtime/fault_handler.{h,cc}`; `runtime/arch/x86/fault_handler_x86.cc`; `runtime/arch/x86_64/quick_entrypoints_x86_64.S`; `runtime/multiplatform/windows/jit_unwind_windows.*`; `compiler/optimizing/code_generator_x86_64.*`; `compiler/utils/x86_64/{assembler,jni_macro_assembler}_x86_64.*`; `runtime/jit/{jit_code_cache,jit_memory_region}.*`; `tools/verify/win64_phase1/{check_win32_boundary_unwind.py,win32_osr_unwind_probe.cc,win32_jit_unwind_registry_probe.cc}`; `tools/verify/win64_phase4/{src/W010ManagedFaultProbe.java,src/JitUnwindLifecycleProbe.java,run_osr_unwind_probe.sh,run_w010_managed_fault_probe.sh,run_jit_unwind_lifecycle.sh,run_jit_fatal_unwind.sh,RESULT-w010-managed-faults.md}`
- **Blocked on / design doc:** native Stage E evidence with W-014, native OSR lookup/fatal acceptance, full XMM6-XMM15 boundary preservation, and broader dynamic-table sampling/churn acceptance; [win32_faults_and_stacks.md](win32_faults_and_stacks.md)
- **Opened:** 2026-07-16
- **Updated:** 2026-07-27 — recovery/GC and chain probes pass; static and dynamic JIT fatal paths reach UEF/minidump; dynamic anchors, serialization, xdata, registration, collection/reuse lifecycle, JNI metadata, and split OSR entry/return unwind pass focused Wine plus Linux builds; XMM12-XMM15 boundaries, native OSR fatal acceptance, broader churn/sampling, and native policy/debugger/stack-budget acceptance remain
...
- **State:** OPEN (Stages A-B and shared Stage D activation implemented locally; native A-B/E acceptance remains)
...
- **Opened:** 2026-07-16
- **Updated:** 2026-07-27 — expanded reservation, post-SOE stack-trace/allocation/GC, and chain probes pass; native A-B/E acceptance remains
...
4. **W-010/W-014 Stage E** — repeat and accept the split OSR unwind plus an OSR fatal path on native Windows, complete XMM6-XMM15 boundary preservation, broaden dynamic-table churn/sampling coverage, then run native policy/fault/stack acceptance.
