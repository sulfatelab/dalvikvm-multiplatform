...
| Threads / managed faults | **W-014 OPEN:** Stages A-B now have exact current-stack bounds, `_beginthreadex` reservation semantics, retained pthread lifetime, Windows thread-pool sizing, measured excluded-low accounting, a verified fixed `PAGE_NOACCESS` page, and detach restoration; native A-B acceptance remains. **W-010 OPEN:** Stage 0 explicitly marks every project PE `/CETCOMPAT:NO` and rejects active/nonzero HSP policy before memory/thread/JIT startup, while the VEH remains diagnostic-only, so nterp/JIT implicit faults still do not become managed Java exceptions. Native HSP compatibility/audit/strict rejection evidence remains pending. |
...
- **Area:** art / exceptions
- **Current behavior:** `sigchain_windows.cc` is a stub. `runtime_windows.cc` installs a first-chance VEH that logs access-violation, stack, illegal-instruction, and divide faults and returns `EXCEPTION_CONTINUE_SEARCH`, plus an unhandled filter that writes a best-effort minidump. Runtime teardown now removes the VEH while `art.dll` is executable and restores ART's predecessor only if ART remains the current UEF, preserving any later host filter. The current fatal UEF itself still returns `EXCEPTION_EXECUTE_HANDLER`; predecessor chaining remains Stage C work. This is crash diagnostics, not ART's Linux-equivalent generated-code fault handling. In particular, nterp's implicit null load at `nterp_op_invoke_virtual+0x3a` remains an access violation instead of becoming `NullPointerException`; the failure reproduces in both instrumented and ordinary product builds.
...
- **Generated-code conflict:** x86_64 optimizing code and nterp still emit the normal unconditional `rsp - ART_STACK_OVERFLOW_GAP_x86_64` probe. Stage B now makes its eventual low-stack access deterministic at ART's fixed page, but the diagnostic-only VEH still cannot translate it into `StackOverflowError`; a sufficiently deep generated stack therefore remains an unhandled native AV until Stages C-D. The switch interpreter continues to use explicit `Thread::stack_end_` comparisons.
...
- **Required stages:** Stage 0 build/startup CET exclusion and W-014 Stages A-B are implemented locally, with native acceptance pending. Next is (C) the dormant W-010 VEH/non-owning real-`CONTEXT` adapter; then (D) atomic implicit-null/SO and nterp/JIT activation; and (E) final Wine/Linux regression plus native Windows acceptance.
...
- **Opened:** 2026-07-16
- **Updated:** 2026-07-26 — Stage B measured selection, fixed-page state, verified committed/reserved restoration, direct-fault probe, and same-thread detach/use/reattach gate implemented; native A-B acceptance remains and Stage C is next
