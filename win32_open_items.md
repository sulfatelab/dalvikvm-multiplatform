...
- **Proper fix / closure:** Design stages A–E are complete. Close only after the remaining configuration and mspace create/grow/free/trim/regrow checks, dedicated non-moving pressure, `VirtualQuery` extent/protection checks, large-heap commit tests, GCStress/ThreadHeavy/HandleLeak, JIT and Linux regression, and repeated native Windows 10 starts pass with recorded evidence. `GcProbe` alone is insufficient.
