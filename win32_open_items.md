...
- **Blocked on / design doc:** `art_quick_to_interpreter_bridge` range-accurate Win64 unwind repair, a replacement Windows SOE delivery design, then the repaired Stage E fatal/SOE matrix and broader dynamic-table sampling/churn acceptance; [win32_faults_and_stacks.md](win32_faults_and_stacks.md)
- **Opened:** 2026-07-16
- **Updated:** 2026-07-28 — native E5 verifies the switch-wrapper repair and identifies `art_quick_to_interpreter_bridge + 0x82` as the next first missing live unwind record
...
4. **W-010/W-014 Stage E** — repair and probe both Win64 stack shapes in `art_quick_to_interpreter_bridge`, then repeat fatal dispatch; design a replacement Windows SOE delivery mechanism after native evidence invalidated the fixed page; then repeat repaired SOE paths and complete dynamic-table churn/sampling, debugger, policy, and stack-budget acceptance.
