...
| Heap memory | W-013 Stages A–E complete; failed native R1 reviewed and repaired (`PAGE_NOACCESS` discard, J-1 provider rebinding, socket-fd ownership, runner accounting); R2 acceptance remains OPEN |
...
- **State:** OPEN (design implemented; failed native R1 repaired; R2 acceptance remains)
...
- **Current behavior:** Stages A through E are complete. `_WIN32`/`WIN32` remain visible; dlmalloc's Win32 defaults respect ART's explicit `HAVE_MMAP=0`, `HAVE_MORECORE=1`, `MORECORE_CONTIGUOUS=1`, `USE_LOCKS=0`, mspace-only policy; growth granularity is `dwPageSize`; failures set `ENOMEM`. Every heap and JIT mspace stores its owner provider in `malloc_state::extp/exts`, and MoreCore dispatches directly without `Runtime::Current()`, JIT-cache lookup, or continuous-space scans. Heap mspaces use `DlMallocSpace::lock_`; JIT mspaces use `Locks::jit_lock_`, with provider detach/rebind across clear, reset, destruction, and JIT-region moves. Executable JIT mspace metadata is attached/detached under `ScopedCodeCacheWrite`, so the diagnostic J-1 RX mapping is never written without its existing temporary write transition. Windows anonymous mappings distinguish anywhere, below-4-GiB, and exact requests explicitly; low/aligned allocation uses `VirtualAlloc2` plus `MEM_ADDRESS_REQUIREMENTS`; reuse inside an existing reservation changes protection in place; and logical views share one allocation-base owner so private and section mappings use the correct whole-release API exactly once. Heap activation, deactivation, discard, trim, clear, RosAlloc page release, and Windows logical shrink now route through `MemMap`; Windows walks `VirtualQuery` protection regions, temporarily makes `PAGE_NOACCESS` subranges writable for `DiscardVirtualMemory`, and restores the exact old protection, while Linux retains `mprotect` and `MADV_DONTNEED`. Runtime/compiler/JIT arenas, ordinary LinearAlloc, and the card table now use Linux-like anywhere placement; only audited object/image/heap ranges, the JIT primary view, and the exact sentinel request remain low. The Windows-only `MarkCard` and non-moving allocation log/range-check/skip workarounds are removed, so both card writes use the common checked barrier paths. Full capacity remains committed initially. Fixed file-view replacement over an ordinary `VirtualAlloc` reservation remains unsupported and is not used by the imageless/JIT product path.
...
- **Low-address policy:** Java object spaces, non-moving/LOS, required image/heap ranges, and the JIT primary view remain below 4 GiB. LinearAlloc, compiler/JIT metadata arenas, and the card table are unrestricted after the Stage E encoding audit; bitmaps, stacks, reference tables, and temporary mappings were already unrestricted. The source gate pins the remaining required-low product inventory and requires an explicit audit for additions.
...
- **Native host gate:** R1 from Windows 10 build 19044 was reviewed and remains `OVERALL FAIL`. It found real-Windows `DiscardVirtualMemory(PAGE_NOACCESS)`, J-1 provider-metadata RX writes, and cross-DLL socket-fd classification defects, plus blank runner accounting and one nondeterministic JIT marker. ART `6253d01afc` and `27a1ac74a4`, root `c943f1f` and `caad337`, and libcore `67ec4ab8dd70` repair those findings. `tools/win64/host_package/package_win64_w013.sh` generates the focused R2 archive; `tools/verify/win64_w013/W013_HOST_CHECKLIST.md` requires native mapping/config/owner probes with fragmented and exhausted low VA, non-moving pressure at 128-MiB and 1-GiB `-Xmx`, moving/LOS GC stress, ThreadHeavy, HandleLeak, default dual-view and J-1 JIT coverage, the fourteen-case JIT matrix, 512-MiB and 1-GiB startup metrics, twenty repeated default-JIT starts, host memory/pagefile capture, log scanning, and dump scanning. Returned R2 evidence remains open.
- **Proper fix / closure:** Design stages A–E, configuration checks, the standalone mspace lifecycle gate, actual-wrapper owner/rebind and 4/4 lifetime death tests, and dedicated Win64/Linux product non-moving pressure are complete. Close only after native `VirtualQuery` extent/protection checks, large-heap commit tests, GCStress/ThreadHeavy/HandleLeak, JIT and Linux regression, and repeated native Windows 10 starts pass with recorded evidence. `GcProbe` alone is insufficient.
...
- **Evidence:** `tools/verify/win64_w013/RESULT.md`; external dlmalloc `f3356ce`; ART Stage A `8c900a9e4b`; ART Stage B `d011d72d56`; ART Stage C `2fa301a13b`; ART Stage D `9ea15456a2`; ART deactivated discard `6253d01afc`; ART Stage E `47567cebcc`; non-moving barrier cleanup `1509b1f95e`; J-1 provider rebinding `27a1ac74a4`; runner accounting `c943f1f`; process-wide socket-fd registry root `caad337` / libcore `67ec4ab8dd70`
- **Opened:** 2026-07-16
- **Updated:** 2026-07-25 — native R1 reviewed; three product defects and two runner defects repaired; R2 acceptance remains open
...
*Last snapshot: 2026-07-25 — W-001/W-011/W-012/W-024 closed; Nterp ON; corrected pagefile-section dual view is the managed/native-JIT default (12/12 smoke, 14/14 matrix); D-1 complete; direct CriticalNative and 7/7 mixed/high-FP normal/FastNative matrices pass unresolved dlsym, rebinding, method tracing, and JVMTI forced interpretation; W-013 Stages A–E are complete, native R1 has been reviewed, and its three product plus two runner defects are repaired for R2; Math.ceil/floor and one common ELF/PE table are restored; interpreter.cc matches upstream; compile records are opt-in; `ART_WIN64_JIT_DUAL=0` temporarily retains J-1 for diagnosis; W-025 broader real-host acceptance remains; 9 OPEN workarounds remaining.*
