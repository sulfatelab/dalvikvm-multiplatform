...
- **State:** OPEN (historical SETUP trap and native-boundary ABI defect repaired; focused runtime/native acceptance remains)
...
- **Current evidence:** `check_w003_quick_boundaries.py` verifies all four SETUP source contracts, the three PE save/restore sequences, Linux absence of the Windows save area, and matched PE/ELF `int3` symbol/count multisets. The rebuilt objects pass with three boundaries, 212 trap-bearing functions, and 401 traps. W-002/W-004 structural gates, Linux/Win64 builds, CoreProbe quick-to-interpreter smoke, and the four-pair Wine OSR matrix also pass after the repair. These numeric trap totals remain a build snapshot rather than hard-coded acceptance values.
- **Required work:** (1) **done:** preserve/restore XMM6–XMM11 at all three ordinary Microsoft C++→managed boundaries; (2) **done:** update Win64 OSR CFA/stack accounting and add source/PE/Linux structural checks; (3) add a focused quick-frame probe covering refs-only, refs-and-args, all-callee-saves, and everything paths under `-Xint`, switch, nterp, and JIT; (4) add a native register-sentinel probe around JNI/quick invoke and retain a focused OSR boundary check; (5) **done structurally:** compare matched PE/ELF trap distributions and reject Windows-only SETUP traps; (6) run broader Wine/Linux controls and native Windows 10 acceptance with fatal/dump scans; (7) keep PE quick-assembly unwind metadata explicitly owned by W-010 unless implemented.
...
- **Opened:** 2026-07-16
- **Updated:** 2026-07-26 — XMM6–XMM11 boundary repair and permanent matched PE/ELF structural gate implemented; focused runtime/native closure remains
