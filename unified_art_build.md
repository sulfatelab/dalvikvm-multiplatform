...
| Windows x86-64 product | PARTIAL / experimental | Linux-hosted cross and native Windows Server 2025 product builds pass; the accepted native baseline passes 77/77 across W-002, W-003, W-004, W-010, W-013, W-014, W-025, and W-027, including the FS-1 product-isolation review; W-028 passes native generation, W-030 passes private-copy/LZ4 boot loading 2/2, and W-031 passes core AOT unwind, underlying managed/JNI body lookup, and JIT-disabled runtime calls 1/1; complete-package import/stale-path acceptance and identical no-op builds pass | migrate additional target applicability and continue CFG, ordinary boot-OAT dispatch, deeper unwind, and product-integration work |
...
| Unified phase catalog | COMPLETE for current declaration truthfulness; PARTIAL target expansion | twelve virtual stages declare 35 native probes, 48 managed JARs, and 16 command gates; Windows has 96 applicable items (73 target-runnable, nine host-review, and 14 compile-only in the product variant), Linux x86-64 has 18 applicable items (fifteen runnable and three compile-only artifacts), and experimental Linux AArch64 has 17 applicable items (fourteen runner-backed gates and three compile-only artifacts); CMake rejects runnable shared libraries and compile-only command gates; native W-028 through W-031 evidence is accepted | expand exact-target applicability only with matching behavioral acceptance |
...
- [x] Windows AOT/OAT sequence step 1 passes the W-028 native operation gate.
...
- [x] Windows AOT/OAT sequence step 2 has an accepted W-029 identity
  ...
  `-Ximage:runtime/boot-image/boot.art`. The native host-review gate passes
  1/1 and rejects seven case, separator, physical-path, and topology
  mismatches. W-030 now makes the generator, manifest, staging, and
...
- [x] W-030 completes boot-only sequence steps 4 and 5 and starts steps 8 and
  ...
  segments for validation-only and executable `ElfOatFile` opens and reused
  VDEX bytes for `oatdex`; Linux retains its prior fixed mapping path. The
  direct native probe passes in 0.08 s. Windows generates an LZ4 image because
...
- [x] W-031 makes unwind step 6 and execution step 10 partial. The writer emits
  ...
  a 515,736-byte `.oat_unwind.windows` section. Native Server 2025 validates
  both open modes, registers the executable table, resolves managed, JNI, and
  all seven trampoline entries, passes synthetic `RtlVirtualUnwind`, locates
...
- [x] Every current virtual stage now owns a real behavioral command or host
  ...
  reviewer that proves `art.dll`, generated asm definitions, and GNU-style
  Clang commands contain no instrumentation. The exact
  `win32-stack-high-water` variant uses the same reviewer in positive mode to
  check the export, offsets, and direct-store objects. Fresh product and
...
- [x] `linux-x86_64-gnu` now owns the `boot_image` capability. A native build
  runs target `dex2oat` through a shell-free Python helper with canonical
  `/system/framework/boot.jar`, a bounded 64/512 MiB runtime heap,
  deterministic invocation storage, and an atomic target-local output tree.
  The fresh product generated `boot.art` (7,854,704 bytes), `boot.oat`
  (18,901,672 bytes), and `boot.vdex` (8,309,376 bytes). The manifest and all
  three artifacts contain no build-machine path; only the canonical boot-JAR
  location is retained. The W-004 image-backed Hello gate passed from an
  isolated runtime root, staging included exactly the three image artifacts
  plus their relative manifest, and the immediate 32-job product repeat was a
...
- [x] Configure, build, and stage now enforce one command-graph audit, also
  ...
  passed 209/209 host tests. The current Linux graph accepts 2,088 compile
  commands, 2,172 Ninja commands, 32 product links, a 2,173-edge build that
  includes the boot image, a Ninja no-op repeat, and a 161-regular-file stage.
  Linux-hosted Windows and native Windows Server
...
- [x] Fresh Linux configuration with Clang 21, CMake, Ninja, and configured
  JDK 21 emits a 93-declaration catalog. Seventeen declarations apply to
  `linux-x86_64-gnu`: two runnable and two compile-only W-003 declarations,
  eleven runnable W-004 gates, and the W-013 non-moving managed artifact plus
  its runnable 128 MiB gate. Fourteen register with CTest; the three managed/shared
...
- [x] Windows-target configuration emits the same 93 declarations and keeps
  ...
  `host-review` declarations. Fifteen applicable
...
- [x] The initial cross-platform CriticalNative and normal/FastNative
  acceptance gave both gates explicit exact-ID selectors for
  `linux-x86_64-gnu` and `windows-x86_64-msvc`; at that checkpoint no other
  architecture or ABI was inferred. Their shared shell-free runner resolves
  ELF/DLL names, library-path separators, absolute versus library-name loading,
  and target JIT controls without a host shell. A fresh Linux tree passed both
  gates and its repeat was a Ninja no-op. A fresh native Windows Server 2025
  tree built all 1,492 actions at 16 jobs, passed product W-003 4/4, and
  repeated with `ninja: no work to do` and another 4/4 pass. Each of the four
  aggregate JNI results records four completed runs, zero dumps, and no machine
  path; source and output scans found zero links or reparse points. The
  Linux-hosted Windows cross tree also completed 1,492 actions at 32 jobs,
  passed its host reviewer, and repeated as a Ninja no-op.
- [x] The W-004 ART embedding probe is now a real exact-target runtime gate for
  `windows-x86_64-msvc`; no other target is inferred. Its shell-free case
  runner copies the regular-file product DLL closure into an isolated `bin`
  directory, gives both repetitions separate boot/ICU/data/tmp/crash roots,
  and validates JNI create/destroy plus exact ART VEH, predecessor/late UEF,
  frame-SEH, and teardown behavior. Entering the real ART UEF intentionally
  emits exactly one dump per process; missing, extra, or failed dump writes are
  errors, and only relative dump names enter the sanitized aggregate. The
  Linux-hosted Windows cross tree completed 1,548 actions at 32 jobs, passed
  its reviewer, and repeated as a Ninja no-op. The native 16 GiB Server 2025
  tree completed the same graph at 16 jobs; after the runtime gate exposed and
  corrected DLL search staging, W-004 passed 27/27 twice. A full-catalog audit
  then found 21 pre-migration Win32 Unicode files in the disposable source
  projection; refreshing that exact regular-file closure rebuilt the affected
  product edges. The current-source catalog passed 67/67 in 127.52 seconds and
  repeated from a true Ninja no-op at 67/67 in 130.57 seconds. W-004 accounted
  for 41.22/43.74 seconds and embedding for 2.93/3.34 seconds. The aggregate
  records two completed repetitions, 22 DLLs, two intentional dumps, no
  machine path, and zero reparse points in both source and output.
- [x] UdpProbe is now target-runnable on exactly
  `windows-x86_64-msvc`. Native reproduction confirmed that Android's bionic
  `SO_BROADCAST = 6` reached the active JNI bridge and was forwarded as the
  wrong Winsock option, producing `setsockopt EINVAL` during
  `DatagramSocket` construction. The bridge now translates the bionic value
  to Winsock `SO_BROADCAST` for both set and get. Linux-hosted Windows cross
  rebuilt `javacore.dll`, passed its W-004 reviewer, and repeated as a Ninja
  no-op at 32 jobs. Native Windows rebuilt the same DLL and passed W-004
  28/28 in 42.16 seconds, then repeated from a Ninja no-op at 28/28 in 41.85
  seconds with 16 jobs. UdpProbe passed in 0.93 seconds both times; after its
  peer address marker became mandatory, a final no-op stage passed 28/28 in
  42.26 seconds and UdpProbe in 0.94 seconds. The complete no-op catalog passed
  68/68 in 126.93 seconds. Its result contains no machine path, and native
  source/output scans contain zero reparse points.
- [x] LocaleProbe is now target-runnable on exactly
  `windows-x86_64-msvc`. Its historical 120-second native timeout does not
  reproduce in the current integrated runtime. The declared contract is
  deliberately narrow: US locale identity/case conversion, UTC epoch and
  one-day calendar arithmetic, and a `zh-Hans-CN` language-tag round trip.
  Resource-backed display names and collation remain unclaimed because the
  current runtime reports a null display language and catches a missing ICU
  resource for the probe's soft collator check. Linux-hosted Windows cross
  reconfigured with no build work, passed the W-004 reviewer, and repeated as
  a Ninja no-op at 32 jobs. Native Windows passed W-004 29/29 in 43.11 seconds
  and repeated from a Ninja no-op at 29/29 in 43.08 seconds with 16 jobs;
  LocaleProbe took 0.95/0.96 seconds. The complete no-op catalog passed 69/69
  in 126.66 seconds. Its result contains no machine path, and native source
  and output scans contain zero reparse points.
- [x] ZipProbe is now target-runnable on exactly
  `windows-x86_64-msvc`. Its historical 120-second timeout does not reproduce
  in the current integrated runtime. The promoted contract requires the exact
  CRC32, raw DEFLATE/INFLATE round-trip success, three matching entries through
  both `ZipInputStream` and `ZipFile`, root-entry lookup, and successful
  temporary-ZIP deletion. Linux-hosted Windows cross rebuilt only the managed
  Zip artifact at 32 jobs and passed the W-004 reviewer. Native Windows passed
  W-004 30/30 in 44.23 seconds and repeated from a Ninja no-op at 30/30 in
  44.24 seconds with 16 jobs; ZipProbe took 1.03/0.97 seconds. The complete
  no-op catalog passed 70/70 in 129.62 seconds. Its result has exit zero, no
  missing or forbidden markers, and no machine path; native source/output
  scans contain zero reparse points.
- [x] BnProbe is now target-runnable on exactly
  `windows-x86_64-msvc`. The compact NativeBN/BigInteger contract requires
  positive addition/multiplication/remainder/modular-exponentiation results,
  an exact byte-array multiplication round trip, and deterministic exact
  sum/remainder/modPow output. Linux-hosted Windows cross reconfigured without
  build work and passed the W-004 reviewer at 32 jobs. Native Windows passed
  W-004 31/31 in 44.92 seconds and repeated from a Ninja no-op at 31/31 in
  45.07 seconds with 16 jobs; BnProbe took 0.93/0.96 seconds. The complete
  no-op catalog passed 71/71 in 129.15 seconds. Its result has exit zero, no
  missing or forbidden markers, and no machine path; native source/output
  scans contain zero reparse points.
- [x] OsConstantsProbe is now target-runnable on exactly
  `windows-x86_64-msvc`. Its contract requires all 18 exact Android/bionic
  AF/SOCK, errno, address-info, file, signal, sysconf, and name-info constant
  values that the Windows JNI bridge consumes and translates. Linux-hosted
  Windows cross reconfigured without build work and passed the W-004 reviewer
  at 32 jobs. Native Windows passed W-004 32/32 in 45.80 seconds and repeated
  from a Ninja no-op at 32/32 in 45.70 seconds with 16 jobs; OsConstantsProbe
  took 0.94 seconds both times. The complete no-op catalog passed 72/72 in
  130.17 seconds. Its result has exit zero, no missing or forbidden markers,
  and no machine path; native source/output scans contain zero reparse points.
- [x] XmlProbe is now target-runnable on exactly `windows-x86_64-msvc`. Its
  namespace-aware Harmony Expat/SAX contract requires exactly three elements
  and the exact `helloworld` text stream. Linux-hosted Windows cross
  reconfigured without build work and passed the W-004 reviewer at 32 jobs.
  Native Windows passed W-004 33/33 in 46.92 seconds and repeated from a Ninja
  no-op at 33/33 in 46.66 seconds with 16 jobs; XmlProbe took 0.93/0.98 seconds.
...
- [x] SocketAddressProbe is now target-runnable on exactly
  ...
  missing or forbidden markers, and no machine path; native source/output
  scans contain zero reparse points.
- [x] AsyncCloseProbe is now target-runnable on exactly
  `windows-x86_64-msvc`. It requires closing a server to unblock a blocked
  accept with a socket error and closing an accepted peer to unblock a client
  read with EOF or an I/O error; both workers must terminate, and an unexpected
  accepted peer or explicit failure marker is forbidden. Linux-hosted Windows
  cross reconfigured without build work and passed the W-004 reviewer at 32
  jobs. Native Windows passed W-004 35/35 in 49.03 seconds and repeated from a
  Ninja no-op at 35/35 in 49.23 seconds with 16 jobs; AsyncCloseProbe took
  1.44/1.48 seconds. The complete no-op catalog passed 75/75 in 132.83 seconds.
  Its result has exit zero, no missing or forbidden markers, and no machine
  path; native source/output scans contain zero reparse points.
...
- [x] W-004's remaining composite-package checker, returned-ZIP reviewer,
  ...
  reproduction is the unified 27-gate W-004 stage at `--parallel 16`; the
...
- [x] All 33 catalog-owned native/assembly probe declarations have logical
  `tests/cases` ownership across 30 source-owning cases with adjacent results.
...
- [x] All 49 retained Java probe sources now have logical `tests/cases`
  ownership and adjacent results. The registry emits 48 managed artifacts;
...
- [x] The L-003 migration promotes ExecProbe and Ipv6Probe on exactly Windows +
  ...
  `DatagramSocket` construction with `setsockopt failed: EINVAL`; at that
  checkpoint those three remained compile-only. The historical all-pass Wine
  result remains evidence, not native acceptance. UdpProbe, LocaleProbe, and
  ZipProbe's later promotions are recorded in the current acceptance entries
  above.
...
- [x] The initial managed-runtime-stress admission ran HandleLeakProbe,
  PerfSmokeProbe, and ThreadHeavyProbe on exactly Windows + x86-64 + MSVC ABI
  through the shared shell-free managed runtime gate. Each uses interpreter
  mode, a 180-second child timeout, an
...
- [x] The documented Linux W-004 `test` command builds its declared product and
  managed-runtime dependencies and passes six CTest gates: imageless and
  image-backed Hello, GC stress, Math CriticalNative, exact
  `dalvikvm -showversion`, and target-side ELF DSO loading wrapped by Python.
  The topology gate requires `libart-compiler.so -> libart.so` while forbidding
  the reverse edge. Its identical rerun is a Ninja no-op.
...
- [x] Building the newly rooted Linux `openjdkjvmti` target exposed two pinned
  ...
  contract.
- [x] The initial Math CriticalNative admission gave one case-local Python
  matrix to the exact Linux and Windows x86-64 targets. It runs `-Xint` and
  threshold-zero JIT twice each, requires an explicit Windows compile record,
  writes portable aggregate JSON, and rejects dumps and filesystem
  links/reparse points. Its W-024 source
...
| `w031` | 1 DLL, 1 managed | 2 exact / 0 typed | 1 target-runnable, 1 compile-only | native Server 2025 core AOT unwind, underlying body lookup, and JIT-disabled managed/JNI runtime-call gate passes; ordinary dispatch, negative injection, and deeper stack walking remain |
| Total | 24 EXEs, 11 DLLs, 48 managed, 16 gates | 45 exact / 54 typed | product: 76 target-runnable, 9 host-review, 14 compile-only | Windows applies 96 declarations (73 runnable, nine host-review, 14 compile-only); Linux x86-64 applies 18 (fifteen runnable, three compile-only), and Linux AArch64 applies 17 (fourteen runnable, three compile-only) |

The shared registry now references zero source files from historical
verification directories. All 99 declarations own canonical source under
`tests/cases/` or a shell-free runner under `tests/support/`; all 32 native
source cases and all 49 Java sources have adjacent results, and shared stage
...
The managed Math CriticalNative declaration independently lists the same exact
three-target set. Its case-local runner distinguishes interpreter/JIT behavior
and retains the Windows-only compile-record assertion without making that
diagnostic an AArch64 requirement.

The managed HandleLeak, PerfSmoke, and ThreadHeavy declarations also list that
exact three-target set and use the common interpreter-only runner with the
optional AArch64 execution prefix. Each probe retains its own selector so
future applicability changes remain independently reviewed.

The managed libcore CoreProbe, InterruptProbe, and RtMem independently list the
same exact three-target set. CoreProbe exercises platform-neutral arraycopy,
UTF-8 charset, reflection, and monitor/thread behavior; InterruptProbe proves
sleeper interruption and termination; RtMem checks runtime memory reporting and
allocation. Every other declaration in their shared case directory retains the
Windows x86-64/MSVC selector pending independent review.

Current evidence remains much narrower than theoretical applicability:

- all 32 Windows-applicable native probes have compile evidence for the
...
- Linux x86-64 builds and runs the two W-003 JNI ABI probes through four exact
  declarations, in addition to its twelve runnable W-004 gates and shared W-013
  gate;
- Linux AArch64 builds and runs both W-003 JNI probes through four exact
  ...
  show-version, imageless Hello, GC stress, HandleLeak, PerfSmoke, ThreadHeavy,
  libcore CoreProbe, InterruptProbe, and RtMem, and the shared 128 MiB W-013 gate under
  its explicit runner; and
...
- [x] Make those two probes fully platform-resolved, then validate rather than
...
- [x] Register behavioral commands and expected-result reviewers for every
  current stage. W-014 now reviews both product absence and instrumented
  presence of FS-1 state. CMake rejects runnable shared libraries and
  compile-only command gates, so a built DLL or empty utility target cannot be
...
- [x] Admit the shared 128 MiB non-moving-heap contract on Linux AArch64. The
  gate now receives the same explicit runner/root argument list as W-004;
  direct target launch had failed before reaching ART and was not treated as
  allocator evidence. The corrected gate allocated 75,497,472 bytes, proved
  low/stable addresses across forced collections and post-GC regrowth, and
  passed in 2.06 seconds. It repeated in 2.13 seconds after a true Ninja no-op.
  The 1024 MiB resource-pressure profile remains Windows x86-64-specific.
- [x] Admit the exact CriticalNative contract on Linux AArch64 without
  broadening normal/FastNative. The W-003 runner now forwards the declared
  external runner/root, and the common test-target policy selects LLD through
  plain Clang on every target. Four QEMU processes covered library/absolute
  loads, registered/dlsym mixed and spilled signatures, threshold-zero JIT,
...
- [x] Admit Math CriticalNative independently on Linux AArch64. Four QEMU
  processes covered `-Xint` and threshold-zero JIT twice each, native
  ceil/floor modifiers, 23 edge inputs, 2,000 rounds, and the exact checksum.
  Math passed in 82.47 seconds; W-004 passed 4/4 in 86.57 seconds and repeated
  4/4 in 84.54 seconds after a true Ninja no-op. Native Linux x86-64 repeated
  W-004 from a no-op at 6/6 in 2.35 seconds.
- [x] Admit compiler-DSO topology/loading independently on Linux AArch64. One
  plain-Clang target executable now provides the same immediate-load proof on
  native x86-64 and QEMU AArch64, while shell-free Python parses both ELF
  dependency tables and writes the path-free result. A fresh 1,624-edge build
  passed W-004 5/5 in 85.13 seconds (topology 0.71 seconds), then repeated 5/5
  in 86.33 seconds from a Ninja no-op (topology 0.68 seconds). A fresh native
  x86-64 graph passed 6/6 in 2.47 seconds and repeated 6/6 in 2.47 seconds from
  a no-op; topology passed in 0.37 seconds both times.
- [x] Admit PerfSmoke independently on both Linux targets without broadening
  HandleLeak or ThreadHeavy at that admission point. Fresh AArch64 and
  x86-64 graphs generated from the same 262 Blueprint files passed their
  audited 2,113/2,089 compile-command and 2,196/2,172 Ninja-command contracts.
  The 1,625-edge AArch64 W-004 build passed 6/6 in 86.74 seconds; PerfSmoke
  passed in 1.66 seconds and repeated in 1.62 seconds after a true Ninja no-op.
  The 1,860-edge x86-64 W-004 build passed 7/7 in 2.67 seconds; PerfSmoke passed
  in 0.23 seconds and repeated in 0.24 seconds after a no-op. Both sanitized
...
- [x] Admit ThreadHeavy independently on both Linux targets without broadening
  HandleLeak at that admission point. Fresh x86-64 and AArch64 W-004
  ...
  links.
- [x] Admit HandleLeak independently on both Linux targets. The unchanged
  x86-64 and AArch64 graph audits retain 2,089/2,113 compile commands,
  2,172/2,196 Ninja commands, and 32 product links. Fresh 1,862/1,627-edge
  builds passed W-004 at 9/9 in 3.53 seconds and 8/8 in 99.23 seconds;
  HandleLeak passed in 0.36/2.56 seconds. True Ninja no-op repeats passed 9/9
  in 3.59 seconds and 8/8 in 99.55 seconds, with HandleLeak at 0.34/2.56
  seconds. Both records require exact-zero exit, 400 file and 80 socket
  cycles, the persisted final marker, and the complete ART/main marker set.
  Their manifests use normalized native/QEMU runner identities, the same
  content-addressed app and boot JARs, no absolute machine path, and no
  filesystem link.
- [x] Admit libcore CoreProbe independently on both Linux targets without
  broadening any other libcore declaration. The shared runner now forwards the
  explicit target runner for managed cases and rejects it for the Windows-only
  path modes. Fresh x86-64/AArch64 builds completed 1,863/1,628 edges and
  passed W-004 at 10/10 in 3.69 seconds and 9/9 in 92.17 seconds; CoreProbe
  passed in 0.23/1.55 seconds. True Ninja no-op repeats passed 10/10 in 3.68
  seconds and 9/9 in 92.92 seconds, with CoreProbe at 0.22/1.52 seconds. Both
  records require exact-zero exit plus arraycopy, UTF-8 charset, reflection,
  2,000-update monitor, and ART main-completion markers. Their manifests use
  the same content-addressed app/boot JARs, normalized native/QEMU identity,
  no absolute machine path, and no filesystem link.
- [x] Admit libcore InterruptProbe independently on both Linux targets without
  broadening another libcore selector. Fresh x86-64/AArch64 builds completed
  1,864/1,629 edges and passed W-004 at 11/11 in 3.82 seconds and 10/10 in
  93.88 seconds. True Ninja no-op repeats passed 11/11 in 3.88 seconds and
  10/10 in 93.98 seconds, with InterruptProbe at 0.22/1.51 seconds. Both
  records require exact-zero exit, sleeper termination after interruption,
  the complete probe marker set, and ART main completion. Their manifests use
  identical content-addressed app/boot JARs, normalized native/QEMU identity,
  no absolute machine path, and no filesystem link.
- [ ] Promote Linux AArch64 beyond the experimental thirteen-gate
  JNI/math/interpreter/allocator/compiler/handle/performance/threading/core/
  interruption slice, and
...
separate build/runtime verification status. Thirty compiled probes remain
truthfully limited to `windows-x86_64-msvc`; the CriticalNative and normal/
FastNative JNI libraries also apply to exact `linux-x86_64-gnu`. Across the
Windows product catalog, 23 compiled probes are compile-only and nine are
target-runnable. Fourteen `GATE` declarations avoid dummy binaries: three exact
`linux-x86_64-gnu` declarations check the runtime version marker and the
runtime/compiler ELF DSO topology plus image-backed boot/runtime packaging;
the Windows W-002, W-003, and W-004 host
...
PE review and twelve-process JIT control/workload matrix; the FS-1 reviewer
checks product isolation or variant stack sampling according to the exact build
identity; and W-027 rejects ANSI Win32 calls in the active
...
catalog builds 23 executable probes and 10 probe DLLs. Loading staged
...
`tests/` tree and places all 99 declarations under stable logical ownership.
All 35 native probes and 48 managed artifacts consume canonical source under
...
Boot-image production is a separate capability-gated Ninja edge over that
boot JAR and the target `dex2oat`. The Python helper uses argument vectors,
isolated target-local runtime directories, canonical logical locations,
bounded parallelism, deterministic compiler options, and atomic directory
replacement. It validates the complete ART/OAT/VDEX set even when `dex2oat`
returns zero, rejects embedded machine paths, and writes a relative hash/size
manifest. Native-runnable capable targets build the image as part of the
runtime package; cross hosts keep the ownership target valid but record
`not-built-cross-host`, while targets without the capability record
`unsupported`. Staging copies regular files only and revalidates the manifest
against the exact boot JAR. The first admitted identity is
`linux-x86_64-gnu`; Windows remains outside this capability until its distinct
AOT/OAT design is implemented and accepted.
...
required. W-031 adds the Windows-only AOT unwind transport and passes native
managed/JNI/seven-trampoline lookup, synthetic virtual unwind, and JIT-disabled
managed/JNI runtime calls. Because startup
upgrades many current entrypoints to nterp, it does not yet prove ordinary
boot-OAT RX dispatch.
...
- repeatable boot artifact generation and product-level image transaction,
  relocation, selection, and successful fallback behavior;
