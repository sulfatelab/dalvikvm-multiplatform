...
| Windows x86-64 product | PARTIAL / experimental | Linux-hosted cross and native Windows Server 2025 product builds pass; the accepted native baseline passes 70/70 across W-002, W-003, W-004, W-010, W-013, W-014, W-025, and W-027, and identical product/test repeats are Ninja no-ops | add boot-image/security packaging and migrate the remaining behavioral coverage |
...
| POSIX-free Windows build host | COMPLETE for the accepted native baseline; PARTIAL end to end | Server 2025 uses configured official JDK 21, Python, CMake, Ninja, and plain Clang drivers; all 70 accepted native tests and the product/test no-op gates pass without POSIX tooling | migrate every retained behavioral gate and complete runtime packaging |
...
- [x] Windows-target configuration emits the same 92 declarations and keeps
  90 items applicable. Sixty-three product-variant items are
  ...
  `host-review` declarations. Twenty applicable
...
| `w004` | 2 EXEs, 1 DLL, 33 managed, 3 gates | 7 exact / 32 typed | product: 31 target-runnable, 1 host-review, 7 compile-only; Windows applicable subset: 29 target-runnable, 1 host-review, 7 compile-only | remaining unregistered libcore behavior; no named L-003 case is native-open |
...
| Total | 22 EXEs, 10 DLLs, 47 managed, 13 gates | 32 exact / 60 typed | product: 65 target-runnable, 7 host-review, 20 compile-only | Windows applies 90 declarations; Linux x86-64 applies eleven |
...
- [x] Run the newly unified stage set on the authoritative Windows Server 2025
  ...
  at 70/70, with 63 target-runnable gates and seven host reviewers; its
