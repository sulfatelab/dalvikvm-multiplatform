...
| Windows x86-64 product | PARTIAL / experimental | Linux-hosted cross and native Windows Server 2025 product builds pass; fresh native W-002 passes 4/4, W-003 product passes 4/4, W-003 frame variant passes 5/5, expanded W-004 passes 21/21, W-010 passes 7/7, W-013 passes 7/7, and W-025 passes 8/8; every identical stage repeat is a Ninja no-op | run the remaining multi-stage catalog and migrate its behavioral tests |
...
| Unified phase catalog | PARTIAL | seven virtual stages declare 32 native probes, 47 managed JARs, and ten command gates; Windows has 87 applicable items (53 target-runnable, five host-review, and 29 compile-only in the product variant), while Linux x86-64 has seven applicable items (six runnable and one compile-only artifact) | migrate the remaining behavioral runners, portable JNI expansion, and result checks |
...
- [x] `PYTHONPATH=tools/bp2cmake python3 -m pytest tools/bp2cmake/tests tests/host -q`:
  157 passed, including generated PE-header, Linux/Windows test-catalog,
...
- [x] Fresh Linux configuration with Clang 21, CMake, Ninja, and configured
  JDK 21 emits an 89-declaration catalog. Seven declarations apply to
...
- [x] Windows-target configuration emits the same 89 declarations and keeps
  87 items applicable. Thirty-eight product-variant items are
  `target-runnable`, and the W-002 managed-entry, W-003 quick-boundary, W-004
  runtime-load, W-013 source-policy, and W-025 JIT-contract reviewers are
  separately registered `host-review` declarations. Forty-four applicable
...
- [x] W-004 now runs 13 accepted Phase-3 libcore behaviors through one
  ...
  separate L-003 matrix remain for the next semantic slice.
- [x] W-004 PathProbe and AbsPathProbe now use the same shell-free case runner
  ...
  were removed; only the generic Phase-3 builder/runner and L-003 matrix remain.
...
- [x] W-025 now owns seven shell-free target-runnable gates plus one
  ...
  `ProhibitDynamicCode`, and the explicit `-Xusejit:false` control. The native
  build used `--parallel 16` on the 16 GiB VM and its repeat was a Ninja no-op.
  All 52 result JSON files in the accumulated native result tree contain zero
  absolute paths, and its non-following scan found zero reparse points.
...
| `w004` | 2 EXEs, 1 DLL, 33 managed, 3 gates | 6 exact / 33 typed | product: 22 target-runnable, 1 host-review, 16 compile-only; Windows applicable subset: 20 target-runnable, 1 host-review, 16 compile-only | Windows embedding plus L-003 and the remaining unregistered libcore behavior |
| `w010` | 4 EXEs, 3 managed | 1 exact / 6 typed | 7 target-runnable | registered Windows x86-64 coverage is complete |
...
| `w025` | 4 EXEs, 3 DLLs, 3 managed, 1 gate | 6 exact / 5 typed | 7 target-runnable, 1 host-review, 3 compile-only | registered Windows x86-64 coverage is complete |
| Total | 22 EXEs, 10 DLLs, 47 managed, 10 gates | 25 exact / 64 typed | product: 55 target-runnable, 5 host-review, 29 compile-only | Windows applies 87 declarations; Linux x86-64 applies seven |

The shared registry now references zero source files from historical
verification directories. All 89 declarations own canonical source under
...
legacy evidence paths. It still contains 17 shell scripts, two PowerShell
...
target-runnable. Nine `GATE` declarations avoid dummy binaries: two exact
...
policy; and the FS-1 variant owns its stack-sampling host review. Managed W-002
...
`tests/` tree and places all 89 declarations under stable logical ownership.
...
points and shell runners temporarily reference canonical source; W-003 and the
W-004 JVMTI case have retired these compatibility paths. A portable VCS
