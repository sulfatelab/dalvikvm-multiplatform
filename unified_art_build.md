...
| Unified phase catalog | COMPLETE for current declaration truthfulness; PARTIAL target expansion | ten virtual stages declare 33 native probes, 47 managed JARs, and 15 command gates; Windows has 92 applicable items (70 target-runnable, nine host-review, and 13 compile-only in the product variant), Linux x86-64 has 18 applicable items (fifteen runnable and three compile-only artifacts), and experimental Linux AArch64 has 17 applicable items (fourteen runner-backed gates and three compile-only artifacts); CMake rejects runnable shared libraries and compile-only command gates; native W-028 and W-029 are accepted | expand exact-target applicability only with matching behavioral acceptance |
...
| Windows AOT/OAT | PARTIAL / separate track | Windows 64-KiB artifact alignment, shared-`libartbase` compiler topology, and W-028 trivial no-image OAT/VDEX generation gate are implemented; W-029 pins a single-component package-relative identity and passes its seven-case native preflight; W-028 passes natively with byte-identical artifacts after consuming the shared record | make Windows generation/startup consume W-029, then continue boot-set generation and executable loading in `win32_aot_oat_tracker.md` |
...
- [x] Windows AOT/OAT sequence step 1 passes the W-028 native operation gate.
  ...
  matches both hashes. Step 1 is complete; boot-image generation and
  executable OAT loading remain pending. See
...
- [x] Windows AOT/OAT sequence step 2 has an accepted W-029 identity
  ...
  mismatches. Step 2 remains partial until the real Windows generator and ART
  startup consume the contract. See `docs/history/windows_x64_w029_result.md`.
...
| `w029` | 1 gate | 1 exact / 0 typed | 1 host-review | native Server 2025 identity preflight passes and rejects all seven intentional mismatches; actual generation/startup remains pending |
| Total | 23 EXEs, 10 DLLs, 47 managed, 15 gates | 41 exact / 54 typed | product: 73 target-runnable, 9 host-review, 13 compile-only | Windows applies 92 declarations (70 runnable, nine host-review, 13 compile-only); Linux x86-64 applies 18 (fifteen runnable, three compile-only), and Linux AArch64 applies 17 (fourteen runnable, three compile-only) |

The shared registry now references zero source files from historical
verification directories. All 95 declarations own canonical source under
...
`tests/` tree and places all 95 declarations under stable logical ownership.
...
output matches them. This completes sequence step 1, but is not boot-image
generation or executable OAT loading.
