...
reviewer. Nineteen accepted Phase-3 libcore behaviors use one checked-in
JSON contract matrix and one case-local, shell-free Python runner. The expanded
native W-004 stage has 29 gates: ART embedding, core/charset/monitor, DNS,
...
relative names. Windows Server 2025 passed 29/29 twice with `--parallel 16`,
...
the JNI bridge maps bionic `SO_BROADCAST` to the Winsock option. ZipProbe is
the sole remaining native-open case. Historical Wine results do not promote
cases. The JVMTI runner, native agent, managed
...
Windows Server 2025 now passes W-027 as part of the complete 69/69 catalog;
the first run completed in 12.38 seconds and the no-op repeat in 12.33 seconds.
The complete catalog contains 62 target-runnable gates and seven host reviewers,
