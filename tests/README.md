...
hosts.
Nine W-004 managed declarations are `target-runnable` on explicit exact-target
...
InterruptProbe, and RtMem cover `linux-x86_64-gnu`, `linux-aarch64-gnu`, and
`windows-x86_64-msvc`; their shell-free runners supply QEMU user mode only for
the AArch64 target. The case-local
`math-critical/run.py` runs `-Xint` and threshold-zero JIT twice each. All nine
...
its boot-image gate. It therefore has twelve W-004 CTest gates, while AArch64
has eleven. With two shared W-003 gates and the shared W-013 gate, Linux x86-64
has fifteen runnable declarations and three compile-only artifacts; AArch64
has fourteen runnable declarations and three compile-only artifacts. Windows
adds its
...
InterruptProbe, and RtMem each have an independent exact selector for both
Linux targets and Windows x86-64; the other libcore declarations remain Windows
x86-64/MSVC-only. The expanded
...
thread stress, and expected-nonzero uncaught exception behavior join the earlier six.
...
L-003 Locale timeout no longer reproduces: its exact Windows contract now
requires locale/case, UTC calendar, and language-tag behavior but deliberately
excludes missing resource-backed display/collation data. UdpProbe passes after
the JNI bridge maps bionic `SO_BROADCAST` to the Winsock option. ZipProbe's
historical timeout also no longer reproduces: CRC32, raw DEFLATE/INFLATE,
multi-entry stream and `ZipFile` reads, content matching, and temporary-file
cleanup now pass as one exact Windows runtime contract. Historical Wine results
...
direct `Os` socket-address gate requires a positive loopback bind plus a
positive accepted loopback peer. AsyncCloseProbe requires both a blocked accept
and a blocked read to terminate after close, with no unexpected accepted peer.
The JVMTI runner, native agent, managed
...
reflection/monitor behavior and interruption, and the 128 MiB non-moving-heap
gate plus its managed artifact. Each runnable gate has its own behavioral
evidence under the explicit runner. The slice does not infer another JNI contract, boot-image,
