...
reviewer. Twenty accepted Phase-3 libcore behaviors use one checked-in
JSON contract matrix and one case-local, shell-free Python runner. The expanded
native W-004 stage has 30 gates: ART embedding, core/charset/monitor, DNS,
...
relative names. Windows Server 2025 passed 30/30 twice with `--parallel 16`,
...
do not promote cases. The JVMTI runner, native agent, managed
...
Windows Server 2025 now passes W-027 as part of the complete 70/70 catalog;
the first run completed in 12.38 seconds and the no-op repeat in 12.33 seconds.
The complete catalog contains 63 target-runnable gates and seven host reviewers,
