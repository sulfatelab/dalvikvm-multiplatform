...
reviewer. Twenty-one accepted Phase-3 libcore behaviors use one checked-in
JSON contract matrix and one case-local, shell-free Python runner. The expanded
native W-004 stage has 31 gates: ART embedding, core/charset/monitor, BigInteger,
DNS,
...
relative names. Windows Server 2025 passed 31/31 twice with `--parallel 16`,
...
round-trip, and exact deterministic output. The JVMTI runner, native agent, managed
...
Windows Server 2025 now passes W-027 as part of the complete 71/71 catalog;
the first run completed in 12.38 seconds and the no-op repeat in 12.33 seconds.
The complete catalog contains 64 target-runnable gates and seven host reviewers,
