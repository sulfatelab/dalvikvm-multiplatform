#!/usr/bin/env python3
"""Verify the Windows x64 W-004 direct Runtime::instance_ load contract."""
...
def check_source(repo: Path) -> None:
    ...
    if direct_load not in text:
        fail("Windows x64 LOAD_RUNTIME_INSTANCE is not the expected direct RIP-relative load")
    linux_load = (
...
def main() -> int:
    ...
    for path in (art_dll, jvmti_dll):
        if not path.is_file():
            fail(f"required Windows x64 artifact is missing: {path}")
