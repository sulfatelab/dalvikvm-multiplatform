...
The Windows x64 rSELF design remains `r15`; Windows GS remains the TEB and
...
The Windows x64 CFA is 96 bytes. Linux retains the original 80-byte CFA and does not
...
- the Windows x64 optimizing compiler excludes r15 from its compiled spill set.
...
`ArtMethod::Invoke`, whose Windows x64 quick boundary preserves native r15 and
...
- ordered Microsoft-to-SysV conversion and r15 publication in source and PE
  object code;
- preservation of Windows x64 rdi/rsi and the Linux 80-byte CFA;
...
| Control | Result |
|---------|--------|
| Complete Windows x64 build, `-j32` | PASS |
...
- creates `dist/windows_x64_w002_host.zip`.
